<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="cl_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['cl_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_x_contrato_comp']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_x_contrato_comp']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_recaudacion_x_contrato_comp"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_actualizar_cliente_cl_id = "<?php echo $this->form_encode_input($this->nmgp_dados_form['cl_id']); ?>";
function sc_btn_actualizar_cliente()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_actualizar_cliente(); }, 500);
      return;
    }
    sc_btn_actualizar_cliente_ok();
}
function sc_btn_actualizar_cliente_cancel()
{
}
function sc_btn_actualizar_cliente_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_cliente_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "cl_id*scin' + var_btn_actualizar_cliente_cl_id + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'form_del_cliente');
}
var Crtl_btn_facturar = false;
...
{
    Crtl_btn_facturar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?facturar?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_btn_salir_var_propietario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['cl_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_selecciona_contrato_comp_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_propietario*scin' + var_btn_btn_salir_var_propietario + '*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scout"; ?>', 'inicio', '_self', '0', '0', 'grid_selecciona_contrato_comp');
}
var var_btn_seleccionar_todo_var_propietario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['cl_id']); ?>";
function sc_btn_seleccionar_todo()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_seleccionar_todo(); }, 500);
      return;
    }
    sc_btn_seleccionar_todo_ok();
}
function sc_btn_seleccionar_todo_cancel()
{
}
function sc_btn_seleccionar_todo_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_selecciona_toda_operacion_contrato; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_propietario*scin' + var_btn_seleccionar_todo_var_propietario + '*scoutvar_tipo_documento*scin99*scoutSC_glo_par_var_contrato*scinvar_contrato*scout"; ?>', '', '_self', '0', '0', 'blank_selecciona_toda_operacion_contrato');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_x_contrato_comp']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_recaudacion_x_contrato_comp_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_recaudacion_x_contrato_comp_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function lookup_contrato(campo)
{
   dados  = document.F1.contrato.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_recaudacion_x_contrato_comp')?>/form_recaudacion_x_contrato_comp.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_contrato, "contrato", dados, false);
}
function recebe_contrato(string)
{
   document.getElementById('id_lookup_contrato').innerHTML = string;
}
<?php 
...
{
   nm_field_disabled("cl_identificacion=disabled;cl_nombre=disabled;cl_direccion=disabled;cl_telefono=disabled;cl_email=disabled;tesoreria=disabled", "");
   nm_field_disabled("total=disabled", "U");
