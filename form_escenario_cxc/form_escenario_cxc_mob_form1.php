<div id="form_escenario_cxc_mob_form1" style='<?php echo ($this->tabCssClass["form_escenario_cxc_mob_form1"]['class'] == 'scTabInactive' ? 'display: none; width: 1px; height: 0px; overflow: scroll' : ''); ?>'>
...
<?php
 if (isset($_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_Clientes'] ]) && '' != $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_Clientes'] ]) {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['grid_cliente_escenario_cxc_script_case_init'] = $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_Clientes'] ];
 }
 else {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['grid_cliente_escenario_cxc_script_case_init'] = $this->Ini->sc_page;
 }
 ...
 if (isset($this->Ini->sc_lig_md5["grid_cliente_escenario_cxc"]) && $this->Ini->sc_lig_md5["grid_cliente_escenario_cxc"] == "S") {
     ...
     $Md5_Lig    = "@SC_par@" . $this->form_encode_input($this->Ini->sc_page) . "@SC_par@form_escenario_cxc_mob@SC_par@" . md5($Parms_Lig);
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['Lig_Md5'][md5($Parms_Lig)] = $Parms_Lig;
 } else {
 ...
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_escenario_cxc_mob_empty.htm' : $this->Ini->link_grid_cliente_escenario_cxc_cons . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page) . '&script_case_detail=Y&sc_ifr_height=450' . $parms_lig_cons;
...
if (isset($this->Ini->sc_lig_target['C_@scinf_Clientes']) && 'nmsc_iframe_liga_grid_cliente_escenario_cxc' != $this->Ini->sc_lig_target['C_@scinf_Clientes'])
{
    ...
    {
        $sDetailSrc .= '&under_dashboard=1&dashboard_app=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['dashboard_info']['dashboard_app'] . '&own_widget=' . $this->Ini->sc_lig_target['C_@scinf_Clientes'] . '&parent_widget=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['dashboard_info']['own_widget'];
        $sDetailSrc  = $this->addUrlParam($sDetailSrc, 'script_case_init', $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['grid_cliente_escenario_cxc_script_case_init']);
    }
?>
...
<script> 
<?php
 $NM_pag_atual = "form_escenario_cxc_mob_form0";
 if (isset($this->nmgp_ancora) && $this->nmgp_ancora != "")
 {
     $NM_pag_atual = "form_escenario_cxc_mob_form" . $this->nmgp_ancora;
 }
?>
<?php
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("form_escenario_cxc_mob");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("form_escenario_cxc_mob");
 parent.scAjaxDetailHeight("form_escenario_cxc_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
<script type="text/javascript">
	...
	function scBtnFn_sys_format_alt() {
		...
		if ($("#sc_b_upd_b.sc-unique-btn-24").length && $("#sc_b_upd_b.sc-unique-btn-24").is(":visible")) {
		    if ($("#sc_b_upd_b.sc-unique-btn-24").hasClass("disabled")) {
		        return;
		    }
			nm_atualiza ('alterar');
			toggleToolbar(event, true); return;
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc_mob']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
