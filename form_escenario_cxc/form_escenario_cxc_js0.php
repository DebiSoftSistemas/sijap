...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_escenario_cxc"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_escenario_cxc_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_escenario_cxc_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("fac_subtotal=disabled;fac_subtotal_iva=disabled;fac_subtotal_cero=disabled;fac_subtotal_no_objeto=disabled;fac_subtotal_excento=disabled;fac_total_descuento=disabled;fac_valor_ice=disabled;fac_valor_iva=disabled;fac_valor_irbpnr=disabled;fac_total=disabled;fac_estado=disabled", "");
...
{
    document.getElementById('form_escenario_cxc_form0').style.width='1px';
    document.getElementById('form_escenario_cxc_form0').style.height='0px';
    document.getElementById('form_escenario_cxc_form0').style.display='none';
    document.getElementById('form_escenario_cxc_form0').style.overflow='scroll';
    document.getElementById('form_escenario_cxc_form1').style.width='1px';
    document.getElementById('form_escenario_cxc_form1').style.height='0px';
    document.getElementById('form_escenario_cxc_form1').style.display='none';
    document.getElementById('form_escenario_cxc_form1').style.overflow='scroll';
<?php
    if (!$_SESSION['sc_session'][$this->Ini->sc_page]['form_escenario_cxc']['pdf_view']) {
?>
...
?>
   ...
   if ("form_escenario_cxc_form0" == N_pagina) {
     scAjaxDetailHeight("form_detalle_escenario_cxc", "450");
    }
   if ("form_escenario_cxc_form1" == N_pagina) {
     scAjaxDetailHeight("grid_cliente_escenario_cxc", "450");
    }
}
