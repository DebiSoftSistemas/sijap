<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="fac_numero" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['fac_numero']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_factura"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_factura; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin01*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_factura');
}
var var_btn_ride_var_tipo_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['fac_tipo_comprobante']); ?>";
var var_btn_ride_var_id_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['fac_numero']); ?>";
...
var var_btn_retencion_var_id_factura_venta = "<?php echo $this->form_encode_input($this->nmgp_dados_form['fac_numero']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_retencion_ventas; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_id_factura_venta*scin' + var_btn_retencion_var_id_factura_venta + '*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_retencion_ventas');
}
var var_btn_bauchert_var_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['fac_numero']); ?>";
function sc_btn_bauchert()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_bauchert(); }, 500);
      return;
    }
    sc_btn_bauchert_ok();
}
function sc_btn_bauchert_cancel()
{
}
function sc_btn_bauchert_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_factura_baucher; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_documento*scin' + var_btn_bauchert_var_documento + '*scout"; ?>', '', '_blank', '0', '0', 'pdf_factura_baucher');
}
function sc_btn_facturar_socio()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_facturar_socio(); }, 500);
      return;
    }
    sc_btn_facturar_socio_ok();
}
function sc_btn_facturar_socio_cancel()
{
}
function sc_btn_facturar_socio_ok()
{
...
{
    Crtl_btn_salir = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?salir?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_factura']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_factura_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_factura_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("secuencial=disabled;establecimiento=disabled;punto_emision=disabled;tipo_libretin=disabled;ambiente=disabled;ruc=disabled;fac_nombre_cliente=disabled;fac_direccion_cliente=disabled;fac_email_cliente=disabled;fac_subtotal=disabled;fac_subtotal_iva=disabled;fac_subtotal_cero=disabled;fac_subtotal_no_objeto=disabled;fac_subtotal_excento=disabled;fac_total_descuento=disabled;fac_valor_ice=disabled;fac_valor_iva=disabled;fac_valor_irbpnr=disabled;fac_propina=disabled;fac_total=disabled;fac_cambio=disabled", "");
