...
//
   function init($Tp_init = "")
   {
      ...
      $this->nm_dt_ult_alt   = "20250620"; 
      $this->nm_hr_ult_alt   = "093306"; 
...
<?php
   ...
   function conectDB()
   {
      ...
      { 
          $this->Db = db_conect_devel($_SESSION['scriptcase']['grid_del_factura']['glo_nm_conexao'], $this->root . $this->path_prod, 'SIJAP', 1, $this->force_db_utf8); 
      } 
      else 
      { 
          ob_start();
...
//----- 
   function controle($linhas = 0)
   {
      ...
      if (isset($_POST['nmgp_opcao']) && $_POST['nmgp_opcao'] == "ajax_event")
      {
          ...
          if ($_POST['nmgp_event'] == "actbtn_contabilizar_onClick")
          {
              $this->fac_numero = NM_charset_decode($_POST['fac_numero']);
              $this->actionbar_clicked_state = NM_charset_decode($_POST['actionbar_clicked_state']);
              $this->Ini->sc_page = filter_input(INPUT_POST, 'script_case_init', FILTER_SANITIZE_NUMBER_INT);
              $this->actbtn_contabilizar_onClick();
          }
          $Temp = ob_get_clean();
          if ($Temp !== false)
          {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_factura']['SC_All_Groupby'] = array('sc_free_total' => 'grid');
...
if ($this->rs[0][0]=="SI"){
	$w_respuesta=$this->anularFactura($this->fac_numero ,$this->sc_temp_Igtech_RucEmpresa);   
}
$check_sql = "SELECT * from sp_anula_pago_factura_credito (".$this->fac_numero .") as (respuesta int,mensaje varchar);";
...
$check_sql = "SELECT * from sp_anula_pago_anticipo (".$this->fac_numero .") as (respuesta int,mensaje varchar);";
...
$update_sql="UPDATE fac_operacion 
				...
				WHERE op_factura=".$this->fac_numero .";";
...
$_SESSION['scriptcase']['grid_del_factura']['contr_erro'] = 'off'; 
  ...
  function actbtn_btn_autorizar_onClick()
  {
    ...
    $this->NM_cmp_hidden = array();
    $_SESSION['scriptcase']['grid_del_factura']['contr_erro'] = 'on';
  $this->sc_ajax_message($this->autorizar_factura($this->fac_numero ,'S'), "Aviso", "modal = Y & button = Y & button_label = Ok & body_icon = Y & redir = grid_del_factura.php", "toast=Y&toasto_pos=center-start&type=success");
...
$_SESSION['scriptcase']['grid_del_factura']['contr_erro'] = 'off'; 
  ...
  function actbtn_contabilizar_onClick()
  {
    $Old_fac_numero = $this->fac_numero;
    $this->NM_ajax_event = true;
    $this->NM_cmp_hidden = array();
...
if ($this->rs[0][0]=="SI"){
	$w_respuesta=$this->enviaFactura($this->fac_numero ,$this->sc_temp_Igtech_RucEmpresa);
	$this->sc_ajax_message($w_respuesta['mensaje'], "Aviso", "modal = Y & button = Y & button_label = Ok & body_icon = Y & redir = grid_del_factura.php", "toast=Y&toasto_pos=center-start&type=success");
}
...
//======= =========================
   ...
   if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
       ...
       elseif (is_file($root . $_SESSION['scriptcase']['grid_del_factura']['glo_nm_path_imag_temp'] . "/sc_apl_default_SIJAP.txt")) {
           $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['grid_del_factura']['glo_nm_path_imag_temp'] . "/sc_apl_default_SIJAP.txt"));
       }
       if (isset($apl_def)) {
           ...
           if (isset($_COOKIE['sc_actual_lang_SIJAP'])) {
               $_SESSION['scriptcase']['grid_del_factura']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_SIJAP'];
           }
       }
   }
