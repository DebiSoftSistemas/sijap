...
$old_error_handler = set_error_handler("factura_rapida_mob_scriptcase_error_handler");

function factura_rapida_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['factura_rapida_mob']['contr_erro']) && $_SESSION['scriptcase']['factura_rapida_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_factura_rapida_mob->contr_factura_rapida_mob->Apl_com_erro = true;
            if ($inicial_factura_rapida_mob->contr_factura_rapida_mob->NM_ajax_flag)
            {
                $inicial_factura_rapida_mob->contr_factura_rapida_mob->NM_ajax_info['errList']['geral_factura_rapida_mob'][] = $msg;
                return;
            }
            factura_rapida_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function factura_rapida_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['factura_rapida_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['factura_rapida_mob'])  ? $_SESSION['scriptcase']['error_icon']['factura_rapida_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['factura_rapida_mob']) && '' != $_SESSION['scriptcase']['error_close']['factura_rapida_mob']) ? $_SESSION['scriptcase']['error_close']['factura_rapida_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['factura_rapida_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['factura_rapida_mob']) ? $_SESSION['scriptcase']['error_span_title']['factura_rapida_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['factura_rapida_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['factura_rapida_mob']) ? $_SESSION['scriptcase']['error_icon_title']['factura_rapida_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['factura_rapida']['error_buffer'] .= ob_get_flush();
}
