...
SCEOT;
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_periodo_baja = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['arr_total']['periodo_baja'];
          $this->array_total_mes_baja = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['arr_total']['mes_baja'];
      }
      else
      {
          $this->array_total_periodo_baja = array();
   ...
   function completeMatrix()
   {
       ...
       {
       $Lab_periodo_baja = "Periodo Baja";
       $Lab_mes_baja = "Mes Baja";
       }
       $this->comp_field = array(
           $Lab_periodo_baja,
           $Lab_mes_baja,
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_order']['Mensual']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_order']['Mensual'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_order']['Mensual'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['summarizing_fields_display']['Mensual'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_periodo_baja as $i_periodo_baja => $d_periodo_baja) {
           if (!isset($i_periodo_baja, $this->comp_label[0]) || !in_array($i_periodo_baja, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_periodo_baja[5] ] = $d_periodo_baja[4];
               $this->comp_label[0][ $d_periodo_baja[5] ] = $d_periodo_baja[4];
           }
           foreach ($this->array_total_mes_baja[$i_periodo_baja] as $i_mes_baja => $d_mes_baja) {
               if (!isset($i_mes_baja, $this->comp_label[1][ $d_periodo_baja[5] ]) || !in_array($i_mes_baja, $this->comp_label[1][ $d_periodo_baja[5] ], true)) {
                   $this->comp_index[1][ $d_mes_baja[5] ] = $d_mes_baja[4];
                   $this->comp_label[1][ $d_periodo_baja[5] ][ $d_mes_baja[5] ] = $d_mes_baja[4];
               }
           }
       }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'periodo_baja', 'prot' => ''),
           array('name' => 'mes_baja', 'prot' => '@aspass@'),
       );
     ...
     if (isset($this->comp_label[0])) {
       foreach ($this->comp_label[0] as $i_periodo_baja => $l_periodo_baja) {
           if (isset($this->array_total_periodo_baja[$i_periodo_baja])) {
               $this->comp_group[$i_periodo_baja] = array();
           }
           foreach ($this->comp_label[1][$i_periodo_baja] as $i_mes_baja => $l_mes_baja) {
               if (isset($this->array_total_mes_baja[$i_periodo_baja][$i_mes_baja])) {
                   $this->comp_group[$i_periodo_baja][$i_mes_baja] = array();
               }
           }
       }
     }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_periodo_baja;
       }
       elseif (1 == $level) {
           $return = $this->array_total_mes_baja;
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_Mensual("res", $this->NM_export);
      ...
      if (is_array($this->array_total_periodo_baja)) {
          ksort($this->array_total_periodo_baja);
      }
      if (is_array($this->array_total_mes_baja)) {
          ksort($this->array_total_mes_baja);
      }
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_anulada']['SC_Ind_Groupby'];
...
{ 
   ...
   //---- 
   function adiciona_registro($sum_do_total, $sum_do_subtotal, $sum_do_valor_iva, $quebra_periodo_baja, $quebra_mes_baja, $quebra_periodo_baja_orig, $quebra_mes_baja_orig)
   {
      //----- Periodo Anulación
      if (!isset($this->array_total_periodo_baja[$quebra_periodo_baja_orig]))
      {
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][0] = 1;
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][1] = $sum_do_total;
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][2] = $sum_do_subtotal;
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][3] = $sum_do_valor_iva;
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][4] = $quebra_periodo_baja;
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][5] = $quebra_periodo_baja_orig;
      }
      else
      {
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][0]++;
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][1] = bcadd($this->array_total_periodo_baja[$quebra_periodo_baja_orig][1], $sum_do_total, 10);
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][2] = bcadd($this->array_total_periodo_baja[$quebra_periodo_baja_orig][2], $sum_do_subtotal, 10);
         $this->array_total_periodo_baja[$quebra_periodo_baja_orig][3] = bcadd($this->array_total_periodo_baja[$quebra_periodo_baja_orig][3], $sum_do_valor_iva, 10);
      }
      //----- Mes Anulación
      if (!isset($this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig]))
      {
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][0] = 1;
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][1] = $sum_do_total;
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][2] = $sum_do_subtotal;
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][3] = $sum_do_valor_iva;
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][4] = $quebra_mes_baja;
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][5] = $quebra_mes_baja_orig;
      }
      else
      {
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][0]++;
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][1] = bcadd($this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][1], $sum_do_total, 10);
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][2] = bcadd($this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][2], $sum_do_subtotal, 10);
         $this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][3] = bcadd($this->array_total_mes_baja[$quebra_periodo_baja_orig][$quebra_mes_baja_orig][3], $sum_do_valor_iva, 10);
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('Mensual', 'periodo_baja');
     $campos_order = $this->Ini->Get_date_order_groupby("periodo_baja", 'asc', $format, $campos_order);
     $format       = $this->Ini->Get_Gb_date_format('Mensual', 'mes_baja');
     $campos_order = $this->Ini->Get_date_order_groupby("mes_baja", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->con_ruta = $this->look_con_ruta ;
            $this->op_estado = $this->look_op_estado ;
            $this->mes_baja = $this->look_mes_baja ;
            nmgp_Form_Num_Val($this->periodo_baja, "", ",", "0", "S", "2", "", "N:", "-") ; 
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->do_total, $this->do_subtotal, $this->do_valor_iva, $this->periodo_baja, $this->mes_baja, $this->periodo_baja_orig,  $this->mes_baja_orig);
            }
   }
//
