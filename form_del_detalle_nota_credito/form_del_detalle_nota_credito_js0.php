...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_detalle_nota_credito"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var Crtl_btn_borrar_todo = false;
function sc_btn_borrar_todo()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_borrar_todo(); }, 500);
      return;
    }
    if (Crtl_btn_borrar_todo) {return;}
    sc_btn_borrar_todo_ok();
}
function sc_btn_borrar_todo_cancel()
{
}
function sc_btn_borrar_todo_ok()
{
    Crtl_btn_borrar_todo = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?borrar_todo?@?";
    document.F1.action = "./";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_del_detalle_nota_credito_submit_form(y, z);
...
{
   nm_field_disabled("dnc_porcentaje_iva_=disabled", "U");
