<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="dp_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['dp_id_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_detalle_proforma_facturar"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_detalle_proforma_facturar']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_detalle_proforma_facturar']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_detalle_proforma_facturar']['dashboard_info']['parent_widget'] . "']\")";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_del_detalle_proforma_facturar_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_dp_producto_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.dp_producto_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_detalle_proforma_facturar')?>/form_del_detalle_proforma_facturar.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_dp_producto_, "dp_producto_", dados, false);
}
function recebe_dp_producto_(string)
{
   document.getElementById('id_lookup_dp_producto_' + nm_num_campo).innerHTML = string;
}
<?php 
