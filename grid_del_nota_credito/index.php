...
<?php
   ...
   function conectDB()
   {
      ...
      { 
          $this->Db = db_conect_devel($_SESSION['scriptcase']['grid_del_nota_credito']['glo_nm_conexao'], $this->root . $this->path_prod, 'SIJAP', 1, $this->force_db_utf8); 
      } 
      else 
      { 
          ob_start();
...
//----- 
   function controle($linhas = 0)
   {
      ...
      if (isset($_POST['nmgp_opcao']) && $_POST['nmgp_opcao'] == "ajax_event")
      {
          ...
          {
              $this->nc_numero = NM_charset_decode($_POST['nc_numero']);
              $this->nc_empresa = NM_charset_decode($_POST['nc_empresa']);
              $this->actionbar_clicked_state = NM_charset_decode($_POST['actionbar_clicked_state']);
              $this->Ini->sc_page = filter_input(INPUT_POST, 'script_case_init', FILTER_SANITIZE_NUMBER_INT);
...
$insert_sql="INSERT INTO del_cuentas_por_pagar(
							...
							SELECT  getdate(),
									...
									nc_tipo_comprobante
							FROM v_del_datos_nota_credito_sri 
							join del_forma_pago_nota_credito fpnc on nc_numero=fp_nota_credito 
							join del_forma_pago fp on fp.fp_id=fpnc.fp_forma_pago 
							WHERE fp.fp_compra_venta='NC'
							and nc_numero=".$this->nc_numero ;
...
$_SESSION['scriptcase']['grid_del_nota_credito']['contr_erro'] = 'off'; 
  ...
  {
    $Old_nc_numero = $this->nc_numero;
    $Old_nc_empresa = $this->nc_empresa;
    $this->NM_ajax_event = true;
    $this->NM_cmp_hidden = array();
    $_SESSION['scriptcase']['grid_del_nota_credito']['contr_erro'] = 'on';
...
if ($this->rs[0][0]=="SI"){
	$w_respuesta=$this->enviaNotaCredito($this->nc_numero ,$nc_empresa );
}
...
$_SESSION['scriptcase']['grid_del_nota_credito']['contr_erro'] = 'off'; 
    ...
    if (!is_object($this->nc_empresa) && $Old_nc_empresa != $this->nc_empresa)
    {
        $this->Format_Ajax_return("nc_empresa");
        $this->Ini->Arr_result['setValue'][] = array('field' => 'nc_empresa', 'value' => NM_charset_to_utf8($this->nc_empresa));
    }
    if (!empty($this->NM_cmp_hidden))
    {
        foreach ($this->NM_cmp_hidden as $cmp => $val)
...
//======= =========================
   ...
   if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
       ...
       elseif (is_file($root . $_SESSION['scriptcase']['grid_del_nota_credito']['glo_nm_path_imag_temp'] . "/sc_apl_default_SIJAP.txt")) {
           $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['grid_del_nota_credito']['glo_nm_path_imag_temp'] . "/sc_apl_default_SIJAP.txt"));
       }
       if (isset($apl_def)) {
           ...
           if (isset($_COOKIE['sc_actual_lang_SIJAP'])) {
               $_SESSION['scriptcase']['grid_del_nota_credito']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_SIJAP'];
           }
       }
   }
