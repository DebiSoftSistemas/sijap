...
class grid_del_admin_empresas_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_del_admin_empresas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_del_admin_empresas']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('emp_tipo_suscripcion' => "emp_tipo_suscripcion",'emp_fecha_crea' => "emp_fecha_crea");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('emp_ambiente_sri' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['emp_tipo_suscripcion']['reg'] = "S";
      $format_dimensions['emp_tipo_suscripcion']['msk'] = "";
      $format_dimensions['emp_fecha_crea']['reg'] = "N";
      $format_dimensions['emp_fecha_crea']['msk'] = "F/Y";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "emp_tipo_suscripcion") {
                  $this->Lookup->lookup_sc_free_group_by_emp_tipo_suscripcion($$Cada_dim,  $emp_tipo_suscripcion);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang , $emp_tipo_suscripcion;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_del_admin_empresas']['contr_total_geral'] = "OK";
   } 

   //-----  emp_tipo_suscripcion
   function quebra_emp_tipo_suscripcion_sc_free_group_by($emp_tipo_suscripcion, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $emp_tipo_suscripcion;
      $tot_emp_tipo_suscripcion = array() ;  
      $tot_emp_tipo_suscripcion[0] = $emp_tipo_suscripcion ; 
   }
   //-----  emp_fecha_crea
   function quebra_emp_fecha_crea_sc_free_group_by($emp_fecha_crea, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $emp_tipo_suscripcion;
      $tot_emp_fecha_crea = array() ;  
      $tot_emp_fecha_crea[0] = $emp_fecha_crea ; 
   }
