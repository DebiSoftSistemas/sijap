...
class grid_fac_cuotas_convenio_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_fac_cuotas_convenio']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_fac_cuotas_convenio']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $cc_producto;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(cc_capital), sum(cc_interes), sum(cc_valor_iva), sum(cc_valor_cuota) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['tot_geral'][5] = $rt->fields[4]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['contr_total_geral'] = "OK";
   } 

   //-----  cc_contrato
   function quebra_cc_contrato_contrato($cc_contrato, $arg_sum_cc_contrato) 
   {
      global $tot_cc_contrato, $cc_producto;
      $tot_cc_contrato = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(cc_capital), sum(cc_interes), sum(cc_valor_iva), sum(cc_valor_cuota) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['where_pesq']; 
      } 
      ...
      { 
         $nm_comando .= " where cc_contrato" . $arg_sum_cc_contrato ; 
      } 
      else 
      { 
         $nm_comando .= " and cc_contrato" . $arg_sum_cc_contrato ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_cc_contrato[0] = NM_encode_input(sc_strip_script($cc_contrato)) ; 
      $tot_cc_contrato[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_cc_contrato[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_cc_contrato[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_cc_contrato[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_cc_contrato[5] = (string)$rt->fields[4]; 
      $rt->Close(); 
   } 

   //-----  cc_cuota
   function quebra_cc_cuota_contrato($cc_contrato, $cc_cuota, $arg_sum_cc_contrato, $arg_sum_cc_cuota) 
   {
      global $tot_cc_cuota, $cc_producto;
      $tot_cc_cuota = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(cc_capital), sum(cc_interes), sum(cc_valor_iva), sum(cc_valor_cuota) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_fac_cuotas_convenio']['where_pesq'])) 
      { 
         $nm_comando .= " where cc_contrato" . $arg_sum_cc_contrato . " and cc_cuota" . $arg_sum_cc_cuota ; 
      } 
      else 
      { 
         $nm_comando .= " and cc_contrato" . $arg_sum_cc_contrato . " and cc_cuota" . $arg_sum_cc_cuota ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_cc_cuota[0] = NM_encode_input(sc_strip_script($cc_cuota)) ; 
      $tot_cc_cuota[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_cc_cuota[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_cc_cuota[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_cc_cuota[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_cc_cuota[5] = (string)$rt->fields[4]; 
      $rt->Close(); 
   } 
