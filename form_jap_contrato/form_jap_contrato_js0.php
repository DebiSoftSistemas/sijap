...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_contrato']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_contrato']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_jap_contrato"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var Crtl_btn_enviar_inspeccion = false;
function sc_btn_enviar_inspeccion()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_enviar_inspeccion(); }, 500);
      return;
    }
    if (Crtl_btn_enviar_inspeccion) {return;}
    sc_btn_enviar_inspeccion_ok();
}
function sc_btn_enviar_inspeccion_cancel()
{
}
function sc_btn_enviar_inspeccion_ok()
{
    Crtl_btn_enviar_inspeccion = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?enviar_inspeccion?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var Crtl_btn_enviar_legalizar = false;
function sc_btn_enviar_legalizar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_enviar_legalizar(); }, 500);
      return;
    }
    if (Crtl_btn_enviar_legalizar) {return;}
    sc_btn_enviar_legalizar_ok();
}
function sc_btn_enviar_legalizar_cancel()
{
}
function sc_btn_enviar_legalizar_ok()
{
    Crtl_btn_enviar_legalizar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?enviar_legalizar?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
function sc_btn_anular()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('Realmente desea Rechazar la Solicitud?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_anular_ok, sc_btn_anular_cancel)
}
...
var Crtl_btn_actualizar = false;
function sc_btn_actualizar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_actualizar(); }, 500);
      return;
    }
    if (Crtl_btn_actualizar) {return;}
    sc_btn_actualizar_ok();
}
function sc_btn_actualizar_cancel()
{
}
function sc_btn_actualizar_ok()
{
    Crtl_btn_actualizar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?actualizar?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var Crtl_btn_btnImprimir = false;
function sc_btn_btnImprimir()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btnImprimir(); }, 500);
      return;
    }
    if (Crtl_btn_btnImprimir) {return;}
    sc_btn_btnImprimir_ok();
}
function sc_btn_btnImprimir_cancel()
{
}
function sc_btn_btnImprimir_ok()
{
    Crtl_btn_btnImprimir = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?btnImprimir?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_ImprimirSolicitud_var_contrato = "<?php echo $this->form_encode_input($this->nmgp_dados_form['con_id']); ?>";
function sc_btn_ImprimirSolicitud()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_ImprimirSolicitud(); }, 500);
      return;
    }
    sc_btn_ImprimirSolicitud_ok();
}
function sc_btn_ImprimirSolicitud_cancel()
{
}
function sc_btn_ImprimirSolicitud_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_form_jap_solicitud_ap_pdf; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_contrato*scin' + var_btn_ImprimirSolicitud_var_contrato + '*scout"; ?>', '', '_blank', '0', '0', 'form_jap_solicitud_ap_pdf');
}
var Crtl_btn_legalizar = false;
function sc_btn_legalizar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_legalizar(); }, 500);
      return;
    }
    if (Crtl_btn_legalizar) {return;}
    sc_btn_legalizar_ok();
}
function sc_btn_legalizar_cancel()
{
}
function sc_btn_legalizar_ok()
{
    Crtl_btn_legalizar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?legalizar?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_ver_contratos_var_ruta = "<?php echo $this->form_encode_input($this->nmgp_dados_form['con_ruta']); ?>";
function sc_btn_ver_contratos()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_ver_contratos(); }, 500);
      return;
    }
    sc_btn_ver_contratos_ok();
}
function sc_btn_ver_contratos_cancel()
{
}
function sc_btn_ver_contratos_ok()
{
  tb_show('', '<?php echo $this->Ini->link_grid_v_jap_rutas_medicion_cons; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "var_ruta*scin' + var_btn_ver_contratos_var_ruta + '*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scoutNMSC_cab*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=800&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_contrato']['where_detal']))
  {
?>  
...
function lookup_con_estado(campo)
{
   dados  = document.F1.con_estado.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_jap_contrato')?>/form_jap_contrato.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_con_estado, "con_estado", dados, false);
}
function recebe_con_estado(string)
{
   document.getElementById('id_lookup_con_estado').innerHTML = string;
}
function lookup_con_usuario_instala(campo)
{
   dados  = document.F1.con_usuario_instala.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_jap_contrato')?>/form_jap_contrato.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_con_usuario_instala, "con_usuario_instala", dados, false);
}
function recebe_con_usuario_instala(string)
{
   document.getElementById('id_lookup_con_usuario_instala').innerHTML = string;
}
<?php 
