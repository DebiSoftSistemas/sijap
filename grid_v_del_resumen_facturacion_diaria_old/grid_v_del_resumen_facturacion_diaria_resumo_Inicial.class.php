...
class grid_v_del_resumen_facturacion_diaria_resumo
{
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_pro_codigo           = array();
...
SCEOT;
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_pro_codigo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['arr_total']['pro_codigo'];
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['arr_total']['pro_codigo'] = $this->array_total_pro_codigo;
   ...
   function completeMatrix()
   {
       ...
       //-----
       foreach ($this->array_total_pro_codigo as $i_pro_codigo => $d_pro_codigo) {
           if (!isset($i_pro_codigo, $this->comp_label[0]) || !in_array($i_pro_codigo, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_pro_codigo[6] ] = $d_pro_codigo[5];
               $this->comp_label[0][ $d_pro_codigo[6] ] = $d_pro_codigo[5];
           }
           foreach ($this->array_total_fac_tipo_comprobante[$i_pro_codigo] as $i_fac_tipo_comprobante => $d_fac_tipo_comprobante) {
               if (!isset($i_fac_tipo_comprobante, $this->comp_label[1][ $d_pro_codigo[6] ]) || !in_array($i_fac_tipo_comprobante, $this->comp_label[1][ $d_pro_codigo[6] ], true)) {
                   $this->comp_index[1][ $d_fac_tipo_comprobante[6] ] = $d_fac_tipo_comprobante[5];
                   $this->comp_label[1][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ] = $d_fac_tipo_comprobante[5];
               }
               foreach ($this->array_total_fac_serie[$i_pro_codigo][$i_fac_tipo_comprobante] as $i_fac_serie => $d_fac_serie) {
                   if (!isset($i_fac_serie, $this->comp_label[2][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ]) || !in_array($i_fac_serie, $this->comp_label[2][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ], true)) {
                       $this->comp_index[2][ $d_fac_serie[6] ] = $d_fac_serie[5];
                       $this->comp_label[2][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ][ $d_fac_serie[6] ] = $d_fac_serie[5];
                   }
                   foreach ($this->array_total_concepto_factura[$i_pro_codigo][$i_fac_tipo_comprobante][$i_fac_serie] as $i_concepto_factura => $d_concepto_factura) {
                       if (!isset($i_concepto_factura, $this->comp_label[3][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ][ $d_fac_serie[6] ]) || !in_array($i_concepto_factura, $this->comp_label[3][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ][ $d_fac_serie[6] ], true)) {
                           $this->comp_index[3][ $d_concepto_factura[6] ] = $d_concepto_factura[5];
                           $this->comp_label[3][ $d_pro_codigo[6] ][ $d_fac_tipo_comprobante[6] ][ $d_fac_serie[6] ][ $d_concepto_factura[6] ] = $d_concepto_factura[5];
                       }
                   }
               }
           }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'pro_codigo', 'prot' => '@aspass@'),
     ...
     if (isset($this->comp_label[0])) {
       foreach ($this->comp_label[0] as $i_pro_codigo => $l_pro_codigo) {
           if (isset($this->array_total_pro_codigo[$i_pro_codigo])) {
               $this->comp_group[$i_pro_codigo] = array();
           }
           foreach ($this->comp_label[1][$i_pro_codigo] as $i_fac_tipo_comprobante => $l_fac_tipo_comprobante) {
               if (isset($this->array_total_fac_tipo_comprobante[$i_pro_codigo][$i_fac_tipo_comprobante])) {
                   $this->comp_group[$i_pro_codigo][$i_fac_tipo_comprobante] = array();
               }
               foreach ($this->comp_label[2][$i_pro_codigo][$i_fac_tipo_comprobante] as $i_fac_serie => $l_fac_serie) {
                   if (isset($this->array_total_fac_serie[$i_pro_codigo][$i_fac_tipo_comprobante][$i_fac_serie])) {
                       $this->comp_group[$i_pro_codigo][$i_fac_tipo_comprobante][$i_fac_serie] = array();
                   }
                   foreach ($this->comp_label[3][$i_pro_codigo][$i_fac_tipo_comprobante][$i_fac_serie] as $i_concepto_factura => $l_concepto_factura) {
                       if (isset($this->array_total_concepto_factura[$i_pro_codigo][$i_fac_tipo_comprobante][$i_fac_serie][$i_concepto_factura])) {
                           $this->comp_group[$i_pro_codigo][$i_fac_tipo_comprobante][$i_fac_serie][$i_concepto_factura] = array();
                       }
                   }
               }
           }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_pro_codigo;
       }
       elseif (1 == $level) {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      ...
      if (is_array($this->array_total_pro_codigo)) {
          ksort($this->array_total_pro_codigo);
      }
...
{ 
   ...
   //---- 
   function adiciona_registro($sum_df_subtotal, $sum_df_descuento, $sum_df_valor_iva, $sum_df_total, $quebra_pro_codigo, $quebra_fac_tipo_comprobante, $quebra_fac_serie, $quebra_concepto_factura, $quebra_pro_codigo_orig, $quebra_fac_tipo_comprobante_orig, $quebra_fac_serie_orig, $quebra_concepto_factura_orig)
   {
      //----- Pro Codigo
      if (!isset($this->array_total_pro_codigo[$quebra_pro_codigo_orig]))
      {
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][0] = 1;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][1] = $sum_df_subtotal;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][2] = $sum_df_descuento;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][3] = $sum_df_valor_iva;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][4] = $sum_df_total;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][5] = $quebra_pro_codigo;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][6] = $quebra_pro_codigo_orig;
      }
      else
      {
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][0]++;
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][1] = bcadd($this->array_total_pro_codigo[$quebra_pro_codigo_orig][1], $sum_df_subtotal, 10);
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][2] = bcadd($this->array_total_pro_codigo[$quebra_pro_codigo_orig][2], $sum_df_descuento, 10);
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][3] = bcadd($this->array_total_pro_codigo[$quebra_pro_codigo_orig][3], $sum_df_valor_iva, 10);
         $this->array_total_pro_codigo[$quebra_pro_codigo_orig][4] = bcadd($this->array_total_pro_codigo[$quebra_pro_codigo_orig][4], $sum_df_total, 10);
      }
      //----- fac_tipo_comprobante
      if (!isset($this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig]))
      {
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][0] = 1;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][1] = $sum_df_subtotal;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][2] = $sum_df_descuento;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][3] = $sum_df_valor_iva;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][4] = $sum_df_total;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][5] = $quebra_fac_tipo_comprobante;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][6] = $quebra_fac_tipo_comprobante_orig;
      }
      else
      {
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][0]++;
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][1] = bcadd($this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][1], $sum_df_subtotal, 10);
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][2] = bcadd($this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][2], $sum_df_descuento, 10);
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][3] = bcadd($this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][3], $sum_df_valor_iva, 10);
         $this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][4] = bcadd($this->array_total_fac_tipo_comprobante[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][4], $sum_df_total, 10);
      }
      //----- fac_serie
      if (!isset($this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig]))
      {
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][0] = 1;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][1] = $sum_df_subtotal;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][2] = $sum_df_descuento;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][3] = $sum_df_valor_iva;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][4] = $sum_df_total;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][5] = $quebra_fac_serie;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][6] = $quebra_fac_serie_orig;
      }
      else
      {
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][0]++;
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][1] = bcadd($this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][1], $sum_df_subtotal, 10);
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][2] = bcadd($this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][2], $sum_df_descuento, 10);
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][3] = bcadd($this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][3], $sum_df_valor_iva, 10);
         $this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][4] = bcadd($this->array_total_fac_serie[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][4], $sum_df_total, 10);
      }
      //----- concepto_factura
      if (!isset($this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig]))
      {
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][0] = 1;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][1] = $sum_df_subtotal;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][2] = $sum_df_descuento;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][3] = $sum_df_valor_iva;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][4] = $sum_df_total;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][5] = $quebra_concepto_factura;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][6] = $quebra_concepto_factura_orig;
      }
      else
      {
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][0]++;
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][1] = bcadd($this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][1], $sum_df_subtotal, 10);
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][2] = bcadd($this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][2], $sum_df_descuento, 10);
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][3] = bcadd($this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][3], $sum_df_valor_iva, 10);
         $this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][4] = bcadd($this->array_total_concepto_factura[$quebra_pro_codigo_orig][$quebra_fac_tipo_comprobante_orig][$quebra_fac_serie_orig][$quebra_concepto_factura_orig][4], $sum_df_total, 10);
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('Inicial', 'pro_codigo');
     $campos_order = $this->Ini->Get_date_order_groupby("pro_codigo", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->pro_codigo = $rs_res->fields[13] ;  
            ...
            $this->pro_codigo_orig = $this->pro_codigo;
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->df_subtotal, $this->df_descuento, $this->df_valor_iva, $this->df_total, $this->pro_codigo, $this->fac_tipo_comprobante, $this->fac_serie, $this->concepto_factura, $this->pro_codigo_orig,  $this->fac_tipo_comprobante_orig,  $this->fac_serie_orig,  $this->concepto_factura_orig);
            }
   }
//
