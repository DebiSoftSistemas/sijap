...
class grid_v_del_resumen_facturacion_diaria_total
{
   ...
   function Calc_resumo_Inicial($destino_resumo, $res_export=false)
   {
      ...
      $cmp_sql_def   = array('pro_codigo' => "pro_codigo",'fac_tipo_comprobante' => "fac_tipo_comprobante",'fac_serie' => "fac_serie",'concepto_factura' => "concepto_factura");
      $cmps_quebra_atual = array("pro_codigo","fac_tipo_comprobante","fac_serie","concepto_factura");
   ...
   function Res_Totaliza_Inicial($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $format_dimensions['pro_codigo']['reg'] = "S";
      $format_dimensions['pro_codigo']['msk'] = "";
   ...
   //-----  pro_codigo
   function quebra_pro_codigo_Inicial($pro_codigo, $arg_sum_pro_codigo) 
   {
      global $tot_pro_codigo, $fac_tipo_comprobante;
      $tot_pro_codigo = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(df_subtotal), sum(df_descuento), sum(df_valor_iva), sum(df_total) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['where_pesq']; 
      } 
      ...
      { 
         $nm_comando .= " where pro_codigo" . $arg_sum_pro_codigo ; 
      } 
      else 
      { 
         $nm_comando .= " and pro_codigo" . $arg_sum_pro_codigo ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_pro_codigo[0] = NM_encode_input(sc_strip_script($pro_codigo));
      }
      else {
          $tot_pro_codigo[0] = sc_strip_script($pro_codigo) ; 
      }
      $tot_pro_codigo[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_pro_codigo[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_pro_codigo[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_pro_codigo[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_pro_codigo[5] = (string)$rt->fields[4]; 
      $rt->Close(); 
   } 

   //-----  fac_tipo_comprobante
   function quebra_fac_tipo_comprobante_Inicial($pro_codigo, $fac_tipo_comprobante, $arg_sum_pro_codigo, $arg_sum_fac_tipo_comprobante) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['where_pesq'])) 
      { 
         $nm_comando .= " where pro_codigo" . $arg_sum_pro_codigo . " and fac_tipo_comprobante" . $arg_sum_fac_tipo_comprobante ; 
      } 
      else 
      { 
         $nm_comando .= " and pro_codigo" . $arg_sum_pro_codigo . " and fac_tipo_comprobante" . $arg_sum_fac_tipo_comprobante ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //-----  fac_serie
   function quebra_fac_serie_Inicial($pro_codigo, $fac_tipo_comprobante, $fac_serie, $arg_sum_pro_codigo, $arg_sum_fac_tipo_comprobante, $arg_sum_fac_serie) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['where_pesq'])) 
      { 
         $nm_comando .= " where pro_codigo" . $arg_sum_pro_codigo . " and fac_tipo_comprobante" . $arg_sum_fac_tipo_comprobante . " and fac_serie" . $arg_sum_fac_serie ; 
      } 
      else 
      { 
         $nm_comando .= " and pro_codigo" . $arg_sum_pro_codigo . " and fac_tipo_comprobante" . $arg_sum_fac_tipo_comprobante . " and fac_serie" . $arg_sum_fac_serie ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //-----  concepto_factura
   function quebra_concepto_factura_Inicial($pro_codigo, $fac_tipo_comprobante, $fac_serie, $concepto_factura, $arg_sum_pro_codigo, $arg_sum_fac_tipo_comprobante, $arg_sum_fac_serie, $arg_sum_concepto_factura) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_resumen_facturacion_diaria']['where_pesq'])) 
      { 
         $nm_comando .= " where pro_codigo" . $arg_sum_pro_codigo . " and fac_tipo_comprobante" . $arg_sum_fac_tipo_comprobante . " and fac_serie" . $arg_sum_fac_serie . " and concepto_factura" . $arg_sum_concepto_factura ; 
      } 
      else 
      { 
         $nm_comando .= " and pro_codigo" . $arg_sum_pro_codigo . " and fac_tipo_comprobante" . $arg_sum_fac_tipo_comprobante . " and fac_serie" . $arg_sum_fac_serie . " and concepto_factura" . $arg_sum_concepto_factura ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
