<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="gc_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['gc_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_grupo_clientes']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_grupo_clientes']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_grupo_clientes"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_Agregar_var_grupo_cliente = "<?php echo $this->form_encode_input($this->nmgp_dados_form['gc_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_form_agregar_cliente_grupo_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_grupo_cliente*scin' + var_btn_Agregar_var_grupo_cliente + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinS*scout"; ?>', '', '_self', '0', '0', 'form_agregar_cliente_grupo');
}
var Crtl_btn_limpiar = false;
...
{
    Crtl_btn_limpiar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?limpiar?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_grupo_clientes']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_grupo_clientes_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_grupo_clientes_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
