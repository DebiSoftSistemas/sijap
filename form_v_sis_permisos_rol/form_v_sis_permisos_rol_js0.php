<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="per_rol_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['per_rol_']); ?>">
<input type="hidden" name="mm_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['mm_id_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_v_sis_permisos_rol"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_v_sis_permisos_rol']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_v_sis_permisos_rol']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_v_sis_permisos_rol']['dashboard_info']['parent_widget'] . "']\")";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_v_sis_permisos_rol_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_mm_modulo_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.mm_modulo_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_v_sis_permisos_rol')?>/form_v_sis_permisos_rol.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_mm_modulo_, "mm_modulo_", dados, false);
}
function recebe_mm_modulo_(string)
{
   document.getElementById('id_lookup_mm_modulo_' + nm_num_campo).innerHTML = string;
}
function lookup_mm_menu_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.mm_menu_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_v_sis_permisos_rol')?>/form_v_sis_permisos_rol.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_mm_menu_, "mm_menu_", dados, false);
}
function recebe_mm_menu_(string)
{
   document.getElementById('id_lookup_mm_menu_' + nm_num_campo).innerHTML = string;
}
<?php 
