<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="dr_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['dr_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_retencion_temporal_ventas']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_retencion_temporal_ventas']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_retencion_temporal_ventas"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_retencion_temporal_ventas']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_retencion_temporal_ventas_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_retencion_temporal_ventas_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function lookup_dr_cliente(campo)
{
   dados  = document.F1.dr_cliente.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_retencion_temporal_ventas')?>/form_del_retencion_temporal_ventas.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_dr_cliente, "dr_cliente", dados, false);
}
function recebe_dr_cliente(string)
{
   document.getElementById('id_lookup_dr_cliente').innerHTML = string;
}
<?php 
...
{
   nm_field_disabled("cod_doc_sustento=disabled;serie_doc_sustento=disabled;secuencial_doc_sustento=disabled;fecha_doc_sustento=disabled;subtotal=disabled;iva=disabled", "");
