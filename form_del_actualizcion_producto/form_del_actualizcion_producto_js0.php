<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="pro_codigo_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pro_codigo_']); ?>">
<input type="hidden" name="pro_empresa_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pro_empresa_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_actualizcion_producto"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_actualizcion_producto']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_actualizcion_producto']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_actualizcion_producto']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
function sc_btn_actualizar_iva()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_actualizar_iva(); }, 500);
      return;
    }
    sc_btn_actualizar_iva_ok();
}
function sc_btn_actualizar_iva_cancel()
{
}
function sc_btn_actualizar_iva_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_control_actualiza_iva_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'control_actualiza_iva');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_actualizcion_producto']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_del_actualizcion_producto_submit_form(y, z);
