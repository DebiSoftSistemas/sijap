<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="dnc_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['dnc_id_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_detalle_nota_credito_compras"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_del_detalle_nota_credito_compras_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_dnc_producto_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.dnc_producto_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_detalle_nota_credito_compras')?>/form_del_detalle_nota_credito_compras.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_dnc_producto_, "dnc_producto_", dados, false);
}
function recebe_dnc_producto_(string)
{
   document.getElementById('id_lookup_dnc_producto_' + nm_num_campo).innerHTML = string;
}
<?php 
