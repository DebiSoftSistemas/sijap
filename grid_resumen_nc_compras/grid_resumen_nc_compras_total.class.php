...
class grid_resumen_nc_compras_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_resumen_nc_compras']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_resumen_nc_compras']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('nc_fecha' => "nc_fecha",'nc_serie' => "nc_serie",'pr_identificacion' => "pr_identificacion",'pr_nombre' => "pr_nombre");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('nc_numero' => 'N','pr_id' => 'N','nc_subtotal' => 'N','nc_total_descuento' => 'N','nc_subtotal_iva' => 'N','nc_valor_iva' => 'N','nc_subtotal_cero' => 'N','nc_subtotal_no_objeto' => 'N','nc_subtotal_excento' => 'N','nc_valor_ice' => 'N','nc_valor_irbpnr' => 'N','nc_total' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nc_subtotal), sum(nc_total_descuento), sum(nc_valor_iva), sum(nc_total), sum(nc_subtotal_iva), sum(nc_subtotal_cero), sum(nc_subtotal_no_objeto), sum(nc_subtotal_excento), sum(nc_valor_ice), sum(nc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(nc_subtotal), sum(nc_total_descuento), sum(nc_valor_iva), sum(nc_total), sum(nc_subtotal_iva), sum(nc_subtotal_cero), sum(nc_subtotal_no_objeto), sum(nc_subtotal_excento), sum(nc_valor_ice), sum(nc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][7] = $rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $rt->fields[7] = (string)$rt->fields[7]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][8] = $rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $rt->fields[8] = (string)$rt->fields[8]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][9] = $rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $rt->fields[9] = (string)$rt->fields[9]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][10] = $rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $rt->fields[10] = (string)$rt->fields[10]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['tot_geral'][11] = $rt->fields[10]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nc_compras']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_nc_serie_sc_free_group_by($nc_serie, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_nc_serie[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_nc_serie[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_nc_serie[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
