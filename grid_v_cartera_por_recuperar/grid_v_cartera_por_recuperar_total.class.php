...
class grid_v_cartera_por_recuperar_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_v_cartera_por_recuperar']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_v_cartera_por_recuperar']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('op_fecha_emision' => "op_fecha_emision",'periodo_emision' => "periodo_emision",'mes_emision' => "mes_emision",'cl_identificacion' => "cl_identificacion",'cl_nombre' => "cl_nombre",'pro_descripcion' => "pro_descripcion",'con_ruta' => "con_ruta",'op_estado' => "op_estado",'con_id' => "con_id",'op_asiento' => "op_asiento");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('op_operacion' => 'N','periodo_emision' => 'N','periodo_consumo' => 'N','do_subtotal' => 'N','do_descuento' => 'N','do_valor_iva' => 'N','do_total' => 'N','op_propietario' => 'N','con_id' => 'N','con_ruta' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2), sum(SC_metric3)#@#cmps_quebras#@# from (";
         $comando .= "select do_total as SC_metric1,do_subtotal as SC_metric2,do_valor_iva as SC_metric3, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['periodo_emision']['reg'] = "S";
      $format_dimensions['periodo_emision']['msk'] = "";
      $format_dimensions['mes_emision']['reg'] = "S";
      $format_dimensions['mes_emision']['msk'] = "";
      ...
      $format_dimensions['con_ruta']['reg'] = "S";
      $format_dimensions['con_ruta']['msk'] = "";
      $format_dimensions['op_estado']['reg'] = "S";
      $format_dimensions['op_estado']['msk'] = "";
      $format_dimensions['con_id']['reg'] = "S";
      $format_dimensions['con_id']['msk'] = "";
      $format_dimensions['op_asiento']['reg'] = "S";
      $format_dimensions['op_asiento']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "periodo_emision") {
          nmgp_Form_Num_Val($$Cada_dim, "", ",", "0", "S", "2", "", "N:", "-") ; 
              }
              if ($Cada_dim == "mes_emision") {
                  $this->Lookup->lookup_sc_free_group_by_mes_emision($$Cada_dim,  $mes_emision);
              }
              ...
              if ($Cada_dim == "con_ruta") {
                  $this->Lookup->lookup_sc_free_group_by_con_ruta($$Cada_dim,  $con_ruta);
              }
              if ($Cada_dim == "op_estado") {
                  $this->Lookup->lookup_sc_free_group_by_op_estado($$Cada_dim,  $op_estado);
              }
              if ($Cada_dim == "con_id") {
                  $this->Lookup->lookup_sc_free_group_by_con_id($$Cada_dim,  $con_id);
              }
              if ($Cada_dim == "op_asiento") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   //----- 
   function Calc_resumo_Mensual($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('periodo_consumo' => "periodo_consumo",'mes_consumo' => "mes_consumo");
      $cmps_quebra_atual = array("periodo_consumo","mes_consumo");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_Mensual($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_Mensual($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      $format_dimensions = array();
      $format_dimensions['periodo_consumo']['reg'] = "S";
      $format_dimensions['periodo_consumo']['msk'] = "";
      $format_dimensions['mes_consumo']['reg'] = "S";
      $format_dimensions['mes_consumo']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'Mensual', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "periodo_consumo") {
          nmgp_Form_Num_Val($$Cada_dim, "", ",", "0", "S", "2", "", "N:", "-") ; 
              }
              if ($Cada_dim == "mes_consumo") {
                  $this->Lookup->lookup_Mensual_mes_consumo($$Cada_dim,  $mes_consumo);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_Mensual($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
   ...
   {
      global $nada, $nm_lang , $con_id, $con_ruta, $op_estado, $mes_consumo;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_Mensual($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang , $con_id, $con_ruta, $op_estado, $mes_consumo;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['contr_total_geral'] == "OK") 
      { 
   ...
   function quebra_op_fecha_emision_sc_free_group_by($op_fecha_emision, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $con_id, $con_ruta, $op_estado, $mes_consumo;
   ...
   //-----  periodo_emision
   function quebra_periodo_emision_sc_free_group_by($periodo_emision, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_periodo_emision = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      } 
      ...
      $tot_periodo_emision[0] = NM_encode_input(sc_strip_script($periodo_emision)) ; 
      $tot_periodo_emision[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_periodo_emision[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_periodo_emision[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_periodo_emision[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_periodo_emision;
   } 

   //-----  mes_emision
   function quebra_mes_emision_sc_free_group_by($mes_emision, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_mes_emision = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_mes_emision[0] = NM_encode_input(sc_strip_script($mes_emision));
      }
      else {
          $tot_mes_emision[0] = sc_strip_script($mes_emision) ; 
      }
      $tot_mes_emision[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_mes_emision[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_mes_emision[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_mes_emision[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_mes_emision;
   } 
   ...
   //-----  con_ruta
   function quebra_con_ruta_sc_free_group_by($con_ruta, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_con_ruta = array() ;  
      ...
      $tot_con_ruta[0] = NM_encode_input(sc_strip_script($con_ruta)) ; 
      $tot_con_ruta[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_con_ruta[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_con_ruta[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_con_ruta[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_con_ruta;
   } 
   ...
   //-----  con_id
   function quebra_con_id_sc_free_group_by($con_id, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_con_id = array() ;  
      ...
      $tot_con_id[0] = NM_encode_input(sc_strip_script($con_id)) ; 
      $tot_con_id[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_con_id[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_con_id[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_con_id[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_con_id;
   } 

   //-----  op_asiento
   function quebra_op_asiento_sc_free_group_by($op_asiento, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_op_asiento = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_op_asiento[0] = NM_encode_input(sc_strip_script($op_asiento));
      }
      else {
          $tot_op_asiento[0] = sc_strip_script($op_asiento) ; 
      }
      $tot_op_asiento[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_op_asiento[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_op_asiento[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_op_asiento[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_op_asiento;
   } 

   //-----  periodo_consumo
   function quebra_periodo_consumo_Mensual($periodo_consumo, $arg_sum_periodo_consumo) 
   {
      global $tot_periodo_consumo, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_periodo_consumo = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      } 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq'])) 
      { 
         $nm_comando .= " where periodo_consumo" . $arg_sum_periodo_consumo ; 
      } 
      else 
      { 
         $nm_comando .= " and periodo_consumo" . $arg_sum_periodo_consumo ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_periodo_consumo[0] = NM_encode_input(sc_strip_script($periodo_consumo)) ; 
      $tot_periodo_consumo[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_periodo_consumo[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_periodo_consumo[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_periodo_consumo[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
   } 

   //-----  mes_consumo
   function quebra_mes_consumo_Mensual($periodo_consumo, $mes_consumo, $arg_sum_periodo_consumo, $arg_sum_mes_consumo) 
   {
      global $tot_mes_consumo, $con_id, $con_ruta, $op_estado, $mes_consumo;
      $tot_mes_consumo = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq']; 
      } 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['where_pesq'])) 
      { 
         $nm_comando .= " where periodo_consumo" . $arg_sum_periodo_consumo . " and mes_consumo" . $arg_sum_mes_consumo ; 
      } 
      else 
      { 
         $nm_comando .= " and periodo_consumo" . $arg_sum_periodo_consumo . " and mes_consumo" . $arg_sum_mes_consumo ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_por_recuperar']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_mes_consumo[0] = NM_encode_input(sc_strip_script($mes_consumo));
      }
      else {
          $tot_mes_consumo[0] = sc_strip_script($mes_consumo) ; 
      }
      $tot_mes_consumo[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_mes_consumo[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_mes_consumo[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_mes_consumo[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
   } 
