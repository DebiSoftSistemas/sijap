<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="pai_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pai_id_']); ?>">
<input type="hidden" name="pai_nombre_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pai_nombre_']); ?>">
...
{
    window.open('<?php echo $this->Ini->path_link . "" . SC_dir_app_name('form_sis_pais') . "/form_sis_pais_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . $this->form_encode_input($this->Ini->sc_page);?>"&apl_prt=form_sis_pais.php&opcao=print&cor_print=AM','','location=no,menubar,resizable,scrollbars,status=no,toolbar');
}
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_sis_pais_submit_form(y, z);
