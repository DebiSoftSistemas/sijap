...
class grid_v_del_inventario_total
{
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      ...
      $cmp_sql_def   = array('bo_establecimiento' => "bo_establecimiento",'bodega' => "bodega",'pro_descripcion' => "pro_descripcion");
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('pro_cantidad_inventario' => 'N','cantidad' => 'N','costo_promedio' => 'N','total' => 'N','kar_bodega' => 'N','bo_establecimiento' => 'N','kar_unidad' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(pro_cantidad_inventario), sum(total)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select pro_cantidad_inventario as SC_metric1,total as SC_metric2, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      $format_dimensions['bo_establecimiento']['reg'] = "S";
      $format_dimensions['bo_establecimiento']['msk'] = "";
      $format_dimensions['bodega']['reg'] = "S";
      $format_dimensions['bodega']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "bo_establecimiento") {
                  $this->Lookup->lookup_sc_free_group_by_bo_establecimiento($$Cada_dim,  $bo_establecimiento);
              }
              if ($Cada_dim == "bodega") {
              }
   ...
   function Calc_resumo_kar_bodega($destino_resumo, $res_export=false)
   {
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('bo_establecimiento' => "bo_establecimiento",'bodega' => "bodega");
      $cmps_quebra_atual = array("bo_establecimiento","bodega");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
   ...
   function Res_Totaliza_kar_bodega($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "bo_establecimiento") {
                  $this->Lookup->lookup_kar_bodega_bo_establecimiento($$Cada_dim,  $bo_establecimiento);
              }
              if ($Cada_dim == "bodega") {
              }
              if (null === $$Cada_dim)
   ...
   {
      global $nada, $nm_lang , $kar_unidad, $bo_establecimiento;
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(pro_cantidad_inventario), sum(total) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(pro_cantidad_inventario), sum(total) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq']; 
   ...
   //-----  bo_establecimiento
   function quebra_bo_establecimiento_sc_free_group_by($bo_establecimiento, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $kar_unidad, $bo_establecimiento;
      $tot_bo_establecimiento = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(pro_cantidad_inventario), sum(total) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq']; 
      } 
      ...
      $tot_bo_establecimiento[0] = NM_encode_input(sc_strip_script($bo_establecimiento)) ; 
      $tot_bo_establecimiento[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_bo_establecimiento[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_bo_establecimiento[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_bo_establecimiento;
   } 

   //-----  bodega
   function quebra_bodega_sc_free_group_by($bodega, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $kar_unidad, $bo_establecimiento;
      $tot_bodega = array() ;  
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_bodega[0] = NM_encode_input(sc_strip_script($bodega));
      }
      else {
          $tot_bodega[0] = sc_strip_script($bodega) ; 
      }
      $tot_bodega[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_bodega[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_bodega[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_bodega;
   } 
   ...
   //-----  bo_establecimiento
   function quebra_bo_establecimiento_kar_bodega($bo_establecimiento, $arg_sum_bo_establecimiento) 
   {
      global $tot_bo_establecimiento, $kar_unidad, $bo_establecimiento;
      $tot_bo_establecimiento = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq'])) 
      { 
         $nm_comando .= " where bo_establecimiento" . $arg_sum_bo_establecimiento ; 
      } 
      else 
      { 
         $nm_comando .= " and bo_establecimiento" . $arg_sum_bo_establecimiento ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //-----  bodega
   function quebra_bodega_kar_bodega($bo_establecimiento, $bodega, $arg_sum_bo_establecimiento, $arg_sum_bodega) 
   {
      global $tot_bodega, $kar_unidad, $bo_establecimiento;
      $tot_bodega = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_inventario']['where_pesq'])) 
      { 
         $nm_comando .= " where bo_establecimiento" . $arg_sum_bo_establecimiento . " and bodega" . $arg_sum_bodega ; 
      } 
      else 
      { 
         $nm_comando .= " and bo_establecimiento" . $arg_sum_bo_establecimiento . " and bodega" . $arg_sum_bodega ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
