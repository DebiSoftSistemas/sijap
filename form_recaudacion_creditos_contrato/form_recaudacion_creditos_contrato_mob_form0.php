...
<script type="text/javascript" src="<?php echo $this->Ini->path_prod ?>/third/jquery_plugin/select2/js/select2.full.min.js"></script>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>form_recaudacion_creditos_contrato/form_recaudacion_creditos_contrato_mob_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
var Nav_binicio_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['first']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['first'] : 'off'); ?>";
var Nav_bavanca_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['forward']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['forward'] : 'off'); ?>";
var Nav_bretorna_macro_disabled = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['back']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['back'] : 'off'); ?>";
var Nav_bfinal_macro_disabled   = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['last']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['last'] : 'off'); ?>";
...
function nav_desliga_img()
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "total_pago")
     {
        $('input[name="total_pago"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="total_pago"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="total_pago"]').removeClass("scFormInputDisabled");
        }
     }
     if (F_name == "cambio")
     {
        $('input[name="cambio"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="cambio"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="cambio"]').removeClass("scFormInputDisabled");
        }
     }
  }
 } // nm_field_disabled
...
include_once('form_recaudacion_creditos_contrato_mob_jquery.php');
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['recarga'];
}
if ('' != $this->contrato)
{
    $this->lookup_contrato($look_rpc_contrato);
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['link_info']['remove_margin']) {
        $remove_margin = 'margin: 0; ';
    }
    if ('' != $remove_margin && isset($str_iframe_body) && '' != $str_iframe_body) {
        $str_iframe_body = '';
    }
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['link_info']['remove_border']) {
...
if (isset($_SESSION['scriptcase']['form_recaudacion_creditos_contrato']['error_buffer']) && '' != $_SESSION['scriptcase']['form_recaudacion_creditos_contrato']['error_buffer'])
{
    echo $_SESSION['scriptcase']['form_recaudacion_creditos_contrato']['error_buffer'];
}
...
<?php
 include_once("form_recaudacion_creditos_contrato_mob_js0.php");
?>
<script type="text/javascript" src="<?php  echo $this->Ini->path_js . "/jsrsClient.js" ?>"> 
</script> 
<script type="text/javascript"> 
...
<form  name="F1" method="post" 
               action="form_recaudacion_creditos_contrato_mob.php" 
               target="_self">
<input type="hidden" name="nmgp_url_saida" value="">
<?php
if ('novo' == $this->nmgp_opcao || 'incluir' == $this->nmgp_opcao)
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['insert_validation'] = md5(time() . rand(1, 99999));
?>
<input type="hidden" name="nmgp_ins_valid" value="<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['insert_validation']; ?>">
<?php
}
?>
...
<?php
$_SESSION['scriptcase']['error_span_title']['form_recaudacion_creditos_contrato_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['form_recaudacion_creditos_contrato_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['help']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['help']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['help']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['help']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['help'];
        }
?>
...
<?php
    ...
    if ((!$this->Embutida_call || $this->form_3versions_single) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && $nm_apl_dependente != 1 && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] != "R" && !$this->aba_iframe && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
        $buttonMacroDisabled = 'sc-unique-btn-2';
        $buttonMacroLabel = "";

        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['exit']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['exit']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['exit']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['exit']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['exit'];
        }
?>
...
<?php
    ...
    if ((!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (!isset($_SESSION['scriptcase']['nm_sc_retorno']) || empty($_SESSION['scriptcase']['nm_sc_retorno']) || $nm_apl_dependente == 1 || $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] == "R" || $this->aba_iframe || $this->nmgp_botoes['exit'] != "on") && ($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['run_iframe'] != "F" && $this->nmgp_botoes['exit'] == "on") && ($nm_apl_dependente == 1 && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['actualizar_cliente']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['actualizar_cliente']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['actualizar_cliente']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['actualizar_cliente']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['actualizar_cliente'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['facturar']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_disabled']['facturar']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['facturar']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['facturar']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['btn_label']['facturar'];
        }
?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_contrato_line" id="hidden_field_data_contrato" style="<?php echo $sStyleHidden_contrato; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_contrato_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_contrato_label" style=""><span id="id_label_contrato"><?php echo $this->nm_new_label['contrato']; ?></span></span><br><input type="hidden" name="contrato" value="<?php echo $this->form_encode_input($contrato); ?>"><span id="id_ajax_label_contrato"><?php echo nl2br($contrato); ?></span>
 <SPAN id="id_lookup_contrato"><?php echo $look_rpc_contrato; ?></SPAN></td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_contrato_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_contrato_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['Lookup_usar_compensacion'][] = 'S'; ?>
<?php  if (in_array("S", $this->usar_compensacion_1))  { echo " checked" ;} ?> onClick="do_ajax_form_recaudacion_creditos_contrato_mob_event_usar_compensacion_onclick();" ><span></span>
...
<span id="id_read_on_valor_pago" class="sc-ui-readonly-valor_pago css_valor_pago_line" style="<?php echo $sStyleReadLab_valor_pago; ?>"><?php echo $this->form_format_readonly("valor_pago", $this->form_encode_input($this->valor_pago)); ?></span><span id="id_read_off_valor_pago" class="css_read_off_valor_pago<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_valor_pago; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> alt="{datatype: 'decimal', maxLength: 20, precision: 2, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['valor_pago']['symbol_dec']); ?>', thousandsSep: '', thousandsFormat: <?php echo $this->field_config['valor_pago']['symbol_fmt']; ?>, manualDecimals: true, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['valor_pago']['format_neg'] ? "'suffix'" : "'prefix'") ?>, enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_total_pago_line" id="hidden_field_data_total_pago" style="<?php echo $sStyleHidden_total_pago; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_total_pago_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_total_pago_label" style=""><span id="id_label_total_pago"><?php echo $this->nm_new_label['total_pago']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["total_pago"]) &&  $this->nmgp_cmp_readonly["total_pago"] == "on") { 

 ?>
<input type="hidden" name="total_pago" value="<?php echo $this->form_encode_input($total_pago) . "\">" . $total_pago . ""; ?>
<?php } else { ?>
<span id="id_read_on_total_pago" class="sc-ui-readonly-total_pago css_total_pago_line" style="<?php echo $sStyleReadLab_total_pago; ?>"><?php echo $this->form_format_readonly("total_pago", $this->form_encode_input($this->total_pago)); ?></span><span id="id_read_off_total_pago" class="css_read_off_total_pago<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_total_pago; ?>">
 <input class="sc-js-input scFormObjectOdd css_total_pago_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_total_pago" type=text name="total_pago" value="<?php echo $this->form_encode_input($total_pago) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> alt="{datatype: 'decimal', maxLength: 20, precision: 2, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['total_pago']['symbol_dec']); ?>', thousandsSep: '', thousandsFormat: <?php echo $this->field_config['total_pago']['symbol_fmt']; ?>, manualDecimals: false, allowNegative: true, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['total_pago']['format_neg'] ? "'suffix'" : "'prefix'") ?>, enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ ', alignment: 'left'}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_total_pago_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_total_pago_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_cambio_line" id="hidden_field_data_cambio" style="<?php echo $sStyleHidden_cambio; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOdd css_cambio_line" style="vertical-align: top;padding: 0px"><span class="scFormLabelOddFormat css_cambio_label" style=""><span id="id_label_cambio"><?php echo $this->nm_new_label['cambio']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["cambio"]) &&  $this->nmgp_cmp_readonly["cambio"] == "on") { 

 ?>
<input type="hidden" name="cambio" value="<?php echo $this->form_encode_input($cambio) . "\">" . $cambio . ""; ?>
<?php } else { ?>
<span id="id_read_on_cambio" class="sc-ui-readonly-cambio css_cambio_line" style="<?php echo $sStyleReadLab_cambio; ?>"><?php echo $this->form_format_readonly("cambio", $this->form_encode_input($this->cambio)); ?></span><span id="id_read_off_cambio" class="css_read_off_cambio<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_cambio; ?>">
 <input class="sc-js-input scFormObjectOdd css_cambio_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_cambio" type=text name="cambio" value="<?php echo $this->form_encode_input($cambio) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> alt="{datatype: 'decimal', maxLength: 20, precision: 2, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['cambio']['symbol_dec']); ?>', thousandsSep: '', thousandsFormat: <?php echo $this->field_config['cambio']['symbol_fmt']; ?>, manualDecimals: false, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['cambio']['format_neg'] ? "'suffix'" : "'prefix'") ?>, enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_cambio_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_cambio_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php $sStyleHidden_cambio_dumb = ('' == $sStyleHidden_cambio) ? 'display: none' : ''; ?>
    <TD class="scFormDataOdd" id="hidden_field_data_cambio_dumb" style="<?php echo $sStyleHidden_cambio_dumb; ?>"></TD>
   </tr>
<?php $sc_hidden_no = 1; ?>
</TABLE></div><!-- bloco_f -->
   </td>
...
<TABLE align="center" id="hidden_bloco_4" class="scFormTable<?php echo $this->classes_100perc_fields['table'] ?>" width="100%" style="height: 100%;"><?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    ...
    {
        $this->nm_new_label['facturas'] = "facturas";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
...
<?php
 if (isset($_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_facturas'] ]) && '' != $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_facturas'] ]) {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['grid_v_del_ventas_credito_contrato_script_case_init'] = $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_facturas'] ];
 }
 else {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['grid_v_del_ventas_credito_contrato_script_case_init'] = $this->Ini->sc_page;
 }
 ...
 if (isset($this->Ini->sc_lig_md5["grid_v_del_ventas_credito_contrato"]) && $this->Ini->sc_lig_md5["grid_v_del_ventas_credito_contrato"] == "S") {
     $Parms_Lig  = "var_propietario*scin" . $this->nmgp_dados_form['cl_id'] . "*scoutvar_contrato*scin" . $this->nmgp_dados_form['contrato'] . "*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scoutNMSC_cab*scinN*scoutNMSC_nav*scinN*scout";
     $Md5_Lig    = "@SC_par@" . $this->form_encode_input($this->Ini->sc_page) . "@SC_par@form_recaudacion_creditos_contrato_mob@SC_par@" . md5($Parms_Lig);
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['Lig_Md5'][md5($Parms_Lig)] = $Parms_Lig;
 } else {
     $Md5_Lig  = "var_propietario*scin" . $this->nmgp_dados_form['cl_id'] . "*scoutvar_contrato*scin" . $this->nmgp_dados_form['contrato'] . "*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scoutNMSC_cab*scinN*scoutNMSC_nav*scinN*scout";
 }
 $parms_lig_cons = "&nmgp_parms=" . $Md5_Lig;
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_recaudacion_creditos_contrato_mob_empty.htm' : $this->Ini->link_grid_v_del_ventas_credito_contrato_cons . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page) . '&script_case_detail=Y' . $parms_lig_cons;
...
if (isset($this->Ini->sc_lig_target['C_@scinf_facturas']) && 'nmsc_iframe_liga_grid_v_del_ventas_credito_contrato' != $this->Ini->sc_lig_target['C_@scinf_facturas'])
{
    if ('novo' != $this->nmgp_opcao)
    {
        $sDetailSrc .= '&under_dashboard=1&dashboard_app=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['dashboard_app'] . '&own_widget=' . $this->Ini->sc_lig_target['C_@scinf_facturas'] . '&parent_widget=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['own_widget'];
        $sDetailSrc  = $this->addUrlParam($sDetailSrc, 'script_case_init', $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['grid_v_del_ventas_credito_contrato_script_case_init']);
    }
?>
<script type="text/javascript">
$(function() {
...
{
?>
<iframe border="0" id="nmsc_iframe_liga_grid_v_del_ventas_credito_contrato"  marginWidth="0" marginHeight="0" frameborder="0" valign="top" height="100" width="100%" name="nmsc_iframe_liga_grid_v_del_ventas_credito_contrato"  scrolling="auto" src="<?php echo $sDetailSrc; ?>"></iframe>
<?php
}
?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("form_recaudacion_creditos_contrato_mob");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("form_recaudacion_creditos_contrato_mob");
 parent.scAjaxDetailHeight("form_recaudacion_creditos_contrato_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
<script type="text/javascript">
	function scBtnFn_btn_salir() {
		if ($("#sc_btn_salir_top").length && $("#sc_btn_salir_top").is(":visible")) {
		    if ($("#sc_btn_salir_top").hasClass("disabled")) {
		        return;
		    }
			sc_btn_btn_salir()
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
