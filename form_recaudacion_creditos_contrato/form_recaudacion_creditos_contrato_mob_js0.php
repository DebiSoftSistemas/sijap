...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_recaudacion_creditos_contrato_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_facturar_ok()
{
    ...
    document.F1.action = "form_recaudacion_creditos_contrato_mob.php";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_selecciona_contrato_credito_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_propietario*scin' + var_btn_btn_salir_var_propietario + '*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scout"; ?>', 'inicio', '_self', '0', '0', 'grid_selecciona_contrato_credito');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_recaudacion_creditos_contrato_mob']['where_detal']))
  {
?>  
...
function lookup_contrato(campo)
{
   ...
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_recaudacion_creditos_contrato_mob')?>/form_recaudacion_creditos_contrato_mob.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_contrato, "contrato", dados, false);
}
...
{
   nm_field_disabled("tesoreria=disabled;total_pago=disabled", "");
   nm_field_disabled("cl_identificacion=disabled;cl_nombre=disabled;cl_direccion=disabled;cl_telefono=disabled;cl_email=disabled;total=disabled;cambio=disabled", "U");
