...
class grid_resumen_ventas_anuladas_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_resumen_ventas_anuladas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_resumen_ventas_anuladas']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fac_fecha' => "fac_fecha",'fac_tipo_comprobante' => "fac_tipo_comprobante",'est_codigo' => "est_codigo",'pen_serie' => "pen_serie",'cl_identificacion' => "cl_identificacion",'cl_nombre' => "cl_nombre",'fac_usuario' => "fac_usuario",'fac_socio_transportista' => "fac_socio_transportista");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('fac_numero' => 'N','fac_ambiente' => 'N','fac_subtotal' => 'N','fac_total_descuento' => 'N','fac_subtotal_iva' => 'N','fac_valor_iva' => 'N','fac_subtotal_cero' => 'N','fac_subtotal_no_objeto' => 'N','fac_subtotal_excento' => 'N','fac_valor_ice' => 'N','fac_valor_irbpnr' => 'N','fac_propina' => 'N','fac_total' => 'N','cl_id' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(fac_subtotal), sum(fac_total_descuento), sum(fac_valor_iva), sum(fac_propina), sum(fac_total), sum(fac_subtotal_iva), sum(fac_subtotal_cero), sum(fac_subtotal_no_objeto), sum(fac_subtotal_excento), sum(fac_valor_ice), sum(fac_valor_irbpnr)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2), sum(SC_metric3), sum(SC_metric4), sum(SC_metric5), sum(SC_metric6), sum(SC_metric7), sum(SC_metric8), sum(SC_metric9), sum(SC_metric10), sum(SC_metric11)#@#cmps_quebras#@# from (";
         $comando .= "select fac_subtotal as SC_metric1,fac_total_descuento as SC_metric2,fac_valor_iva as SC_metric3,fac_propina as SC_metric4,fac_total as SC_metric5,fac_subtotal_iva as SC_metric6,fac_subtotal_cero as SC_metric7,fac_subtotal_no_objeto as SC_metric8,fac_subtotal_excento as SC_metric9,fac_valor_ice as SC_metric10,fac_valor_irbpnr as SC_metric11, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['fac_socio_transportista']['reg'] = "S";
      $format_dimensions['fac_socio_transportista']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "fac_socio_transportista") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang , $fac_tipo_comprobante, $fac_usuario, $fac_socio_transportista;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fac_subtotal), sum(fac_total_descuento), sum(fac_valor_iva), sum(fac_propina), sum(fac_total), sum(fac_subtotal_iva), sum(fac_subtotal_cero), sum(fac_subtotal_no_objeto), sum(fac_subtotal_excento), sum(fac_valor_ice), sum(fac_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(fac_subtotal), sum(fac_total_descuento), sum(fac_valor_iva), sum(fac_propina), sum(fac_total), sum(fac_subtotal_iva), sum(fac_subtotal_cero), sum(fac_subtotal_no_objeto), sum(fac_subtotal_excento), sum(fac_valor_ice), sum(fac_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][7] = $rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $rt->fields[7] = (string)$rt->fields[7]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][8] = $rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $rt->fields[8] = (string)$rt->fields[8]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][9] = $rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $rt->fields[9] = (string)$rt->fields[9]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][10] = $rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $rt->fields[10] = (string)$rt->fields[10]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][11] = $rt->fields[10]; 
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $rt->fields[11] = (string)$rt->fields[11]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['tot_geral'][12] = $rt->fields[11]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_fac_fecha_sc_free_group_by($fac_fecha, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $fac_tipo_comprobante, $fac_usuario, $fac_socio_transportista;
      $tot_fac_fecha = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fac_subtotal), sum(fac_total_descuento), sum(fac_valor_iva), sum(fac_propina), sum(fac_total), sum(fac_subtotal_iva), sum(fac_subtotal_cero), sum(fac_subtotal_no_objeto), sum(fac_subtotal_excento), sum(fac_valor_ice), sum(fac_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['where_pesq']; 
      ...
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fac_fecha[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fac_fecha[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fac_fecha[11] = (string)$rt->fields[10]; 
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_fac_fecha[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
   ...
   function quebra_fac_tipo_comprobante_sc_free_group_by($fac_tipo_comprobante, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fac_tipo_comprobante[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fac_tipo_comprobante[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fac_tipo_comprobante[11] = (string)$rt->fields[10]; 
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_fac_tipo_comprobante[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
   ...
   function quebra_est_codigo_sc_free_group_by($est_codigo, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_est_codigo[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
   ...
   function quebra_pen_serie_sc_free_group_by($pen_serie, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_pen_serie[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
   ...
   function quebra_cl_identificacion_sc_free_group_by($cl_identificacion, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_cl_identificacion[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
   ...
   function quebra_cl_nombre_sc_free_group_by($cl_nombre, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_cl_nombre[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
   ...
   function quebra_fac_usuario_sc_free_group_by($fac_usuario, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fac_usuario[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fac_usuario[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fac_usuario[11] = (string)$rt->fields[10]; 
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_fac_usuario[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fac_usuario;
   } 

   //-----  fac_socio_transportista
   function quebra_fac_socio_transportista_sc_free_group_by($fac_socio_transportista, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $fac_tipo_comprobante, $fac_usuario, $fac_socio_transportista;
      $tot_fac_socio_transportista = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fac_subtotal), sum(fac_total_descuento), sum(fac_valor_iva), sum(fac_propina), sum(fac_total), sum(fac_subtotal_iva), sum(fac_subtotal_cero), sum(fac_subtotal_no_objeto), sum(fac_subtotal_excento), sum(fac_valor_ice), sum(fac_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_ventas_anuladas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_fac_socio_transportista[0] = NM_encode_input(sc_strip_script($fac_socio_transportista));
      }
      else {
          $tot_fac_socio_transportista[0] = sc_strip_script($fac_socio_transportista) ; 
      }
      $tot_fac_socio_transportista[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fac_socio_transportista[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fac_socio_transportista[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_fac_socio_transportista[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_fac_socio_transportista[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_fac_socio_transportista[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_fac_socio_transportista[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_fac_socio_transportista[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fac_socio_transportista[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fac_socio_transportista[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fac_socio_transportista[11] = (string)$rt->fields[10]; 
      $rt->fields[11] = str_replace(",", ".", $rt->fields[11]);
      $tot_fac_socio_transportista[12] = (string)$rt->fields[11]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fac_socio_transportista;
   } 
