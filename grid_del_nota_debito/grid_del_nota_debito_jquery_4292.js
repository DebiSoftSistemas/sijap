function grid_del_nota_debito_nd_fecha_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nd_fecha" + seqRow).html();
}

function grid_del_nota_debito_nd_fecha_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nd_fecha" + seqRow).html(newValue);
}

function grid_del_nota_debito_sc_field_0_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_sc_field_0" + seqRow).html();
}

function grid_del_nota_debito_sc_field_0_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_sc_field_0" + seqRow).html(newValue);
}

function grid_del_nota_debito_sc_field_1_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_sc_field_1" + seqRow).html();
}

function grid_del_nota_debito_sc_field_1_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_sc_field_1" + seqRow).html(newValue);
}

function grid_del_nota_debito_cl_nombre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_nombre" + seqRow).html();
}

function grid_del_nota_debito_cl_nombre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_nombre" + seqRow).html(newValue);
}

function grid_del_nota_debito_cl_identificacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_identificacion" + seqRow).html();
}

function grid_del_nota_debito_cl_identificacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_identificacion" + seqRow).html(newValue);
}

function grid_del_nota_debito_nd_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nd_total" + seqRow).html();
}

function grid_del_nota_debito_nd_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nd_total" + seqRow).html(newValue);
}

function grid_del_nota_debito_nd_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nd_estado_sri" + seqRow).html();
}

function grid_del_nota_debito_nd_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nd_estado_sri" + seqRow).html(newValue);
}

function grid_del_nota_debito_cl_direccion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cl_direccion" + seqRow).html();
}

function grid_del_nota_debito_cl_direccion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cl_direccion" + seqRow).html(newValue);
}

function grid_del_nota_debito_nd_autorizacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nd_autorizacion" + seqRow).html();
}

function grid_del_nota_debito_nd_autorizacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nd_autorizacion" + seqRow).html(newValue);
}

function grid_del_nota_debito_nd_error_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nd_error_sri" + seqRow).html();
}

function grid_del_nota_debito_nd_error_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nd_error_sri" + seqRow).html(newValue);
}

function grid_del_nota_debito_nd_numero_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nd_numero" + seqRow).html();
}

function grid_del_nota_debito_nd_numero_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nd_numero" + seqRow).html(newValue);
}

function grid_del_nota_debito_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("nd_fecha" == field) {
    SC_tmp = grid_del_nota_debito_nd_fecha_getValue(seqRow);
  }
  if ("sc_field_0" == field) {
    SC_tmp = grid_del_nota_debito_sc_field_0_getValue(seqRow);
  }
  if ("sc_field_1" == field) {
    SC_tmp = grid_del_nota_debito_sc_field_1_getValue(seqRow);
  }
  if ("cl_nombre" == field) {
    SC_tmp = grid_del_nota_debito_cl_nombre_getValue(seqRow);
  }
  if ("cl_identificacion" == field) {
    SC_tmp = grid_del_nota_debito_cl_identificacion_getValue(seqRow);
  }
  if ("nd_total" == field) {
    SC_tmp = grid_del_nota_debito_nd_total_getValue(seqRow);
  }
  if ("nd_estado_sri" == field) {
    SC_tmp = grid_del_nota_debito_nd_estado_sri_getValue(seqRow);
  }
  if ("cl_direccion" == field) {
    SC_tmp = grid_del_nota_debito_cl_direccion_getValue(seqRow);
  }
  if ("nd_autorizacion" == field) {
    SC_tmp = grid_del_nota_debito_nd_autorizacion_getValue(seqRow);
  }
  if ("nd_error_sri" == field) {
    SC_tmp = grid_del_nota_debito_nd_error_sri_getValue(seqRow);
  }
  if ("nd_numero" == field) {
    SC_tmp = grid_del_nota_debito_nd_numero_getValue(seqRow);
  }
...
function grid_del_nota_debito_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("nd_fecha" == field) {
    grid_del_nota_debito_nd_fecha_setValue(newValue, seqRow);
  }
  if ("sc_field_0" == field) {
    grid_del_nota_debito_sc_field_0_setValue(newValue, seqRow);
  }
  if ("sc_field_1" == field) {
    grid_del_nota_debito_sc_field_1_setValue(newValue, seqRow);
  }
  if ("cl_nombre" == field) {
    grid_del_nota_debito_cl_nombre_setValue(newValue, seqRow);
  }
  if ("cl_identificacion" == field) {
    grid_del_nota_debito_cl_identificacion_setValue(newValue, seqRow);
  }
  if ("nd_total" == field) {
    grid_del_nota_debito_nd_total_setValue(newValue, seqRow);
  }
  if ("nd_estado_sri" == field) {
    grid_del_nota_debito_nd_estado_sri_setValue(newValue, seqRow);
  }
  if ("cl_direccion" == field) {
    grid_del_nota_debito_cl_direccion_setValue(newValue, seqRow);
  }
  if ("nd_autorizacion" == field) {
    grid_del_nota_debito_nd_autorizacion_setValue(newValue, seqRow);
  }
  if ("nd_error_sri" == field) {
    grid_del_nota_debito_nd_error_sri_setValue(newValue, seqRow);
  }
  if ("nd_numero" == field) {
    grid_del_nota_debito_nd_numero_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&nd_numero=" + grid_del_nota_debito_getValue("nd_numero", seqRow) + "&lang_doc_anulado=" + grid_del_nota_debito_getValue("lang_doc_anulado", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_del_nota_debito_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
...
function scAjaxCall_actbtn_btn_autorizar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_autorizar_onClick&script_case_init=" + document.F3.script_case_init.value + "&nd_numero=" + grid_del_nota_debito_getValue("nd_numero", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      var i, fieldDisplay, oResp;
      Tst_integrid = jsonReturn.trim();
