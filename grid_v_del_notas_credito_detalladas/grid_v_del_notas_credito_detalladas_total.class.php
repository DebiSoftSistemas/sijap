...
class grid_v_del_notas_credito_detalladas_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_v_del_notas_credito_detalladas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_v_del_notas_credito_detalladas']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('anio' => "anio",'mes' => "mes",'nc_fecha' => "nc_fecha",'nc_serie' => "nc_serie",'nc_secuencial' => "nc_secuencial",'pro_descripcion' => "pro_descripcion",'nc_usuario' => "nc_usuario");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('nc_numero' => 'N','nc_cliente' => 'N','dnc_cantidad' => 'N','dnc_precio_unitario' => 'N','dnc_subtotal' => 'N','dnc_descuento' => 'N','dnc_base_iva' => 'N','dnc_valor_ice' => 'N','dnc_valor_iva' => 'N','dnc_valor_irbpnr' => 'N','dnc_total' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(dnc_subtotal), sum(dnc_descuento), sum(dnc_base_iva), sum(dnc_valor_iva), sum(dnc_total), sum(dnc_valor_ice), sum(dnc_valor_irbpnr)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         ...
         $comando .= "select dnc_subtotal as SC_metric1,dnc_descuento as SC_metric2,dnc_base_iva as SC_metric3,dnc_valor_iva as SC_metric4,dnc_total as SC_metric5,dnc_valor_ice as SC_metric6,dnc_valor_irbpnr as SC_metric7, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['nc_serie']['reg'] = "S";
      $format_dimensions['nc_serie']['msk'] = "";
      $format_dimensions['nc_secuencial']['reg'] = "S";
      $format_dimensions['nc_secuencial']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "nc_serie") {
              }
              if ($Cada_dim == "nc_secuencial") {
              }
   ...
   {
      global $nada, $nm_lang , $mes;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(dnc_subtotal), sum(dnc_descuento), sum(dnc_base_iva), sum(dnc_valor_iva), sum(dnc_total), sum(dnc_valor_ice), sum(dnc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(dnc_subtotal), sum(dnc_descuento), sum(dnc_base_iva), sum(dnc_valor_iva), sum(dnc_total), sum(dnc_valor_ice), sum(dnc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][7] = $rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $rt->fields[7] = (string)$rt->fields[7]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['tot_geral'][8] = $rt->fields[7]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['contr_total_geral'] = "OK";
   } 
   ...
   //-----  nc_serie
   function quebra_nc_serie_sc_free_group_by($nc_serie, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $mes;
      $tot_nc_serie = array() ;  
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_nc_serie[0] = NM_encode_input(sc_strip_script($nc_serie));
      }
      else {
          $tot_nc_serie[0] = sc_strip_script($nc_serie) ; 
      }
      $tot_nc_serie[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nc_serie[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_nc_serie[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_nc_serie[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_nc_serie[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_nc_serie[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_nc_serie[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_nc_serie[8] = (string)$rt->fields[7]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nc_serie;
   } 

   //-----  nc_secuencial
   function quebra_nc_secuencial_sc_free_group_by($nc_secuencial, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $mes;
      $tot_nc_secuencial = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(dnc_subtotal), sum(dnc_descuento), sum(dnc_base_iva), sum(dnc_valor_iva), sum(dnc_total), sum(dnc_valor_ice), sum(dnc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_del_notas_credito_detalladas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_nc_secuencial[0] = NM_encode_input(sc_strip_script($nc_secuencial));
      }
      else {
          $tot_nc_secuencial[0] = sc_strip_script($nc_secuencial) ; 
      }
      $tot_nc_secuencial[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nc_secuencial[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_nc_secuencial[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_nc_secuencial[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_nc_secuencial[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_nc_secuencial[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_nc_secuencial[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_nc_secuencial[8] = (string)$rt->fields[7]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nc_secuencial;
   } 
