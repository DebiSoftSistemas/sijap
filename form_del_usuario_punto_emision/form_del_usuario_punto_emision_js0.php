<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="upe_usuario_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['upe_usuario_']); ?>">
<input type="hidden" name="upe_punto_emision_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['upe_punto_emision_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_usuario_punto_emision"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_del_usuario_punto_emision_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_upe_punto_emision_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.upe_punto_emision_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_usuario_punto_emision')?>/form_del_usuario_punto_emision.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_upe_punto_emision_, "upe_punto_emision_", dados, false);
}
function recebe_upe_punto_emision_(string)
{
   document.getElementById('id_lookup_upe_punto_emision_' + nm_num_campo).innerHTML = string;
}
<?php 
