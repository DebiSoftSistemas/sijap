...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_fac_seleccion_operacion_convenio"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_seleccion_operacion_convenio']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_seleccion_operacion_convenio']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_seleccion_operacion_convenio']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
var Crtl_btn_agregar_todo = false;
function sc_btn_agregar_todo()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_agregar_todo(); }, 500);
      return;
    }
    if (Crtl_btn_agregar_todo) {return;}
    sc_btn_agregar_todo_ok();
}
function sc_btn_agregar_todo_cancel()
{
}
function sc_btn_agregar_todo_ok()
{
    Crtl_btn_agregar_todo = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?agregar_todo?@?";
    document.F1.action = "./";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_fac_seleccion_operacion_convenio_submit_form(y, z);
