...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_registro_lecturas_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_registro_lecturas_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_jap_registro_lecturas_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_simular_ok()
{
    ...
    document.F1.action = "form_jap_registro_lecturas_mob.php";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_imprimir_deudas_var_escenario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['esc_id']); ?>";
function sc_btn_imprimir_deudas()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_imprimir_deudas(); }, 500);
      return;
    }
    sc_btn_imprimir_deudas_ok();
}
function sc_btn_imprimir_deudas_cancel()
{
}
function sc_btn_imprimir_deudas_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_lecturas_multiple; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_escenario*scin' + var_btn_imprimir_deudas_var_escenario + '*scout"; ?>', '', '_blank', '0', '0', 'pdf_lecturas_multiple');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_registro_lecturas_mob']['where_detal']))
  {
?>  
...
function lookup_esc_estado(campo)
{
   ...
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_jap_registro_lecturas_mob')?>/form_jap_registro_lecturas_mob.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_esc_estado, "esc_estado", dados, false);
}
