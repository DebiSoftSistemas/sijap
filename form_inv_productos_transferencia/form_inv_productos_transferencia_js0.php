<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="pt_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pt_id_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_inv_productos_transferencia"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
    Crtl_btn_enviar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?enviar?@?";
    document.F1.action = "./";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_inv_productos_transferencia_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_pt_unidad_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.pt_unidad_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_inv_productos_transferencia')?>/form_inv_productos_transferencia.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_pt_unidad_, "pt_unidad_", dados, false);
}
function recebe_pt_unidad_(string)
{
   document.getElementById('id_lookup_pt_unidad_' + nm_num_campo).innerHTML = string;
}
<?php 
