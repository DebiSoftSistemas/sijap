<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="pro_codigo" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pro_codigo']); ?>">
<input type="hidden" name="pro_empresa" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pro_empresa']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_producto']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_producto']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_producto"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_producto']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_producto_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_producto_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function lookup_pro_iva(campo)
{
   dados  = document.F1.pro_iva.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_producto')?>/form_del_producto.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_pro_iva, "pro_iva", dados, false);
}
function recebe_pro_iva(string)
{
   document.getElementById('id_lookup_pro_iva').innerHTML = string;
}
function lookup_pro_ice(campo)
{
   dados  = document.F1.pro_ice.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_producto')?>/form_del_producto.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_pro_ice, "pro_ice", dados, false);
}
function recebe_pro_ice(string)
{
   document.getElementById('id_lookup_pro_ice').innerHTML = string;
}
function lookup_pro_irbpnr(campo)
{
   dados  = document.F1.pro_irbpnr.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_del_producto')?>/form_del_producto.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_pro_irbpnr, "pro_irbpnr", dados, false);
}
function recebe_pro_irbpnr(string)
{
   document.getElementById('id_lookup_pro_irbpnr').innerHTML = string;
}
<?php 
...
{
   nm_field_disabled("pro_valor_subsidio=disabled;tarifa_ice=disabled;total_ice=disabled;tarifa_iva=disabled;base_iva=disabled;total_iva=disabled;tarifa_irbpnr=disabled;total_irbpnr=disabled;pro_iva=disabled;precio_con_impuesto=disabled;pro_ice=disabled;pro_irbpnr=disabled", "");
