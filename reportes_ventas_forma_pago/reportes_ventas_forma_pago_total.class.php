...
class reportes_ventas_forma_pago_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['reportes_ventas_forma_pago']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['reportes_ventas_forma_pago']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fac_fecha' => "fac_fecha",'fac_tipo_comprobante' => "fac_tipo_comprobante",'fp_descripcion' => "fp_descripcion",'fac_serie' => "fac_serie",'fac_secuencial' => "fac_secuencial",'cl_nombre' => "cl_nombre",'cl_identificacion' => "cl_identificacion",'fac_usuario' => "fac_usuario");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('fac_numero' => 'N','fac_ambiente' => 'N','cl_id' => 'N','fac_total' => 'N','fp_valor' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "fac_usuario") {
                  $this->Lookup->lookup_sc_free_group_by_fac_usuario($$Cada_dim,  $fac_usuario,  $_SESSION['Igtech_RucEmpresa']);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   //----- 
   function Calc_resumo_fac_serie_fac_secuencial($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fac_serie' => "fac_serie");
      $cmps_quebra_atual = array("fac_serie");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_fac_serie_fac_secuencial($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_fac_serie_fac_secuencial($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'fac_serie_fac_secuencial', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_fac_serie_fac_secuencial($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
   ...
   function quebra_geral_sc_free_group_by($res_limit=false, $res_export=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fp_valor) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(fp_valor) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_fac_serie_fac_secuencial($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang , $fac_tipo_comprobante;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['contr_total_geral'] == "OK") 
      { 
   ...
   function quebra_fac_fecha_sc_free_group_by($fac_fecha, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $fac_tipo_comprobante;
      $tot_fac_fecha = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fp_valor) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['where_pesq']; 
   ...
   //-----  fac_serie
   function quebra_fac_serie_fac_serie_fac_secuencial($fac_serie, $arg_sum_fac_serie) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['reportes_ventas_forma_pago']['where_pesq'])) 
      { 
         $nm_comando .= " where fac_serie" . $arg_sum_fac_serie ; 
      } 
      else 
      { 
         $nm_comando .= " and fac_serie" . $arg_sum_fac_serie ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
