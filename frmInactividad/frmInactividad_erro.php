...
$old_error_handler = set_error_handler("frmInactividad_scriptcase_error_handler");

function frmInactividad_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['frmInactividad']['contr_erro']) && $_SESSION['scriptcase']['frmInactividad']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_frmInactividad->contr_frmInactividad->Apl_com_erro = true;
            if ($inicial_frmInactividad->contr_frmInactividad->NM_ajax_flag)
            {
                $inicial_frmInactividad->contr_frmInactividad->NM_ajax_info['errList']['geral_frmInactividad'][] = $msg;
                return;
            }
            frmInactividad_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function frmInactividad_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['frmInactividad'])  && '' != $_SESSION['scriptcase']['error_icon']['frmInactividad'])  ? $_SESSION['scriptcase']['error_icon']['frmInactividad']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['frmInactividad']) && '' != $_SESSION['scriptcase']['error_close']['frmInactividad']) ? $_SESSION['scriptcase']['error_close']['frmInactividad'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['frmInactividad']) && '' != $_SESSION['scriptcase']['error_span_title']['frmInactividad']) ? $_SESSION['scriptcase']['error_span_title']['frmInactividad'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['frmInactividad']) && '' != $_SESSION['scriptcase']['error_icon_title']['frmInactividad']) ? $_SESSION['scriptcase']['error_icon_title']['frmInactividad'] : '';
?>
