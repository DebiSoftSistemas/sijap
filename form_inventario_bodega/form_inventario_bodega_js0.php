<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="bo_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['bo_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_bodega']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_bodega']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_inventario_bodega"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_agregar_var_bodega = "<?php echo $this->form_encode_input($this->nmgp_dados_form['bo_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_form_selecciona_producto_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_bodega*scin' + var_btn_agregar_var_bodega + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'form_selecciona_producto');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_bodega']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_inventario_bodega_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_inventario_bodega_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function lookup_bo_establecimiento(campo)
{
   dados  = document.F1.bo_establecimiento.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_inventario_bodega')?>/form_inventario_bodega.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_bo_establecimiento, "bo_establecimiento", dados, false);
}
function recebe_bo_establecimiento(string)
{
   document.getElementById('id_lookup_bo_establecimiento').innerHTML = string;
}
<?php 
