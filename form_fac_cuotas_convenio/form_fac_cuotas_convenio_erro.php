...
$old_error_handler = set_error_handler("form_fac_cuotas_convenio_scriptcase_error_handler");

function form_fac_cuotas_convenio_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_fac_cuotas_convenio']['contr_erro']) && $_SESSION['scriptcase']['form_fac_cuotas_convenio']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_fac_cuotas_convenio->contr_form_fac_cuotas_convenio->Apl_com_erro = true;
            if ($inicial_form_fac_cuotas_convenio->contr_form_fac_cuotas_convenio->NM_ajax_flag)
            {
                $inicial_form_fac_cuotas_convenio->contr_form_fac_cuotas_convenio->NM_ajax_info['errList']['geral_form_fac_cuotas_convenio'][] = $msg;
                return;
            }
            form_fac_cuotas_convenio_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_fac_cuotas_convenio_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_fac_cuotas_convenio'])  && '' != $_SESSION['scriptcase']['error_icon']['form_fac_cuotas_convenio'])  ? $_SESSION['scriptcase']['error_icon']['form_fac_cuotas_convenio']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_fac_cuotas_convenio']) && '' != $_SESSION['scriptcase']['error_close']['form_fac_cuotas_convenio']) ? $_SESSION['scriptcase']['error_close']['form_fac_cuotas_convenio'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_fac_cuotas_convenio']) && '' != $_SESSION['scriptcase']['error_span_title']['form_fac_cuotas_convenio']) ? $_SESSION['scriptcase']['error_span_title']['form_fac_cuotas_convenio'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_fac_cuotas_convenio']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_fac_cuotas_convenio']) ? $_SESSION['scriptcase']['error_icon_title']['form_fac_cuotas_convenio'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_fac_cuotas_convenio']['error_buffer'] .= ob_get_flush();
}
