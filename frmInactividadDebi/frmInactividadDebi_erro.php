...
$old_error_handler = set_error_handler("frmInactividadDebi_scriptcase_error_handler");

function frmInactividadDebi_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['frmInactividadDebi']['contr_erro']) && $_SESSION['scriptcase']['frmInactividadDebi']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_frmInactividadDebi->contr_frmInactividadDebi->Apl_com_erro = true;
            if ($inicial_frmInactividadDebi->contr_frmInactividadDebi->NM_ajax_flag)
            {
                $inicial_frmInactividadDebi->contr_frmInactividadDebi->NM_ajax_info['errList']['geral_frmInactividadDebi'][] = $msg;
                return;
            }
            frmInactividadDebi_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function frmInactividadDebi_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['frmInactividadDebi'])  && '' != $_SESSION['scriptcase']['error_icon']['frmInactividadDebi'])  ? $_SESSION['scriptcase']['error_icon']['frmInactividadDebi']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['frmInactividadDebi']) && '' != $_SESSION['scriptcase']['error_close']['frmInactividadDebi']) ? $_SESSION['scriptcase']['error_close']['frmInactividadDebi'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['frmInactividadDebi']) && '' != $_SESSION['scriptcase']['error_span_title']['frmInactividadDebi']) ? $_SESSION['scriptcase']['error_span_title']['frmInactividadDebi'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['frmInactividadDebi']) && '' != $_SESSION['scriptcase']['error_icon_title']['frmInactividadDebi']) ? $_SESSION['scriptcase']['error_icon_title']['frmInactividadDebi'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['frmInactividadDebi']['error_buffer'] .= ob_get_flush();
}
