function grid_v_del_datos_guia_remision_sc_field_0_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_sc_field_0" + seqRow).html();
}

function grid_v_del_datos_guia_remision_sc_field_0_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_sc_field_0" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_secuencial_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_secuencial" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_secuencial_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_secuencial" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_tr_nombre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tr_nombre" + seqRow).html();
}

function grid_v_del_datos_guia_remision_tr_nombre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tr_nombre" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_tr_identificacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tr_identificacion" + seqRow).html();
}

function grid_v_del_datos_guia_remision_tr_identificacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tr_identificacion" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_tr_placa_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tr_placa" + seqRow).html();
}

function grid_v_del_datos_guia_remision_tr_placa_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tr_placa" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_fecha_ini_transporte_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_fecha_ini_transporte" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_fecha_ini_transporte_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_fecha_ini_transporte" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_fecha_fin_transporte_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_fecha_fin_transporte" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_fecha_fin_transporte_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_fecha_fin_transporte" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_estado_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_estado_sri" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_estado_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_estado_sri" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_numero_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_gr_numero" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_numero_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_numero" + seqRow).html(newValue);
}
function grid_v_del_datos_guia_remision_gr_numero_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_gr_numero" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_tipo_comprobante_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_tipo_comprobante" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_tipo_comprobante_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_tipo_comprobante" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_empresa_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_empresa" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_empresa_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_empresa" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_error_sri_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_error_sri" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_error_sri_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_error_sri" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_gr_autorizacion_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_gr_autorizacion" + seqRow).html();
}

function grid_v_del_datos_guia_remision_gr_autorizacion_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_gr_autorizacion" + seqRow).html(newValue);
}

function grid_v_del_datos_guia_remision_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("sc_field_0" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_sc_field_0_getValue(seqRow);
  }
  if ("gr_secuencial" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_secuencial_getValue(seqRow);
  }
  if ("tr_nombre" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_tr_nombre_getValue(seqRow);
  }
  if ("tr_identificacion" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_tr_identificacion_getValue(seqRow);
  }
  if ("tr_placa" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_tr_placa_getValue(seqRow);
  }
  if ("gr_fecha_ini_transporte" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_fecha_ini_transporte_getValue(seqRow);
  }
  if ("gr_fecha_fin_transporte" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_fecha_fin_transporte_getValue(seqRow);
  }
  if ("gr_estado_sri" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_estado_sri_getValue(seqRow);
  }
  if ("gr_numero" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_numero_getValue(seqRow);
  }
  if ("gr_tipo_comprobante" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_tipo_comprobante_getValue(seqRow);
  }
  if ("gr_empresa" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_empresa_getValue(seqRow);
  }
  if ("gr_error_sri" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_error_sri_getValue(seqRow);
  }
  if ("gr_autorizacion" == field) {
    SC_tmp = grid_v_del_datos_guia_remision_gr_autorizacion_getValue(seqRow);
  }
...
function grid_v_del_datos_guia_remision_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("sc_field_0" == field) {
    grid_v_del_datos_guia_remision_sc_field_0_setValue(newValue, seqRow);
  }
  if ("gr_secuencial" == field) {
    grid_v_del_datos_guia_remision_gr_secuencial_setValue(newValue, seqRow);
  }
  if ("tr_nombre" == field) {
    grid_v_del_datos_guia_remision_tr_nombre_setValue(newValue, seqRow);
  }
  if ("tr_identificacion" == field) {
    grid_v_del_datos_guia_remision_tr_identificacion_setValue(newValue, seqRow);
  }
  if ("tr_placa" == field) {
    grid_v_del_datos_guia_remision_tr_placa_setValue(newValue, seqRow);
  }
  if ("gr_fecha_ini_transporte" == field) {
    grid_v_del_datos_guia_remision_gr_fecha_ini_transporte_setValue(newValue, seqRow);
  }
  if ("gr_fecha_fin_transporte" == field) {
    grid_v_del_datos_guia_remision_gr_fecha_fin_transporte_setValue(newValue, seqRow);
  }
  if ("gr_estado_sri" == field) {
    grid_v_del_datos_guia_remision_gr_estado_sri_setValue(newValue, seqRow);
  }
  if ("gr_numero" == field) {
    grid_v_del_datos_guia_remision_gr_numero_setValue(newValue, seqRow);
  }
  if ("gr_tipo_comprobante" == field) {
    grid_v_del_datos_guia_remision_gr_tipo_comprobante_setValue(newValue, seqRow);
  }
  if ("gr_empresa" == field) {
    grid_v_del_datos_guia_remision_gr_empresa_setValue(newValue, seqRow);
  }
  if ("gr_error_sri" == field) {
    grid_v_del_datos_guia_remision_gr_error_sri_setValue(newValue, seqRow);
  }
  if ("gr_autorizacion" == field) {
    grid_v_del_datos_guia_remision_gr_autorizacion_setValue(newValue, seqRow);
  }
  if ("gr_numero_Hidden" == field) {
    grid_v_del_datos_guia_remision_gr_numero_Hidden_setValue(newValue, seqRow);
  }
}
...
function scAjaxCall_actbtn_btn_anular_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_anular_onClick&script_case_init=" + document.F3.script_case_init.value + "&gr_numero=" + grid_v_del_datos_guia_remision_getValue("gr_numero", seqRow) + "&lang_doc_anulado=" + grid_v_del_datos_guia_remision_getValue("lang_doc_anulado", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_v_del_datos_guia_remision_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
...
function scAjaxCall_actbtn_btn_autorizar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  ...
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=actbtn_btn_autorizar_onClick&script_case_init=" + document.F3.script_case_init.value + "&gr_numero=" + grid_v_del_datos_guia_remision_getValue("gr_numero", seqRow) + "&actionbar_clicked_state=" + actionBarButtonState + ""
   })
   .done(function(jsonReturn) {
      var i, fieldDisplay, oResp;
      Tst_integrid = jsonReturn.trim();
