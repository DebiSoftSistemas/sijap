<?php

class grid_jap_contratos_cliente_cartera_rtf
{
   var $Db;
   var $Erro;
   var $Ini;
   var $Lookup;
   var $nm_data;
   var $Texto_tag;
   var $Arquivo;
   var $Tit_doc;
   var $sc_proc_grid; 
   var $NM_cmp_hidden = array();

   //---- 
   function __construct()
   {
      $this->nm_data   = new nm_data("es");
      $this->Texto_tag = "";
   }


function actionBar_isValidState($buttonName, $buttonState)
{
    switch ($buttonName) {
    }

    return false;
}


function actionBar_displayState($buttonName)
{
    switch ($buttonName) {
    }
}

function actionBar_getStateHint($buttonName)
{
    switch ($buttonName) {
    }
}

function actionBar_getStateConfirm($buttonName)
{
    switch ($buttonName) {
    }
}

function actionBar_getStateDisable($buttonName)
{
    if (isset($this->sc_actionbar_disabled[$buttonName]) && $this->sc_actionbar_disabled[$buttonName]) {
        return ' disabled';
    }

    return '';
}

function actionBar_getStateHide($buttonName)
{
    if (isset($this->sc_actionbar_hidden[$buttonName]) && $this->sc_actionbar_hidden[$buttonName]) {
        return ' sc-actionbar-button-hidden';
    }

    return '';
}

   //---- 
   function monta_rtf()
   {
      $this->inicializa_vars();
      $this->gera_texto_tag();
      $this->grava_arquivo_rtf();
      if ($this->Ini->sc_export_ajax)
      {
          $this->Arr_result['file_export']  = NM_charset_to_utf8($this->Rtf_f);
          $this->Arr_result['title_export'] = NM_charset_to_utf8($this->Tit_doc);
          $Temp = ob_get_clean();
          if ($Temp !== false && trim($Temp) != "")
          {
              $this->Arr_result['htmOutput'] = NM_charset_to_utf8($Temp);
          }
          $oJson = new Services_JSON();
          echo $oJson->encode($this->Arr_result);
          exit;
      }
      else
      {
          $this->progress_bar_end();
      }
   }

   //----- 
   function inicializa_vars()
   {
      global $nm_lang;
      if (isset($GLOBALS['nmgp_parms']) && !empty($GLOBALS['nmgp_parms'])) 
      { 
          $GLOBALS['nmgp_parms'] = str_replace("@aspass@", "'", $GLOBALS['nmgp_parms']);
          $todox = str_replace("?#?@?@?", "?#?@ ?@?", $GLOBALS["nmgp_parms"]);
          $todo  = explode("?@?", $todox);
          foreach ($todo as $param)
          {
               $cadapar = explode("?#?", $param);
               if (1 < sizeof($cadapar))
               {
                   if (substr($cadapar[0], 0, 11) == "SC_glo_par_")
                   {
                       $cadapar[0] = substr($cadapar[0], 11);
                       $cadapar[1] = $_SESSION[$cadapar[1]];
                   }
                   if (isset($GLOBALS['sc_conv_var'][$cadapar[0]]))
                   {
                       $cadapar[0] = $GLOBALS['sc_conv_var'][$cadapar[0]];
                   }
                   elseif (isset($GLOBALS['sc_conv_var'][strtolower($cadapar[0])]))
                   {
                       $cadapar[0] = $GLOBALS['sc_conv_var'][strtolower($cadapar[0])];
                   }
                   nm_limpa_str_grid_jap_contratos_cliente_cartera($cadapar[1]);
                   nm_protect_num_grid_jap_contratos_cliente_cartera($cadapar[0], $cadapar[1]);
                   if ($cadapar[1] == "@ ") {$cadapar[1] = trim($cadapar[1]); }
                   $Tmp_par   = $cadapar[0];
                   $$Tmp_par = $cadapar[1];
                   if ($Tmp_par == "nmgp_opcao")
                   {
                       $_SESSION['sc_session'][$script_case_init]['grid_jap_contratos_cliente_cartera']['opcao'] = $cadapar[1];
                   }
               }
          }
      }
      if (!isset($Igtech_RucEmpresa) && isset($igtech_rucempresa)) 
      {
         $Igtech_RucEmpresa = $igtech_rucempresa;
      }
      if (isset($Igtech_RucEmpresa)) 
      {
          $_SESSION['Igtech_RucEmpresa'] = $Igtech_RucEmpresa;
          nm_limpa_str_grid_jap_contratos_cliente_cartera($_SESSION["Igtech_RucEmpresa"]);
      }
      if (isset($var_cliente)) 
      {
          $_SESSION['var_cliente'] = $var_cliente;
          nm_limpa_str_grid_jap_contratos_cliente_cartera($_SESSION["var_cliente"]);
      }
      $dir_raiz          = strrpos($_SERVER['PHP_SELF'],"/") ;  
      $dir_raiz          = substr($_SERVER['PHP_SELF'], 0, $dir_raiz + 1) ;  
      $this->nm_location = $this->Ini->sc_protocolo . $this->Ini->server . $dir_raiz; 
      require_once($this->Ini->path_aplicacao . "grid_jap_contratos_cliente_cartera_total.class.php"); 
      $this->Tot      = new grid_jap_contratos_cliente_cartera_total($this->Ini->sc_page);
      $this->prep_modulos("Tot");
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['SC_Ind_Groupby'];
      if (method_exists($this->Tot,$Gb_geral))
      {
          $this->Tot->$Gb_geral();
          $this->count_ger = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['tot_geral'][1];
      }
      if (!$this->Ini->sc_export_ajax) {
          require_once($this->Ini->path_lib_php . "/sc_progress_bar.php");
          $this->pb = new scProgressBar();
          $this->pb->setRoot($this->Ini->root);
          $this->pb->setDir($_SESSION['scriptcase']['grid_jap_contratos_cliente_cartera']['glo_nm_path_imag_temp'] . "/");
          $this->pb->setProgressbarMd5($_GET['pbmd5']);
          $this->pb->initialize();
          $this->pb->setReturnUrl("./");
          $this->pb->setReturnOption('volta_grid');
          $this->pb->setTotalSteps($this->count_ger);
      }
      $this->Arquivo    = "sc_rtf";
      $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
      $this->Arquivo   .= "_grid_jap_contratos_cliente_cartera";
      $this->Arquivo   .= ".rtf";
      $this->Tit_doc    = "grid_jap_contratos_cliente_cartera.rtf";
   }
   //---- 
   function prep_modulos($modulo)
   {
      $this->$modulo->Ini    = $this->Ini;
      $this->$modulo->Db     = $this->Db;
      $this->$modulo->Erro   = $this->Erro;
      $this->$modulo->Lookup = $this->Lookup;
   }


   //----- 
   function gera_texto_tag()
   {
     global $nm_lang;
      global $nm_nada, $nm_lang;

      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      $this->sc_proc_grid = false; 
      $nm_raiz_img  = ""; 
      $this->New_label['con_numero_contrato'] = "Nro. " . $this->Ini->Nm_lang['lang_contrato'] . "";
      $this->New_label['con_condicion_contrato'] = "" . $this->Ini->Nm_lang['lang_condicion_contrato'] . "";
      $this->New_label['con_tipo_servicio'] = "" . $this->Ini->Nm_lang['lang_tipo_servicio'] . "";
      $this->New_label['con_numero_medidor'] = "" . $this->Ini->Nm_lang['lang_numero_medidor'] . "";
      $this->New_label['con_ruta'] = "" . $this->Ini->Nm_lang['lang_ruta'] . "";
      $this->New_label['con_direccion_predio'] = "" . $this->Ini->Nm_lang['lang_direccion_predio'] . "";
      $this->New_label['con_lectura_actual'] = "" . $this->Ini->Nm_lang['lang_lectura_actual'] . "";
      $this->New_label['con_estado'] = "" . $this->Ini->Nm_lang['lang_estado'] . "";
      $this->New_label['historial'] = "" . $this->Ini->Nm_lang['lang_historial'] . "";
      $this->New_label['con_id'] = "" . $this->Ini->Nm_lang['lang_id'] . "";
      $this->New_label['con_nombre'] = "" . $this->Ini->Nm_lang['lang_lot_nombre_completo'] . "";
      $this->New_label['con_cedula'] = "" . $this->Ini->Nm_lang['lang_cedula'] . "";
      $this->New_label['con_telefono'] = "" . $this->Ini->Nm_lang['lang_lot_telefono'] . "";
      $this->New_label['con_correo'] = "" . $this->Ini->Nm_lang['lang_email'] . "";
      $this->New_label['con_toma_administrativa'] = "" . $this->Ini->Nm_lang['lang_toma_administrativa'] . "";
      $this->New_label['con_toma_operativa'] = "" . $this->Ini->Nm_lang['lang_toma_operativa'] . "";
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_jap_contratos_cliente_cartera']['field_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_jap_contratos_cliente_cartera']['field_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_jap_contratos_cliente_cartera']['field_display'] as $NM_cada_field => $NM_cada_opc)
          {
              $this->NM_cmp_hidden[$NM_cada_field] = $NM_cada_opc;
          }
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['usr_cmp_sel']))
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
          {
              $this->NM_cmp_hidden[$NM_cada_field] = $NM_cada_opc;
          }
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['php_cmp_sel']))
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
          {
              $this->NM_cmp_hidden[$NM_cada_field] = $NM_cada_opc;
          }
      }
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_pesq_filtro'];
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['campos_busca']))
      { 
          $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['campos_busca'];
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $Busca_temp = NM_conv_charset($Busca_temp, $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $this->con_nombre = (isset($Busca_temp['con_nombre'])) ? $Busca_temp['con_nombre'] : ""; 
          $tmp_pos = (is_string($this->con_nombre)) ? strpos($this->con_nombre, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_nombre))
          {
              $this->con_nombre = substr($this->con_nombre, 0, $tmp_pos);
          }
          $this->con_cedula = (isset($Busca_temp['con_cedula'])) ? $Busca_temp['con_cedula'] : ""; 
          $tmp_pos = (is_string($this->con_cedula)) ? strpos($this->con_cedula, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_cedula))
          {
              $this->con_cedula = substr($this->con_cedula, 0, $tmp_pos);
          }
          $this->con_tipo_servicio = (isset($Busca_temp['con_tipo_servicio'])) ? $Busca_temp['con_tipo_servicio'] : ""; 
          $tmp_pos = (is_string($this->con_tipo_servicio)) ? strpos($this->con_tipo_servicio, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_tipo_servicio))
          {
              $this->con_tipo_servicio = substr($this->con_tipo_servicio, 0, $tmp_pos);
          }
          $this->con_condicion_contrato = (isset($Busca_temp['con_condicion_contrato'])) ? $Busca_temp['con_condicion_contrato'] : ""; 
          $tmp_pos = (is_string($this->con_condicion_contrato)) ? strpos($this->con_condicion_contrato, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_condicion_contrato))
          {
              $this->con_condicion_contrato = substr($this->con_condicion_contrato, 0, $tmp_pos);
          }
          $this->con_ruta = (isset($Busca_temp['con_ruta'])) ? $Busca_temp['con_ruta'] : ""; 
          $tmp_pos = (is_string($this->con_ruta)) ? strpos($this->con_ruta, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_ruta))
          {
              $this->con_ruta = substr($this->con_ruta, 0, $tmp_pos);
          }
          $this->con_numero_contrato = (isset($Busca_temp['con_numero_contrato'])) ? $Busca_temp['con_numero_contrato'] : ""; 
          $tmp_pos = (is_string($this->con_numero_contrato)) ? strpos($this->con_numero_contrato, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_numero_contrato))
          {
              $this->con_numero_contrato = substr($this->con_numero_contrato, 0, $tmp_pos);
          }
          $this->con_numero_medidor = (isset($Busca_temp['con_numero_medidor'])) ? $Busca_temp['con_numero_medidor'] : ""; 
          $tmp_pos = (is_string($this->con_numero_medidor)) ? strpos($this->con_numero_medidor, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_numero_medidor))
          {
              $this->con_numero_medidor = substr($this->con_numero_medidor, 0, $tmp_pos);
          }
          $this->con_estado = (isset($Busca_temp['con_estado'])) ? $Busca_temp['con_estado'] : ""; 
          $tmp_pos = (is_string($this->con_estado)) ? strpos($this->con_estado, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->con_estado))
          {
              $this->con_estado = substr($this->con_estado, 0, $tmp_pos);
          }
      } 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_name'] .= ".rtf";
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_name']);
      }
      $this->arr_export = array('label' => array(), 'lines' => array());
      $this->arr_span   = array();

      $this->Texto_tag .= "<table>\r\n";
      $this->Texto_tag .= "<tr>\r\n";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['field_order'] as $Cada_col)
      { 
          $SC_Label = (isset($this->New_label['con_numero_contrato'])) ? $this->New_label['con_numero_contrato'] : ""; 
          if ($Cada_col == "con_numero_contrato" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_condicion_contrato'])) ? $this->New_label['con_condicion_contrato'] : ""; 
          if ($Cada_col == "con_condicion_contrato" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_tipo_servicio'])) ? $this->New_label['con_tipo_servicio'] : ""; 
          if ($Cada_col == "con_tipo_servicio" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_numero_medidor'])) ? $this->New_label['con_numero_medidor'] : ""; 
          if ($Cada_col == "con_numero_medidor" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_ruta'])) ? $this->New_label['con_ruta'] : ""; 
          if ($Cada_col == "con_ruta" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_direccion_predio'])) ? $this->New_label['con_direccion_predio'] : ""; 
          if ($Cada_col == "con_direccion_predio" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_lectura_actual'])) ? $this->New_label['con_lectura_actual'] : ""; 
          if ($Cada_col == "con_lectura_actual" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_estado'])) ? $this->New_label['con_estado'] : ""; 
          if ($Cada_col == "con_estado" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['historial'])) ? $this->New_label['historial'] : ""; 
          if ($Cada_col == "historial" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_id'])) ? $this->New_label['con_id'] : ""; 
          if ($Cada_col == "con_id" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_propietario'])) ? $this->New_label['con_propietario'] : "Con Propietario"; 
          if ($Cada_col == "con_propietario" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_nombre'])) ? $this->New_label['con_nombre'] : ""; 
          if ($Cada_col == "con_nombre" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_cedula'])) ? $this->New_label['con_cedula'] : ""; 
          if ($Cada_col == "con_cedula" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_telefono'])) ? $this->New_label['con_telefono'] : ""; 
          if ($Cada_col == "con_telefono" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_correo'])) ? $this->New_label['con_correo'] : ""; 
          if ($Cada_col == "con_correo" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_toma_administrativa'])) ? $this->New_label['con_toma_administrativa'] : ""; 
          if ($Cada_col == "con_toma_administrativa" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
          $SC_Label = (isset($this->New_label['con_toma_operativa'])) ? $this->New_label['con_toma_operativa'] : ""; 
          if ($Cada_col == "con_toma_operativa" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $SC_Label = NM_charset_to_utf8($SC_Label);
              $SC_Label = str_replace('<', '&lt;', $SC_Label);
              $SC_Label = str_replace('>', '&gt;', $SC_Label);
              $this->Texto_tag .= "<td>" . $SC_Label . "</td>\r\n";
          }
      } 
      $this->Texto_tag .= "</tr>\r\n";
      $this->nm_field_dinamico = array();
      $this->nm_order_dinamico = array();
      $nmgp_select_count = "SELECT count(*) AS countTest from " . $this->Ini->nm_tabela; 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      { 
          $nmgp_select = "SELECT con_numero_contrato, con_condicion_contrato, con_tipo_servicio, con_numero_medidor, con_ruta, con_direccion_predio, con_lectura_actual, con_estado, con_id, con_propietario, con_nombre, con_cedula, con_telefono, con_correo, con_toma_administrativa, con_toma_operativa from " . $this->Ini->nm_tabela; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mysql))
      { 
          $nmgp_select = "SELECT con_numero_contrato, con_condicion_contrato, con_tipo_servicio, con_numero_medidor, con_ruta, con_direccion_predio, con_lectura_actual, con_estado, con_id, con_propietario, con_nombre, con_cedula, con_telefono, con_correo, con_toma_administrativa, con_toma_operativa from " . $this->Ini->nm_tabela; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
       $nmgp_select = "SELECT con_numero_contrato, con_condicion_contrato, con_tipo_servicio, con_numero_medidor, con_ruta, con_direccion_predio, con_lectura_actual, con_estado, con_id, con_propietario, con_nombre, con_cedula, con_telefono, con_correo, con_toma_administrativa, con_toma_operativa from " . $this->Ini->nm_tabela; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_oracle))
      { 
          $nmgp_select = "SELECT con_numero_contrato, con_condicion_contrato, con_tipo_servicio, con_numero_medidor, con_ruta, con_direccion_predio, con_lectura_actual, con_estado, con_id, con_propietario, con_nombre, con_cedula, con_telefono, con_correo, con_toma_administrativa, con_toma_operativa from " . $this->Ini->nm_tabela; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_informix))
      { 
          $nmgp_select = "SELECT con_numero_contrato, con_condicion_contrato, con_tipo_servicio, con_numero_medidor, con_ruta, con_direccion_predio, con_lectura_actual, con_estado, con_id, con_propietario, con_nombre, con_cedula, con_telefono, con_correo, con_toma_administrativa, con_toma_operativa from " . $this->Ini->nm_tabela; 
      } 
      else 
      { 
          $nmgp_select = "SELECT con_numero_contrato, con_condicion_contrato, con_tipo_servicio, con_numero_medidor, con_ruta, con_direccion_predio, con_lectura_actual, con_estado, con_id, con_propietario, con_nombre, con_cedula, con_telefono, con_correo, con_toma_administrativa, con_toma_operativa from " . $this->Ini->nm_tabela; 
      } 
      $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_pesq'];
      $nmgp_select_count .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_pesq'];
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_resumo']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_resumo'])) 
      { 
          if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_pesq'])) 
          { 
              $nmgp_select .= " where " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_resumo']; 
              $nmgp_select_count .= " where " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_resumo']; 
          } 
          else
          { 
              $nmgp_select .= " and (" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_resumo'] . ")"; 
              $nmgp_select_count .= " and (" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['where_resumo'] . ")"; 
          } 
      } 
      $nmgp_order_by = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['order_grid'];
      $nmgp_select .= $nmgp_order_by; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nmgp_select_count;
      $rt = $this->Db->Execute($nmgp_select_count);
      if ($rt === false && !$rt->EOF && $GLOBALS["NM_ERRO_IBASE"] != 1)
      {
         $this->Erro->mensagem(__FILE__, __LINE__, "banco", $this->Ini->Nm_lang['lang_errm_dber'], $this->Db->ErrorMsg());
         exit;
      }
      $this->count_ger = $rt->fields[0];
      $rt->Close();
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nmgp_select;
      $rs = $this->Db->Execute($nmgp_select);
      if ($rs === false && !$rs->EOF && $GLOBALS["NM_ERRO_IBASE"] != 1)
      {
         $this->Erro->mensagem(__FILE__, __LINE__, "banco", $this->Ini->Nm_lang['lang_errm_dber'], $this->Db->ErrorMsg());
         exit;
      }
      $this->SC_seq_register = 0;
      $PB_tot = (isset($this->count_ger) && $this->count_ger > 0) ? "/" . $this->count_ger : "";
      while (!$rs->EOF)
      {
         $this->SC_seq_register++;
         if (!$this->Ini->sc_export_ajax) {
             $Mens_bar = NM_charset_to_utf8($this->Ini->Nm_lang['lang_othr_prcs']);
             $this->pb->setProgressbarMessage($Mens_bar . ": " . $this->SC_seq_register . $PB_tot);
             $this->pb->addSteps(1);
         }
         $this->Texto_tag .= "<tr>\r\n";
         $this->con_numero_contrato = $rs->fields[0] ;  
         $this->con_condicion_contrato = $rs->fields[1] ;  
         $this->con_condicion_contrato = (string)$this->con_condicion_contrato;
         $this->con_tipo_servicio = $rs->fields[2] ;  
         $this->con_tipo_servicio = (string)$this->con_tipo_servicio;
         $this->con_numero_medidor = $rs->fields[3] ;  
         $this->con_ruta = $rs->fields[4] ;  
         $this->con_ruta = (string)$this->con_ruta;
         $this->con_direccion_predio = $rs->fields[5] ;  
         $this->con_lectura_actual = $rs->fields[6] ;  
         $this->con_lectura_actual = (string)$this->con_lectura_actual;
         $this->con_estado = $rs->fields[7] ;  
         $this->con_id = $rs->fields[8] ;  
         $this->con_id = (string)$this->con_id;
         $this->con_propietario = $rs->fields[9] ;  
         $this->con_propietario = (string)$this->con_propietario;
         $this->con_nombre = $rs->fields[10] ;  
         $this->con_cedula = $rs->fields[11] ;  
         $this->con_telefono = $rs->fields[12] ;  
         $this->con_correo = $rs->fields[13] ;  
         $this->con_toma_administrativa = $rs->fields[14] ;  
         $this->con_toma_administrativa = (string)$this->con_toma_administrativa;
         $this->con_toma_operativa = $rs->fields[15] ;  
         $this->con_toma_operativa = (string)$this->con_toma_operativa;
         //----- lookup - con_condicion_contrato
         $this->look_con_condicion_contrato = $this->con_condicion_contrato; 
         $this->Lookup->lookup_con_condicion_contrato($this->look_con_condicion_contrato, $this->con_condicion_contrato) ; 
         $this->look_con_condicion_contrato = ($this->look_con_condicion_contrato == "&nbsp;") ? "" : $this->look_con_condicion_contrato; 
         //----- lookup - con_tipo_servicio
         $this->look_con_tipo_servicio = $this->con_tipo_servicio; 
         $this->Lookup->lookup_con_tipo_servicio($this->look_con_tipo_servicio, $this->con_tipo_servicio) ; 
         $this->look_con_tipo_servicio = ($this->look_con_tipo_servicio == "&nbsp;") ? "" : $this->look_con_tipo_servicio; 
         //----- lookup - con_ruta
         $this->look_con_ruta = $this->con_ruta; 
         $this->Lookup->lookup_con_ruta($this->look_con_ruta, $this->con_ruta) ; 
         $this->look_con_ruta = ($this->look_con_ruta == "&nbsp;") ? "" : $this->look_con_ruta; 
         //----- lookup - con_estado
         $this->look_con_estado = $this->con_estado; 
         $this->Lookup->lookup_con_estado($this->look_con_estado, $this->con_estado) ; 
         $this->look_con_estado = ($this->look_con_estado == "&nbsp;") ? "" : $this->look_con_estado; 
         //----- lookup - con_toma_administrativa
         $this->look_con_toma_administrativa = $this->con_toma_administrativa; 
         $this->Lookup->lookup_con_toma_administrativa($this->look_con_toma_administrativa, $this->con_toma_administrativa) ; 
         $this->look_con_toma_administrativa = ($this->look_con_toma_administrativa == "&nbsp;") ? "" : $this->look_con_toma_administrativa; 
         //----- lookup - con_toma_operativa
         $this->look_con_toma_operativa = $this->con_toma_operativa; 
         $this->Lookup->lookup_con_toma_operativa($this->look_con_toma_operativa, $this->con_toma_operativa) ; 
         $this->look_con_toma_operativa = ($this->look_con_toma_operativa == "&nbsp;") ? "" : $this->look_con_toma_operativa; 
         $this->sc_proc_grid = true; 
         foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['field_order'] as $Cada_col)
         { 
            if (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off")
            { 
                $NM_func_exp = "NM_export_" . $Cada_col;
                $this->$NM_func_exp();
            } 
         } 
         $this->Texto_tag .= "</tr>\r\n";
         $rs->MoveNext();
      }
      $this->Texto_tag .= "</table>\r\n";
      if(isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['export_sel_columns']['field_order']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['field_order'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['export_sel_columns']['field_order'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['export_sel_columns']['field_order']);
      }
      if(isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['export_sel_columns']['usr_cmp_sel']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['usr_cmp_sel'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['export_sel_columns']['usr_cmp_sel'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['export_sel_columns']['usr_cmp_sel']);
      }
      $rs->Close();
   }
   //----- con_numero_contrato
   function NM_export_con_numero_contrato()
   {
             nmgp_Form_Num_Val($this->con_numero_contrato, "", "", "0", "S", "2", "", "N:1", "-") ; 
         $this->con_numero_contrato = NM_charset_to_utf8($this->con_numero_contrato);
         $this->con_numero_contrato = str_replace('<', '&lt;', $this->con_numero_contrato);
         $this->con_numero_contrato = str_replace('>', '&gt;', $this->con_numero_contrato);
         $this->Texto_tag .= "<td>" . $this->con_numero_contrato . "</td>\r\n";
   }
   //----- con_condicion_contrato
   function NM_export_con_condicion_contrato()
   {
         nmgp_Form_Num_Val($this->look_con_condicion_contrato, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->look_con_condicion_contrato = NM_charset_to_utf8($this->look_con_condicion_contrato);
         $this->look_con_condicion_contrato = str_replace('<', '&lt;', $this->look_con_condicion_contrato);
         $this->look_con_condicion_contrato = str_replace('>', '&gt;', $this->look_con_condicion_contrato);
         $this->Texto_tag .= "<td>" . $this->look_con_condicion_contrato . "</td>\r\n";
   }
   //----- con_tipo_servicio
   function NM_export_con_tipo_servicio()
   {
         nmgp_Form_Num_Val($this->look_con_tipo_servicio, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->look_con_tipo_servicio = NM_charset_to_utf8($this->look_con_tipo_servicio);
         $this->look_con_tipo_servicio = str_replace('<', '&lt;', $this->look_con_tipo_servicio);
         $this->look_con_tipo_servicio = str_replace('>', '&gt;', $this->look_con_tipo_servicio);
         $this->Texto_tag .= "<td>" . $this->look_con_tipo_servicio . "</td>\r\n";
   }
   //----- con_numero_medidor
   function NM_export_con_numero_medidor()
   {
         $this->con_numero_medidor = html_entity_decode($this->con_numero_medidor, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->con_numero_medidor = strip_tags($this->con_numero_medidor);
         $this->con_numero_medidor = NM_charset_to_utf8($this->con_numero_medidor);
         $this->con_numero_medidor = str_replace('<', '&lt;', $this->con_numero_medidor);
         $this->con_numero_medidor = str_replace('>', '&gt;', $this->con_numero_medidor);
         $this->Texto_tag .= "<td>" . $this->con_numero_medidor . "</td>\r\n";
   }
   //----- con_ruta
   function NM_export_con_ruta()
   {
         nmgp_Form_Num_Val($this->look_con_ruta, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->look_con_ruta = NM_charset_to_utf8($this->look_con_ruta);
         $this->look_con_ruta = str_replace('<', '&lt;', $this->look_con_ruta);
         $this->look_con_ruta = str_replace('>', '&gt;', $this->look_con_ruta);
         $this->Texto_tag .= "<td>" . $this->look_con_ruta . "</td>\r\n";
   }
   //----- con_direccion_predio
   function NM_export_con_direccion_predio()
   {
         $this->con_direccion_predio = html_entity_decode($this->con_direccion_predio, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->con_direccion_predio = strip_tags($this->con_direccion_predio);
         $this->con_direccion_predio = NM_charset_to_utf8($this->con_direccion_predio);
         $this->con_direccion_predio = str_replace('<', '&lt;', $this->con_direccion_predio);
         $this->con_direccion_predio = str_replace('>', '&gt;', $this->con_direccion_predio);
         $this->Texto_tag .= "<td>" . $this->con_direccion_predio . "</td>\r\n";
   }
   //----- con_lectura_actual
   function NM_export_con_lectura_actual()
   {
             nmgp_Form_Num_Val($this->con_lectura_actual, "", "", "0", "S", "2", "", "N:1", "-") ; 
         $this->con_lectura_actual = NM_charset_to_utf8($this->con_lectura_actual);
         $this->con_lectura_actual = str_replace('<', '&lt;', $this->con_lectura_actual);
         $this->con_lectura_actual = str_replace('>', '&gt;', $this->con_lectura_actual);
         $this->Texto_tag .= "<td>" . $this->con_lectura_actual . "</td>\r\n";
   }
   //----- con_estado
   function NM_export_con_estado()
   {
         $this->look_con_estado = html_entity_decode($this->look_con_estado, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->look_con_estado = strip_tags($this->look_con_estado);
         $this->look_con_estado = NM_charset_to_utf8($this->look_con_estado);
         $this->look_con_estado = str_replace('<', '&lt;', $this->look_con_estado);
         $this->look_con_estado = str_replace('>', '&gt;', $this->look_con_estado);
         $this->Texto_tag .= "<td>" . $this->look_con_estado . "</td>\r\n";
   }
   //----- historial
   function NM_export_historial()
   {
         $this->historial = NM_charset_to_utf8($this->historial);
         $this->historial = str_replace('<', '&lt;', $this->historial);
         $this->historial = str_replace('>', '&gt;', $this->historial);
         $this->Texto_tag .= "<td>" . $this->historial . "</td>\r\n";
   }
   //----- con_id
   function NM_export_con_id()
   {
             nmgp_Form_Num_Val($this->con_id, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->con_id = NM_charset_to_utf8($this->con_id);
         $this->con_id = str_replace('<', '&lt;', $this->con_id);
         $this->con_id = str_replace('>', '&gt;', $this->con_id);
         $this->Texto_tag .= "<td>" . $this->con_id . "</td>\r\n";
   }
   //----- con_propietario
   function NM_export_con_propietario()
   {
             nmgp_Form_Num_Val($this->con_propietario, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->con_propietario = NM_charset_to_utf8($this->con_propietario);
         $this->con_propietario = str_replace('<', '&lt;', $this->con_propietario);
         $this->con_propietario = str_replace('>', '&gt;', $this->con_propietario);
         $this->Texto_tag .= "<td>" . $this->con_propietario . "</td>\r\n";
   }
   //----- con_nombre
   function NM_export_con_nombre()
   {
         $this->con_nombre = html_entity_decode($this->con_nombre, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->con_nombre = strip_tags($this->con_nombre);
         $this->con_nombre = NM_charset_to_utf8($this->con_nombre);
         $this->con_nombre = str_replace('<', '&lt;', $this->con_nombre);
         $this->con_nombre = str_replace('>', '&gt;', $this->con_nombre);
         $this->Texto_tag .= "<td>" . $this->con_nombre . "</td>\r\n";
   }
   //----- con_cedula
   function NM_export_con_cedula()
   {
         $this->con_cedula = html_entity_decode($this->con_cedula, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->con_cedula = strip_tags($this->con_cedula);
         $this->con_cedula = NM_charset_to_utf8($this->con_cedula);
         $this->con_cedula = str_replace('<', '&lt;', $this->con_cedula);
         $this->con_cedula = str_replace('>', '&gt;', $this->con_cedula);
         $this->Texto_tag .= "<td>" . $this->con_cedula . "</td>\r\n";
   }
   //----- con_telefono
   function NM_export_con_telefono()
   {
         $this->con_telefono = html_entity_decode($this->con_telefono, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->con_telefono = strip_tags($this->con_telefono);
         $this->con_telefono = NM_charset_to_utf8($this->con_telefono);
         $this->con_telefono = str_replace('<', '&lt;', $this->con_telefono);
         $this->con_telefono = str_replace('>', '&gt;', $this->con_telefono);
         $this->Texto_tag .= "<td>" . $this->con_telefono . "</td>\r\n";
   }
   //----- con_correo
   function NM_export_con_correo()
   {
         $this->con_correo = html_entity_decode($this->con_correo, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->con_correo = strip_tags($this->con_correo);
         $this->con_correo = NM_charset_to_utf8($this->con_correo);
         $this->con_correo = str_replace('<', '&lt;', $this->con_correo);
         $this->con_correo = str_replace('>', '&gt;', $this->con_correo);
         $this->Texto_tag .= "<td>" . $this->con_correo . "</td>\r\n";
   }
   //----- con_toma_administrativa
   function NM_export_con_toma_administrativa()
   {
         nmgp_Form_Num_Val($this->look_con_toma_administrativa, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->look_con_toma_administrativa = NM_charset_to_utf8($this->look_con_toma_administrativa);
         $this->look_con_toma_administrativa = str_replace('<', '&lt;', $this->look_con_toma_administrativa);
         $this->look_con_toma_administrativa = str_replace('>', '&gt;', $this->look_con_toma_administrativa);
         $this->Texto_tag .= "<td>" . $this->look_con_toma_administrativa . "</td>\r\n";
   }
   //----- con_toma_operativa
   function NM_export_con_toma_operativa()
   {
         nmgp_Form_Num_Val($this->look_con_toma_operativa, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
         $this->look_con_toma_operativa = NM_charset_to_utf8($this->look_con_toma_operativa);
         $this->look_con_toma_operativa = str_replace('<', '&lt;', $this->look_con_toma_operativa);
         $this->look_con_toma_operativa = str_replace('>', '&gt;', $this->look_con_toma_operativa);
         $this->Texto_tag .= "<td>" . $this->look_con_toma_operativa . "</td>\r\n";
   }

   //----- 
   function grava_arquivo_rtf()
   {
      global $nm_lang, $doc_wrap;
      $this->Rtf_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      $rtf_f       = fopen($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo, "w");
      require_once($this->Ini->path_third      . "/rtf_new/document_generator/cl_xml2driver.php"); 
      $text_ok  =  "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\r\n"; 
      $text_ok .=  "<DOC config_file=\"" . $this->Ini->path_third . "/rtf_new/doc_config.inc\" >\r\n"; 
      $text_ok .=  $this->Texto_tag; 
      $text_ok .=  "</DOC>\r\n"; 
      $xml = new nDOCGEN($text_ok,"RTF"); 
      fwrite($rtf_f, $xml->get_result_file());
      fclose($rtf_f);
   }

   function nm_conv_data_db($dt_in, $form_in, $form_out)
   {
       $dt_out = $dt_in;
       if (strtoupper($form_in) == "DB_FORMAT") {
           if ($dt_out == "null" || $dt_out == "")
           {
               $dt_out = "";
               return $dt_out;
           }
           $form_in = "AAAA-MM-DD";
       }
       if (strtoupper($form_out) == "DB_FORMAT") {
           if (empty($dt_out))
           {
               $dt_out = "null";
               return $dt_out;
           }
           $form_out = "AAAA-MM-DD";
       }
       if (strtoupper($form_out) == "SC_FORMAT_REGION") {
           $this->nm_data->SetaData($dt_in, strtoupper($form_in));
           $prep_out  = (strpos(strtolower($form_in), "dd") !== false) ? "dd" : "";
           $prep_out .= (strpos(strtolower($form_in), "mm") !== false) ? "mm" : "";
           $prep_out .= (strpos(strtolower($form_in), "aa") !== false) ? "aaaa" : "";
           $prep_out .= (strpos(strtolower($form_in), "yy") !== false) ? "aaaa" : "";
           return $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DT", $prep_out));
       }
       else {
           nm_conv_form_data($dt_out, $form_in, $form_out);
           return $dt_out;
       }
   }
   function progress_bar_end()
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
      $path_doc_md5 = md5($this->Ini->path_imag_temp . "/" . $this->Arquivo);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera'][$path_doc_md5][0] = $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera'][$path_doc_md5][1] = $this->Tit_doc;
      $Mens_bar = $this->Ini->Nm_lang['lang_othr_file_msge'];
      if ($_SESSION['scriptcase']['charset'] != "UTF-8") {
          $Mens_bar = sc_convert_encoding($Mens_bar, "UTF-8", $_SESSION['scriptcase']['charset']);
      }
      $this->pb->setProgressbarMessage($Mens_bar);
      $this->pb->setDownloadLink($this->Ini->path_imag_temp . "/" . $this->Arquivo);
      $this->pb->setDownloadMd5($path_doc_md5);
      $this->pb->completed();
   }
   //---- 
   function monta_html()
   {
      global $nm_url_saida, $nm_lang;
      include($this->Ini->path_btn . $this->Ini->Str_btn_grid);
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera']['rtf_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
      $path_doc_md5 = md5($this->Ini->path_imag_temp . "/" . $this->Arquivo);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera'][$path_doc_md5][0] = $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_jap_contratos_cliente_cartera'][$path_doc_md5][1] = $this->Tit_doc;
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
            "http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd">
<HTML<?php echo $_SESSION['scriptcase']['reg_conf']['html_dir'] ?>>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_modificar_contrato'] ?> :: RTF</TITLE>
 <META http-equiv="Content-Type" content="text/html; charset=<?php echo $_SESSION['scriptcase']['charset_html'] ?>" />
<?php
if ($_SESSION['scriptcase']['proc_mobile'])
{
?>
  <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0" />
<?php
}
?>
  <META http-equiv="Expires" content="Fri, Jan 01 1900 00:00:00 GMT"/>
  <META http-equiv="Last-Modified" content="<?php echo gmdate("D, d M Y H:i:s"); ?> GMT"/>
  <META http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate"/>
  <META http-equiv="Cache-Control" content="post-check=0, pre-check=0"/>
  <META http-equiv="Pragma" content="no-cache"/>
 <link rel="shortcut icon" href="../_lib/img/usr__NM__img__NM__favicon.png">
  <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_all ?>_export.css" /> 
  <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_all ?>_export<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
 <?php
 if(isset($this->Ini->str_google_fonts) && !empty($this->Ini->str_google_fonts))
 {
 ?>
    <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->str_google_fonts ?>" />
 <?php
 }
 ?>
  <link rel="stylesheet" type="text/css" href="../_lib/buttons/<?php echo $this->Ini->Str_btn_css ?>" /> 
</HEAD>
<BODY class="scExportPage">
<?php echo $this->Ini->Ajax_result_set ?>
<table style="border-collapse: collapse; border-width: 0; height: 100%; width: 100%"><tr><td style="padding: 0; text-align: center; vertical-align: middle">
 <table class="scExportTable" align="center">
  <tr>
   <td class="scExportTitle" style="height: 25px">RTF</td>
  </tr>
  <tr>
   <td class="scExportLine" style="width: 100%">
    <table style="border-collapse: collapse; border-width: 0; width: 100%"><tr><td class="scExportLineFont" style="padding: 3px 0 0 0" id="idMessage">
    <?php echo $this->Ini->Nm_lang['lang_othr_file_msge'] ?>
    </td><td class="scExportLineFont" style="text-align:right; padding: 3px 0 0 0">
     <?php echo nmButtonOutput($this->arr_buttons, "bexportview", "document.Fview.submit()", "document.Fview.submit()", "idBtnView", "", "", "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
 ?>
     <?php echo nmButtonOutput($this->arr_buttons, "bdownload", "document.Fdown.submit()", "document.Fdown.submit()", "idBtnDown", "", "", "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
 ?>
     <?php echo nmButtonOutput($this->arr_buttons, "bvoltar", "document.F0.submit()", "document.F0.submit()", "idBtnBack", "", "", "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
 ?>
    </td></tr></table>
   </td>
  </tr>
 </table>
</td></tr></table>
<form name="Fview" method="get" action="<?php echo $this->Ini->path_imag_temp . "/" . $this->Arquivo ?>" target="_blank" style="display: none"> 
</form>
<form name="Fdown" method="get" action="grid_jap_contratos_cliente_cartera_download.php" target="_blank" style="display: none"> 
<input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<input type="hidden" name="nm_tit_doc" value="grid_jap_contratos_cliente_cartera"> 
<input type="hidden" name="nm_name_doc" value="<?php echo $path_doc_md5 ?>"> 
</form>
<FORM name="F0" method=post action="./"> 
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="volta_grid"> 
</FORM> 
</BODY>
</HTML>
<?php
   }
   function nm_gera_mask(&$nm_campo, $nm_mask)
   { 
      $trab_campo = $nm_campo;
      $trab_mask  = $nm_mask;
      $tam_campo  = strlen($nm_campo);
      $trab_saida = "";
      $str_highlight_ini = "";
      $str_highlight_fim = "";
      if(substr($nm_campo, 0, 23) == '<div class="highlight">' && substr($nm_campo, -6) == '</div>')
      {
           $str_highlight_ini = substr($nm_campo, 0, 23);
           $str_highlight_fim = substr($nm_campo, -6);

           $trab_campo = substr($nm_campo, 23, -6);
           $tam_campo  = strlen($trab_campo);
      }      $mask_num = false;
      for ($x=0; $x < strlen($trab_mask); $x++)
      {
          if (substr($trab_mask, $x, 1) == "#")
          {
              $mask_num = true;
              break;
          }
      }
      if ($mask_num )
      {
          $ver_duas = explode(";", $trab_mask);
          if (isset($ver_duas[1]) && !empty($ver_duas[1]))
          {
              $cont1 = count(explode("#", $ver_duas[0])) - 1;
              $cont2 = count(explode("#", $ver_duas[1])) - 1;
              if ($tam_campo >= $cont2)
              {
                  $trab_mask = $ver_duas[1];
              }
              else
              {
                  $trab_mask = $ver_duas[0];
              }
          }
          $tam_mask = strlen($trab_mask);
          $xdados = 0;
          for ($x=0; $x < $tam_mask; $x++)
          {
              if (substr($trab_mask, $x, 1) == "#" && $xdados < $tam_campo)
              {
                  $trab_saida .= substr($trab_campo, $xdados, 1);
                  $xdados++;
              }
              elseif ($xdados < $tam_campo)
              {
                  $trab_saida .= substr($trab_mask, $x, 1);
              }
          }
          if ($xdados < $tam_campo)
          {
              $trab_saida .= substr($trab_campo, $xdados);
          }
          $nm_campo = $str_highlight_ini . $trab_saida . $str_highlight_ini;
          return;
      }
      for ($ix = strlen($trab_mask); $ix > 0; $ix--)
      {
           $char_mask = substr($trab_mask, $ix - 1, 1);
           if ($char_mask != "x" && $char_mask != "z")
           {
               $trab_saida = $char_mask . $trab_saida;
           }
           else
           {
               if ($tam_campo != 0)
               {
                   $trab_saida = substr($trab_campo, $tam_campo - 1, 1) . $trab_saida;
                   $tam_campo--;
               }
               else
               {
                   $trab_saida = "0" . $trab_saida;
               }
           }
      }
      if ($tam_campo != 0)
      {
          $trab_saida = substr($trab_campo, 0, $tam_campo) . $trab_saida;
          $trab_mask  = str_repeat("z", $tam_campo) . $trab_mask;
      }
   
      $iz = 0; 
      for ($ix = 0; $ix < strlen($trab_mask); $ix++)
      {
           $char_mask = substr($trab_mask, $ix, 1);
           if ($char_mask != "x" && $char_mask != "z")
           {
               if ($char_mask == "." || $char_mask == ",")
               {
                   $trab_saida = substr($trab_saida, 0, $iz) . substr($trab_saida, $iz + 1);
               }
               else
               {
                   $iz++;
               }
           }
           elseif ($char_mask == "x" || substr($trab_saida, $iz, 1) != "0")
           {
               $ix = strlen($trab_mask) + 1;
           }
           else
           {
               $trab_saida = substr($trab_saida, 0, $iz) . substr($trab_saida, $iz + 1);
           }
      }
      $nm_campo = $str_highlight_ini . $trab_saida . $str_highlight_ini;
   } 
}

?>
