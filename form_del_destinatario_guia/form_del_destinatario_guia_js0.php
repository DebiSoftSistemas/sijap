<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="dg_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['dg_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_destinatario_guia']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_destinatario_guia']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_destinatario_guia"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nuevo_destinatario; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_var_id_guia*scinvar_id_guia*scoutvar_tipo_transporte*scinF*scout"; ?>', '', '_blank', '0', '0', 'blank_nuevo_destinatario');
}
function sc_btn_entre_establecimientos()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_entre_establecimientos(); }, 500);
      return;
    }
    sc_btn_entre_establecimientos_ok();
}
function sc_btn_entre_establecimientos_cancel()
{
}
function sc_btn_entre_establecimientos_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nuevo_destinatario; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_var_id_guia*scinvar_id_guia*scoutvar_tipo_transporte*scinE*scout"; ?>', '', '_blank', '0', '0', 'blank_nuevo_destinatario');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_destinatario_guia']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_destinatario_guia_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_destinatario_guia_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("tipo_sustento=disabled;serie_sustento=disabled;secuencial_sustento=disabled;autorizacion_sustento=disabled;fecha_sustento=disabled", "");
