<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="op_operacion_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['op_operacion_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_fac_seleccion_operacion_transferir"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_seleccion_operacion_transferir']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_seleccion_operacion_transferir']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_seleccion_operacion_transferir']['dashboard_info']['parent_widget'] . "']\")";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_fac_seleccion_operacion_transferir_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_op_tipo_cartera_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.op_tipo_cartera_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_fac_seleccion_operacion_transferir')?>/form_fac_seleccion_operacion_transferir.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_op_tipo_cartera_, "op_tipo_cartera_", dados, false);
}
function recebe_op_tipo_cartera_(string)
{
   document.getElementById('id_lookup_op_tipo_cartera_' + nm_num_campo).innerHTML = string;
}
function lookup_op_propietario_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.op_propietario_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_fac_seleccion_operacion_transferir')?>/form_fac_seleccion_operacion_transferir.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_op_propietario_, "op_propietario_", dados, false);
}
function recebe_op_propietario_(string)
{
   document.getElementById('id_lookup_op_propietario_' + nm_num_campo).innerHTML = string;
}
function lookup_op_estado_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.op_estado_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_fac_seleccion_operacion_transferir')?>/form_fac_seleccion_operacion_transferir.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_op_estado_, "op_estado_", dados, false);
}
function recebe_op_estado_(string)
{
   document.getElementById('id_lookup_op_estado_' + nm_num_campo).innerHTML = string;
}
<?php 
