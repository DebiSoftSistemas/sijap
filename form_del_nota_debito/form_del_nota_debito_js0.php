<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="nd_numero" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['nd_numero']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nota_debito']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nota_debito']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_nota_debito"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_factura; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin05*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_factura');
}
var var_btn_ride_var_id_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['nd_numero']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_mostrar; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin05*scoutvar_id_documento*scin' + var_btn_ride_var_id_documento + '*scoutvar_extension_documento*scinpdf*scout"; ?>', '', '_blank', '0', '0', 'pdf_mostrar');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_nota_debito']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_nota_debito_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_nota_debito_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("establecimiento=disabled;punto_emision=disabled;secuencial=disabled;tipo_libretin=disabled;ambiente=disabled;tipo_doc_sustento=disabled;serie_sustento=disabled;secuencial_sustento=disabled;fecha_sustento=disabled;nombre_cliente=disabled;ruc_cliente=disabled;direccion_cliente=disabled;email_cliente=disabled;nd_subtotal=disabled;nd_subtotal_iva=disabled;nd_subtotal_cero=disabled;nd_subtotal_no_objeto=disabled;nd_subtotal_excento=disabled;nd_valor_ice=disabled;nd_valor_iva=disabled;nd_total=disabled", "");
