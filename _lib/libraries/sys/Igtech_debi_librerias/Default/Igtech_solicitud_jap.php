...
class MYPDF extends TCPDF {
    ...
    function Header(){
        ...
        if($this->grid)
            $this->DrawGrid();
		$vetor_x = 		array(5, 	5, 		5, 			5, 		10);
        $vetor_y = 		array(5, 	27, 	69, 		241, 	75);
        $largura = 		array(200, 	200, 	200, 		200, 	190);
        $altura = 		array(20, 	40, 	170,		50, 	95);
		$raio = 		array(2, 	2, 		2, 			2, 		2);
		$resolucao = 	array(2, 	2, 		2, 			2, 		2);
	...
	function cuerpoContrato(){	
		...
		$y=42;
		$this->SetXY(30,$y);
	    $this->MultiCell(0, 8, $this->w_solicitante, 0, 'L', 0, 0, '', '', true);
		$this->SetXY(150,$y);
		...
		$this->SetFont('helvetica', 'B', 12);
		$this->SetXY(7,70);
	    $this->MultiCell(75, 8, 'CROQUIS DE LA UBICACIÓN', 0, 'L', 0, 0, '', '', true);
		
	   
		$this->SetFont('helvetica', '', 10);
		$x=15;				
		$y=170;
		$w_nota='El croquis debe incluir todas las referencias de edificios y parques cercanos y puede ser hecho a mano alzada y sin escala.';
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_nota."\n", 0, 'J', 0, 2, '' ,'', true);
		
		$y+=10;
		$this->SetXY($x,$y);
		$this->SetTextColor(255,0,0);
	    $this->MultiCell(0, 7,"IMPORTANTE", 0, 'C', 0, 2, '' ,'', true);
		
		$this->SetTextColor(0,0,0);
		$w_texto_importante='- PRESENTE LAS COPIAS ACTUALIZADAS DE LAS ESCRITURAS O SU SIMILAR, (CONCESIÓN,ETC)';
		$y+=5;
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_texto_importante, 0, 'L', 0, 2, '' ,'', true);
		
		$w_texto_importante='- ESTA SOLICITUD ES VALIDA POR UN PERIODO ANUAL COMPLETO ENERO A DICIEMBRE';
		$y+=5;
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_texto_importante, 0, 'L', 0, 2, '' ,'', true);
		
		$w_texto_importante='- EL PRESUPUESTO SERA AJUSTADO SIN PREVIO AVISO';
		$y+=5;
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_texto_importante, 0, 'L', 0, 2, '' ,'', true);
		
		$w_texto_importante='- SIRVASE CONCURRIR A ESTA MISMA VENTANILLA EN 10 DIAS LABORALES';
		$y+=5;
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_texto_importante, 0, 'L', 0, 2, '' ,'', true);
		
		$w_texto_importante='- COPIAS DE LA CEDULA DE CIUDADANIA Y PAPELETA DE VOTACION A COLOR';
		$y+=5;
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_texto_importante, 0, 'L', 0, 2, '' ,'', true);
		
		$w_texto_importante='- AVAL DEL BARRIO AL QUE PERTENECE';
		$y+=5;
		$this->SetXY($x,$y);
	    $this->MultiCell(0, 7, $w_texto_importante, 0, 'L', 0, 2, '' ,'', true);
		...
		$this->SetXY(150,$y);
	    $this->MultiCell(0, 8, $this->w_cedula_solicitante, 0, 'L', 0, 0, '', '', true);
		$w_texto_aviso="Sirvase concurrir a esta misma ventanilla con este comprobante luego de 10 días laborales, no lo extravíe.";
		$this->SetXY(7,-12);
	    $this->MultiCell(0, 8, $w_texto_aviso, 0, 'C', 0, 0, '', '', true);
    }
