<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="rc_contrato_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['rc_contrato_']); ?>">
<input type="hidden" name="rc_producto_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['rc_producto_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_jap_rubro_contrato"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_jap_rubro_contrato_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_rc_estado_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.rc_estado_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_jap_rubro_contrato')?>/form_jap_rubro_contrato.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_rc_estado_, "rc_estado_", dados, false);
}
function recebe_rc_estado_(string)
{
   document.getElementById('id_lookup_rc_estado_' + nm_num_campo).innerHTML = string;
}
<?php 
