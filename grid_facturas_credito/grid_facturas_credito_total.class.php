...
class grid_facturas_credito_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_facturas_credito']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_facturas_credito']['campos_busca']))
      { 
   ...
   {
      global $nm_lang, $pagos;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fac_fecha' => "fac_fecha",'fac_fecha_scgb_19' => "fac_fecha",'fac_cliente' => "fac_cliente");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('fp_id' => 'N','fp_valor' => 'N','fp_valor_pago' => 'N','fp_saldo' => 'N','fac_numero' => 'N','fac_cliente' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(fp_valor), sum(fp_valor_pago), sum(fp_saldo)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2), sum(SC_metric3)#@#cmps_quebras#@# from (";
         $comando .= "select fp_valor as SC_metric1,fp_valor_pago as SC_metric2,fp_saldo as SC_metric3, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['fac_fecha_scgb_19']['reg'] = "N";
      $format_dimensions['fac_fecha_scgb_19']['msk'] = "F";
      $format_dimensions['fac_cliente']['reg'] = "S";
      $format_dimensions['fac_cliente']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "fac_cliente") {
                  $this->Lookup->lookup_sc_free_group_by_fac_cliente($$Cada_dim,  $fac_cliente);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang , $fac_cliente;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fp_valor), sum(fp_valor_pago), sum(fp_saldo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(fp_valor), sum(fp_valor_pago), sum(fp_saldo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_fac_fecha_sc_free_group_by($fac_fecha, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $fac_cliente;
      $tot_fac_fecha = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fp_valor), sum(fp_valor_pago), sum(fp_saldo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['where_pesq']; 
   ...
   //-----  fac_fecha_scgb_19
   function quebra_fac_fecha_scgb_19_sc_free_group_by($fac_fecha_scgb_19, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $fac_cliente;
      $tot_fac_fecha_scgb_19 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fp_valor), sum(fp_valor_pago), sum(fp_saldo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['where_pesq']; 
      } 
      ...
      $tot_fac_fecha_scgb_19[0] = NM_encode_input(sc_strip_script($fac_fecha_scgb_19)) ; 
      $tot_fac_fecha_scgb_19[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fac_fecha_scgb_19[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fac_fecha_scgb_19[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_fac_fecha_scgb_19[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fac_fecha_scgb_19;
   } 

   //-----  fac_cliente
   function quebra_fac_cliente_sc_free_group_by($fac_cliente, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $fac_cliente;
      $tot_fac_cliente = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fp_valor), sum(fp_valor_pago), sum(fp_saldo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_credito']['where_pesq']; 
      } 
      ...
      $tot_fac_cliente[0] = NM_encode_input(sc_strip_script($fac_cliente)) ; 
      $tot_fac_cliente[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fac_cliente[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fac_cliente[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_fac_cliente[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fac_cliente;
   } 
