...
<?php
   ...
   function conectDB()
   {
      ...
      { 
          $this->Db = db_conect_devel($_SESSION['scriptcase']['blank_autorizar_nc_pendientes']['glo_nm_conexao'], $this->root . $this->path_prod, 'SIJAP', 1, $this->force_db_utf8); 
      } 
      else 
      { 
          ob_start();
...
if ($this->rs[0][0]=="SI"){
	$select_sql="SELECT nc_numero,nc_estado 
					FROM del_nota_credito 
	...
	if (false == $this->rs_operacion ) {
		...
		while(!$this->rs_operacion->EOF){
			...
			if ($this->rs_operacion->fields[1]=='N'){
				$documentos_anuladas[]=$this->rs_operacion->fields[0];
			}
		...
		foreach($documentos_anuladas as $documento_anulada){
			$this->anularNotaCredito($documento_anulada,$this->sc_temp_Igtech_RucEmpresa);   
		}
	}
}	
...
//======= =========================
   ...
   if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
       ...
       elseif (is_file($root . $_SESSION['scriptcase']['blank_autorizar_nc_pendientes']['glo_nm_path_imag_temp'] . "/sc_apl_default_SIJAP.txt")) {
           $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['blank_autorizar_nc_pendientes']['glo_nm_path_imag_temp'] . "/sc_apl_default_SIJAP.txt"));
       }
       if (isset($apl_def)) {
           ...
           if (isset($_COOKIE['sc_actual_lang_SIJAP'])) {
               $_SESSION['scriptcase']['blank_autorizar_nc_pendientes']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_SIJAP'];
           }
       }
   }
