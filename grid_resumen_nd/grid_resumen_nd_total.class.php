...
class grid_resumen_nd_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_resumen_nd']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_resumen_nd']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('nd_fecha' => "nd_fecha",'est_codigo' => "est_codigo",'pen_serie' => "pen_serie",'cl_identificacion' => "cl_identificacion",'cl_nombre' => "cl_nombre",'nd_secuencial_docmod' => "nd_secuencial_docmod",'nd_usuario' => "nd_usuario");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('nd_numero' => 'N','nd_subtotal' => 'N','nd_subtotal_iva' => 'N','nd_valor_iva' => 'N','nd_subtotal_cero' => 'N','nd_subtotal_no_objeto' => 'N','nd_subtotal_excento' => 'N','nd_valor_ice' => 'N','nd_total' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(nd_subtotal), sum(nd_valor_iva), sum(nd_total), sum(nd_subtotal_iva), sum(nd_subtotal_cero), sum(nd_subtotal_no_objeto), sum(nd_subtotal_excento), sum(nd_valor_ice)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         ...
         $comando .= "select nd_subtotal as SC_metric1,nd_valor_iva as SC_metric2,nd_total as SC_metric3,nd_subtotal_iva as SC_metric4,nd_subtotal_cero as SC_metric5,nd_subtotal_no_objeto as SC_metric6,nd_subtotal_excento as SC_metric7,nd_valor_ice as SC_metric8, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['nd_fecha']['reg'] = "S";
      $format_dimensions['nd_fecha']['msk'] = "";
      ...
      $format_dimensions['nd_secuencial_docmod']['reg'] = "S";
      $format_dimensions['nd_secuencial_docmod']['msk'] = "";
      $format_dimensions['nd_usuario']['reg'] = "S";
      $format_dimensions['nd_usuario']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "nd_secuencial_docmod") {
              }
              if ($Cada_dim == "nd_usuario") {
                  $this->Lookup->lookup_sc_free_group_by_nd_usuario($$Cada_dim,  $nd_usuario);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nd_subtotal), sum(nd_valor_iva), sum(nd_total), sum(nd_subtotal_iva), sum(nd_subtotal_cero), sum(nd_subtotal_no_objeto), sum(nd_subtotal_excento), sum(nd_valor_ice) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(nd_subtotal), sum(nd_valor_iva), sum(nd_total), sum(nd_subtotal_iva), sum(nd_subtotal_cero), sum(nd_subtotal_no_objeto), sum(nd_subtotal_excento), sum(nd_valor_ice) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][7] = $rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $rt->fields[7] = (string)$rt->fields[7]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][8] = $rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $rt->fields[8] = (string)$rt->fields[8]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['tot_geral'][9] = $rt->fields[8]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['contr_total_geral'] = "OK";
   } 

   //-----  nd_fecha
   function quebra_nd_fecha_sc_free_group_by($nd_fecha, $Where_qb, $Cmp_Name) 
   {
      ...
      $tot_nd_fecha[0] = NM_encode_input(sc_strip_script($nd_fecha)) ; 
      $tot_nd_fecha[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nd_fecha[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_nd_fecha[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_nd_fecha[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_nd_fecha[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_nd_fecha[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_nd_fecha[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_nd_fecha[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_nd_fecha[9] = (string)$rt->fields[8]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nd_fecha;
   } 
   ...
   function quebra_nd_secuencial_docmod_sc_free_group_by($nd_secuencial_docmod, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_nd_secuencial_docmod = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nd_subtotal), sum(nd_valor_iva), sum(nd_total), sum(nd_subtotal_iva), sum(nd_subtotal_cero), sum(nd_subtotal_no_objeto), sum(nd_subtotal_excento), sum(nd_valor_ice) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_nd_secuencial_docmod[0] = NM_encode_input(sc_strip_script($nd_secuencial_docmod));
      }
      else {
          $tot_nd_secuencial_docmod[0] = sc_strip_script($nd_secuencial_docmod) ; 
      }
      $tot_nd_secuencial_docmod[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nd_secuencial_docmod[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_nd_secuencial_docmod[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_nd_secuencial_docmod[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_nd_secuencial_docmod[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_nd_secuencial_docmod[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_nd_secuencial_docmod[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_nd_secuencial_docmod[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_nd_secuencial_docmod[9] = (string)$rt->fields[8]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nd_secuencial_docmod;
   } 

   //-----  nd_usuario
   function quebra_nd_usuario_sc_free_group_by($nd_usuario, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_nd_usuario = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nd_subtotal), sum(nd_valor_iva), sum(nd_total), sum(nd_subtotal_iva), sum(nd_subtotal_cero), sum(nd_subtotal_no_objeto), sum(nd_subtotal_excento), sum(nd_valor_ice) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_nd_usuario[0] = NM_encode_input(sc_strip_script($nd_usuario));
      }
      else {
          $tot_nd_usuario[0] = sc_strip_script($nd_usuario) ; 
      }
      $tot_nd_usuario[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nd_usuario[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_nd_usuario[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_nd_usuario[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_nd_usuario[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_nd_usuario[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_nd_usuario[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_nd_usuario[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_nd_usuario[9] = (string)$rt->fields[8]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nd_usuario;
   } 
