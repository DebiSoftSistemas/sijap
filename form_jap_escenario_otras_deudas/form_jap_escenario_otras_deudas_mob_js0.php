<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="esc_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['esc_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_escenario_otras_deudas_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_escenario_otras_deudas_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_jap_escenario_otras_deudas_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var Crtl_btn_simular = false;
function sc_btn_simular()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_simular(); }, 500);
      return;
    }
    if (Crtl_btn_simular) {return;}
    sc_btn_simular_ok();
}
function sc_btn_simular_cancel()
{
}
function sc_btn_simular_ok()
{
    Crtl_btn_simular = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?simular?@?";
    document.F1.action = "form_jap_escenario_otras_deudas_mob.php";
    document.F1.target = "_self";
    document.F1.submit();
}
var Crtl_btn_emitir = false;
function sc_btn_emitir()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_emitir(); }, 500);
      return;
    }
    if (Crtl_btn_emitir) {return;}
    sc_btn_emitir_ok();
}
function sc_btn_emitir_cancel()
{
}
function sc_btn_emitir_ok()
{
    Crtl_btn_emitir = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?emitir?@?";
    document.F1.action = "form_jap_escenario_otras_deudas_mob.php";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_agregar_var_escenario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['esc_id']); ?>";
function sc_btn_agregar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_agregar(); }, 500);
      return;
    }
    sc_btn_agregar_ok();
}
function sc_btn_agregar_cancel()
{
}
function sc_btn_agregar_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_jap__seleccionar_contrato_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_escenario*scin' + var_btn_agregar_var_escenario + '*scout"; ?>', 'inicio', '_self', '0', '0', 'grid_jap__seleccionar_contrato');
}
var Crtl_btn_agregar_todos = false;
function sc_btn_agregar_todos()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_agregar_todos(); }, 500);
      return;
    }
    if (Crtl_btn_agregar_todos) {return;}
    sc_btn_agregar_todos_ok();
}
function sc_btn_agregar_todos_cancel()
{
}
function sc_btn_agregar_todos_ok()
{
    Crtl_btn_agregar_todos = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?agregar_todos?@?";
    document.F1.action = "form_jap_escenario_otras_deudas_mob.php";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_agregar_x_ruta_var_escenario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['esc_id']); ?>";
function sc_btn_agregar_x_ruta()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_agregar_x_ruta(); }, 500);
      return;
    }
    sc_btn_agregar_x_ruta_ok();
}
function sc_btn_agregar_x_ruta_cancel()
{
}
function sc_btn_agregar_x_ruta_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_control_contrato_ruta_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_escenario*scin' + var_btn_agregar_x_ruta_var_escenario + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'control_contrato_ruta');
}
var var_btn_importar_var_escenario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['esc_id']); ?>";
function sc_btn_importar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_importar(); }, 500);
      return;
    }
    sc_btn_importar_ok();
}
function sc_btn_importar_cancel()
{
}
function sc_btn_importar_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_control_carga_contratos_deudas_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_escenario*scin' + var_btn_importar_var_escenario + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'control_carga_contratos_deudas');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_jap_escenario_otras_deudas_mob']['where_detal']))
  {
?>  
...
function lookup_esc_estado(campo)
{
   dados  = document.F1.esc_estado.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_jap_escenario_otras_deudas_mob')?>/form_jap_escenario_otras_deudas_mob.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_esc_estado, "esc_estado", dados, false);
}
function recebe_esc_estado(string)
{
   document.getElementById('id_lookup_esc_estado').innerHTML = string;
}
<?php 
