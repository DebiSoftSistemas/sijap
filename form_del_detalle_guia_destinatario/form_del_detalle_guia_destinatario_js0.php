<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="dgd_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['dgd_id_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_detalle_guia_destinatario"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_eliminar_todas = false;
function sc_btn_eliminar_todas()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_eliminar_todas(); }, 500);
      return;
    }
    if (Crtl_btn_eliminar_todas) {return;}
    sc_btn_eliminar_todas_ok();
}
function sc_btn_eliminar_todas_cancel()
{
}
function sc_btn_eliminar_todas_ok()
{
    Crtl_btn_eliminar_todas = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?eliminar_todas?@?";
    document.F1.action = "./";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_del_detalle_guia_destinatario_submit_form(y, z);
