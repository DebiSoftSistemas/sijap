...
class grid_v_sis_reporte_ventas_comisionistas_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('periodo' => "periodo",'mes' => "mes",'com_ruc' => "com_ruc",'com_razon_social' => "com_razon_social");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('fac_base_iva' => 'N','fac_total_iva' => 'N','fac_total' => 'N','mes' => 'N','periodo' => 'N','valor_comision' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(fac_base_iva), sum(fac_total_iva), sum(fac_total), sum(valor_comision)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2), sum(SC_metric3), sum(SC_metric4)#@#cmps_quebras#@# from (";
         $comando .= "select fac_base_iva as SC_metric1,fac_total_iva as SC_metric2,fac_total as SC_metric3,valor_comision as SC_metric4, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['com_ruc']['reg'] = "S";
      $format_dimensions['com_ruc']['msk'] = "";
      $format_dimensions['com_razon_social']['reg'] = "S";
      $format_dimensions['com_razon_social']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
   ...
   function quebra_geral_sc_free_group_by($res_limit=false, $res_export=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fac_base_iva), sum(fac_total_iva), sum(fac_total), sum(valor_comision) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(fac_base_iva), sum(fac_total_iva), sum(fac_total), sum(valor_comision) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['tot_geral'][5] = $rt->fields[4]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_periodo_sc_free_group_by($periodo, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_periodo[5] = (string)$rt->fields[4]; 
      $rt->Close(); 
   ...
   //-----  com_ruc
   function quebra_com_ruc_sc_free_group_by($com_ruc, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $mes;
      $tot_com_ruc = array() ;  
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_com_ruc[0] = NM_encode_input(sc_strip_script($com_ruc));
      }
      else {
          $tot_com_ruc[0] = sc_strip_script($com_ruc) ; 
      }
      $tot_com_ruc[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_com_ruc[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_com_ruc[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_com_ruc[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_com_ruc[5] = (string)$rt->fields[4]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_com_ruc;
   } 

   //-----  com_razon_social
   function quebra_com_razon_social_sc_free_group_by($com_razon_social, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $mes;
      $tot_com_razon_social = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fac_base_iva), sum(fac_total_iva), sum(fac_total), sum(valor_comision) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_sis_reporte_ventas_comisionistas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_com_razon_social[0] = NM_encode_input(sc_strip_script($com_razon_social));
      }
      else {
          $tot_com_razon_social[0] = sc_strip_script($com_razon_social) ; 
      }
      $tot_com_razon_social[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_com_razon_social[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_com_razon_social[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_com_razon_social[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_com_razon_social[5] = (string)$rt->fields[4]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_com_razon_social;
   } 
