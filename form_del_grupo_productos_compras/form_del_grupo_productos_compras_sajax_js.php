  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_del_grupo_productos_compras" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_del_grupo_productos_compras_form();
	}
...
?>
  ...
  // ---------- Validate gpc_id_
  function do_ajax_form_del_grupo_productos_compras_validate_gpc_id_(iNumLinha)
  {
    var nomeCampo_gpc_id_ = "gpc_id_" + iNumLinha;
    var var_gpc_id_ = scAjaxGetFieldSelect(nomeCampo_gpc_id_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_del_grupo_productos_compras_validate_gpc_id_(var_gpc_id_, iNumLinha, var_script_case_init, do_ajax_form_del_grupo_productos_compras_validate_gpc_id__cb);
  } // do_ajax_form_del_grupo_productos_compras_validate_gpc_id_

  function do_ajax_form_del_grupo_productos_compras_validate_gpc_id__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "gpc_id_" + iLineNumber;
    }
    else
    {
      sFieldValid = "gpc_id_";
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "gpc_id_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "gpc_id_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_del_grupo_productos_compras_validate_gpc_id__cb

  // ---------- Validate gpc_empresa_
  function do_ajax_form_del_grupo_productos_compras_validate_gpc_empresa_(iNumLinha)
  {
    var nomeCampo_gpc_empresa_ = "gpc_empresa_" + iNumLinha;
    var var_gpc_empresa_ = scAjaxGetFieldText(nomeCampo_gpc_empresa_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_del_grupo_productos_compras_validate_gpc_empresa_(var_gpc_empresa_, iNumLinha, var_script_case_init, do_ajax_form_del_grupo_productos_compras_validate_gpc_empresa__cb);
  } // do_ajax_form_del_grupo_productos_compras_validate_gpc_empresa_

  function do_ajax_form_del_grupo_productos_compras_validate_gpc_empresa__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "gpc_empresa_" + iLineNumber;
    }
    else
    {
      sFieldValid = "gpc_empresa_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "gpc_empresa_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "gpc_empresa_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_del_grupo_productos_compras_validate_gpc_empresa__cb

  // ---------- Validate gpc_impuesto_
  function do_ajax_form_del_grupo_productos_compras_validate_gpc_impuesto_(iNumLinha)
  {
    var nomeCampo_gpc_impuesto_ = "gpc_impuesto_" + iNumLinha;
    var var_gpc_impuesto_ = scAjaxGetFieldSelect(nomeCampo_gpc_impuesto_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_del_grupo_productos_compras_validate_gpc_impuesto_(var_gpc_impuesto_, iNumLinha, var_script_case_init, do_ajax_form_del_grupo_productos_compras_validate_gpc_impuesto__cb);
  } // do_ajax_form_del_grupo_productos_compras_validate_gpc_impuesto_

  function do_ajax_form_del_grupo_productos_compras_validate_gpc_impuesto__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "gpc_impuesto_" + iLineNumber;
    }
    else
    {
      sFieldValid = "gpc_impuesto_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "gpc_impuesto_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "gpc_impuesto_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_del_grupo_productos_compras_validate_gpc_impuesto__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_del_grupo_productos_compras_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_del_grupo_productos_compras_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_del_grupo_productos_compras_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_del_grupo_productos_compras_add_new_line_cb);
  } // do_ajax_form_del_grupo_productos_compras_add_new_line

  function do_ajax_form_del_grupo_productos_compras_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_del_grupo_productos_compras_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_del_grupo_productos_compras_backup_line(iNumLinha)
  {
    var var_gpc_id_ = scAjaxGetFieldSelect("gpc_id_" + iNumLinha);
    var var_gpc_empresa_ = scAjaxGetFieldText("gpc_empresa_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_del_grupo_productos_compras_backup_line(var_gpc_id_, var_gpc_empresa_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_del_grupo_productos_compras_backup_line_cb);
  } // do_ajax_form_del_grupo_productos_compras_backup_line

  function do_ajax_form_del_grupo_productos_compras_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_del_grupo_productos_compras_backup_line_cb

  function do_ajax_form_del_grupo_productos_compras_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_del_grupo_productos_compras_cancel_insert

  function do_ajax_form_del_grupo_productos_compras_cancel_update(iSeqVert)
  {
    do_ajax_form_del_grupo_productos_compras_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_del_grupo_productos_compras_cancel_update

  function do_ajax_form_del_grupo_productos_compras_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_del_grupo_productos_compras_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_del_grupo_productos_compras_table_refresh()
  {
    var var_gpc_id_ = document.F2.gpc_id_.value;
    var var_gpc_empresa_ = document.F2.gpc_empresa_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_del_grupo_productos_compras_table_refresh(var_gpc_id_, var_gpc_empresa_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_del_grupo_productos_compras_table_refresh_cb);
  } //  do_ajax_form_del_grupo_productos_compras_table_refresh

  function do_ajax_form_del_grupo_productos_compras_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.gpc_id_.value = scAjaxGetKeyValue("gpc_id_");
    document.F2.gpc_empresa_.value = scAjaxGetKeyValue("gpc_empresa_");
    ...
    scAjaxAlert(do_ajax_form_del_grupo_productos_compras_table_refresh_cb_after_alert);
  } // do_ajax_form_del_grupo_productos_compras_table_refresh_cb
  function do_ajax_form_del_grupo_productos_compras_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_del_grupo_productos_compras_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_del_grupo_productos_compras_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_del_grupo_productos_compras_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    var var_gpc_id_ = scAjaxGetFieldSelect("gpc_id_" + iNumLinha);
    var var_gpc_empresa_ = scAjaxGetFieldText("gpc_empresa_" + iNumLinha);
    var var_gpc_impuesto_ = scAjaxGetFieldSelect("gpc_impuesto_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_del_grupo_productos_compras_submit_form(var_gpc_id_, var_gpc_empresa_, var_gpc_impuesto_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_del_grupo_productos_compras_submit_form_cb);
  } // do_ajax_form_del_grupo_productos_compras_submit_form

  function do_ajax_form_del_grupo_productos_compras_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("gpc_id_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("gpc_empresa_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("gpc_impuesto_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_del_grupo_productos_compras_submit_form_cb_after_alert);
  } // do_ajax_form_del_grupo_productos_compras_submit_form_cb
  function do_ajax_form_del_grupo_productos_compras_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_del_grupo_productos_compras_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_del_grupo_productos_compras_navigate_form()
  {
    perform_ajax_form_del_grupo_productos_compras_navigate_form();
  }

  function perform_ajax_form_del_grupo_productos_compras_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_del_grupo_productos_compras_cancel_insert(sc_form_del_grupo_productos_compras_get_last_line_number());
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    {
      scAjaxHideErrorDisplay("gpc_id_" + iNavForm);
      scAjaxHideErrorDisplay("gpc_empresa_" + iNavForm);
      scAjaxHideErrorDisplay("gpc_impuesto_" + iNavForm);
    }
    var var_gpc_id_ = document.F2.gpc_id_.value;
    var var_gpc_empresa_ = document.F2.gpc_empresa_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_del_grupo_productos_compras_navigate_form(var_gpc_id_, var_gpc_empresa_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_del_grupo_productos_compras_navigate_form_cb);
  } // do_ajax_form_del_grupo_productos_compras_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_del_grupo_productos_compras']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_del_grupo_productos_compras_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.gpc_id_.value = scAjaxGetKeyValue("gpc_id_" + var_last_index);
    document.F2.gpc_empresa_.value = scAjaxGetKeyValue("gpc_empresa_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_del_grupo_productos_compras_navigate_form_cb_after_alert);
  } // do_ajax_form_del_grupo_productos_compras_navigate_form_cb
  function do_ajax_form_del_grupo_productos_compras_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_del_grupo_productos_compras_navigate_form_cb_after_alert

    function sc_form_del_grupo_productos_compras_get_last_line_number()
    {
    ...
    } //sc_form_del_grupo_productos_compras_get_last_line_number

  function sc_hide_form_del_grupo_productos_compras_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "gpc_id_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "gpc_empresa_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "gpc_impuesto_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["gpc_id_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_id'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["gpc_empresa_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_empresa'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["gpc_impuesto_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_impuesto'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["gpc_id_"][iNumLinha] = "gpc_id_" + iNumLinha;
    ajax_field_mult["gpc_empresa_"][iNumLinha] = "gpc_empresa_" + iNumLinha;
    ajax_field_mult["gpc_impuesto_"][iNumLinha] = "gpc_impuesto_" + iNumLinha;
    ajax_field_id["gpc_id_" + iNumLinha] = new Array("hidden_field_label_gpc_id_", "hidden_field_data_gpc_id_" + iNumLinha);
    ajax_field_id["gpc_impuesto_" + iNumLinha] = new Array("hidden_field_label_gpc_impuesto_", "hidden_field_data_gpc_impuesto_" + iNumLinha);
    ajax_error_count["gpc_id_" + iNumLinha] = "off";
    ajax_error_count["gpc_empresa_" + iNumLinha] = "off";
    ajax_error_count["gpc_impuesto_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["gpc_id_" + iNumLinha] = "on";
    ajax_read_only["gpc_empresa_" + iNumLinha] = "on";
    ajax_read_only["gpc_impuesto_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["gpc_id_" + iNumLinha] = "on";
    ajax_read_only["gpc_empresa_" + iNumLinha] = "on";
    ajax_read_only["gpc_impuesto_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["gpc_id_" + iNumLinha] = null;
    ajax_error_list["gpc_empresa_" + iNumLinha] = null;
    ajax_error_list["gpc_impuesto_" + iNumLinha] = null;
  }
  ...
  var ajax_field_mult = {
    "gpc_id_": new Array(),
    "gpc_empresa_": new Array(),
    "gpc_impuesto_": new Array()
  };
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["gpc_id_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["gpc_id_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["gpc_id_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["gpc_id_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["gpc_id_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["gpc_empresa_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["gpc_empresa_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["gpc_empresa_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["gpc_empresa_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["gpc_empresa_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["gpc_impuesto_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["gpc_impuesto_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["gpc_impuesto_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["gpc_impuesto_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["gpc_impuesto_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['gpc_id_'])) ? $this->nmgp_cmp_readonly['gpc_id_'] : 'on';
?>
    scAjaxFieldRead("gpc_id_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['gpc_empresa_'])) ? $this->nmgp_cmp_readonly['gpc_empresa_'] : 'on';
?>
    scAjaxFieldRead("gpc_empresa_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['gpc_impuesto_'])) ? $this->nmgp_cmp_readonly['gpc_impuesto_'] : 'off';
?>
    scAjaxFieldRead("gpc_impuesto_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("gpc_id_" + iSeq, "on");
    scAjaxFieldRead("gpc_empresa_" + iSeq, "on");
    scAjaxFieldRead("gpc_impuesto_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["gpc_id_" + iRow] || "on" == ajax_error_count["gpc_empresa_" + iRow] || "on" == ajax_error_count["gpc_impuesto_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_gpc_id_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_gpc_empresa_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_gpc_impuesto_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["gpc_id_" + iRow] && "off" == ajax_error_count["gpc_empresa_" + iRow] && "off" == ajax_error_count["gpc_impuesto_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["gpc_id_" + iRow] = "off";
        ajax_error_count["gpc_empresa_" + iRow] = "off";
        ajax_error_count["gpc_impuesto_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_gpc_id_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_gpc_empresa_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_gpc_impuesto_" + iRow).removeClass("scFormErrorLine");
    }
  }
