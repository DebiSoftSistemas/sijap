...
function scEventControl_init(iSeqRow) {
  scEventControl_data["tl_codigo_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["tl_descripcion_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["tl_codigo_" + iSeqRow] && scEventControl_data["tl_codigo_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["tl_codigo_" + iSeqRow] && scEventControl_data["tl_codigo_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["tl_descripcion_" + iSeqRow] && scEventControl_data["tl_descripcion_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["tl_descripcion_" + iSeqRow] && scEventControl_data["tl_descripcion_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_tl_codigo_' + iSeqRow).bind('blur', function() { sc_form_sri_tipo_libretin_tl_codigo__onblur(this, iSeqRow) })
                                        .bind('change', function() { sc_form_sri_tipo_libretin_tl_codigo__onchange(this, iSeqRow) })
                                        .bind('focus', function() { sc_form_sri_tipo_libretin_tl_codigo__onfocus(this, iSeqRow) });
  $('#id_sc_field_tl_descripcion_' + iSeqRow).bind('blur', function() { sc_form_sri_tipo_libretin_tl_descripcion__onblur(this, iSeqRow) })
                                             .bind('change', function() { sc_form_sri_tipo_libretin_tl_descripcion__onchange(this, iSeqRow) })
                                             .bind('focus', function() { sc_form_sri_tipo_libretin_tl_descripcion__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_sri_tipo_libretin_tl_codigo__onblur(oThis, iSeqRow) {
  do_ajax_form_sri_tipo_libretin_validate_tl_codigo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_sri_tipo_libretin_tl_codigo__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_sri_tipo_libretin_tl_codigo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_sri_tipo_libretin_tl_descripcion__onblur(oThis, iSeqRow) {
  do_ajax_form_sri_tipo_libretin_validate_tl_descripcion_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_sri_tipo_libretin_tl_descripcion__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_sri_tipo_libretin_tl_descripcion__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("tl_codigo_", "", status);
	displayChange_field("tl_descripcion_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_tl_codigo_(row, status);
	displayChange_field_tl_descripcion_(row, status);
}
...
function displayChange_field(field, row, status) {
	...
	if ("tl_descripcion_" == field) {
		displayChange_field_tl_descripcion_(row, status);
	}
}

function displayChange_field_tl_codigo_(row, status) {
    var fieldId;
}

function displayChange_field_tl_descripcion_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_sri_tipo_libretin_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_sri_tipo_libretin')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
