<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="gr_numero" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['gr_numero']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_guia_remision']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_guia_remision']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_guia_remision"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nueva_factura; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin06*scout"; ?>', '', '_self', '0', '0', 'blank_nueva_factura');
}
var var_btn_Imprimir_var_tipo_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['gr_tipo_comprobante']); ?>";
var var_btn_Imprimir_var_id_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['gr_numero']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_mostrar; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_tipo_documento*scin' + var_btn_Imprimir_var_tipo_documento + '*scoutvar_id_documento*scin' + var_btn_Imprimir_var_id_documento + '*scoutvar_extension_documento*scinpdf*scout"; ?>', '', '_blank', '0', '0', 'pdf_mostrar');
}
var var_btn_desde_factura_var_id_guia = "<?php echo $this->form_encode_input($this->nmgp_dados_form['gr_numero']); ?>";
function sc_btn_desde_factura()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_desde_factura(); }, 500);
      return;
    }
    sc_btn_desde_factura_ok();
}
function sc_btn_desde_factura_cancel()
{
}
function sc_btn_desde_factura_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nuevo_destinatario; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_id_guia*scin' + var_btn_desde_factura_var_id_guia + '*scoutvar_tipo_transporte*scinF*scout"; ?>', '', '_self', '0', '0', 'blank_nuevo_destinatario');
}
var var_btn_entre_establecimiento_var_id_guia = "<?php echo $this->form_encode_input($this->nmgp_dados_form['gr_numero']); ?>";
function sc_btn_entre_establecimiento()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_entre_establecimiento(); }, 500);
      return;
    }
    sc_btn_entre_establecimiento_ok();
}
function sc_btn_entre_establecimiento_cancel()
{
}
function sc_btn_entre_establecimiento_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_nuevo_destinatario; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_id_guia*scin' + var_btn_entre_establecimiento_var_id_guia + '*scoutvar_tipo_transporte*scinE*scout"; ?>', '', '_self', '0', '0', 'blank_nuevo_destinatario');
}
...
var var_btn_baucher_var_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['gr_numero']); ?>";
function sc_btn_baucher()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_baucher(); }, 500);
      return;
    }
    sc_btn_baucher_ok();
}
function sc_btn_baucher_cancel()
{
}
function sc_btn_baucher_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_baucher_guia; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_documento*scin' + var_btn_baucher_var_documento + '*scout"; ?>', '', '_blank', '0', '0', 'pdf_baucher_guia');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_guia_remision']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_guia_remision_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_guia_remision_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("establecimiento=disabled;punto_emision=disabled;secuencial=disabled;tipo_libretin=disabled;ambiente=disabled;ruc_transportista=disabled;nombre_transportista=disabled;placa_transportista=disabled", "");
