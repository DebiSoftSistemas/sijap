  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_sri_comprobante_sustento" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_sri_comprobante_sustento_form();
	}
...
?>
  ...
  // ---------- Validate cs_id_
  function do_ajax_form_sri_comprobante_sustento_validate_cs_id_(iNumLinha)
  {
    var nomeCampo_cs_id_ = "cs_id_" + iNumLinha;
    var var_cs_id_ = scAjaxGetFieldHidden(nomeCampo_cs_id_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_sri_comprobante_sustento_validate_cs_id_(var_cs_id_, iNumLinha, var_script_case_init, do_ajax_form_sri_comprobante_sustento_validate_cs_id__cb);
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_id_

  function do_ajax_form_sri_comprobante_sustento_validate_cs_id__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "cs_id_" + iLineNumber;
    }
    else
    {
      sFieldValid = "cs_id_";
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "cs_id_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "cs_id_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_id__cb

  // ---------- Validate cs_sustento_
  function do_ajax_form_sri_comprobante_sustento_validate_cs_sustento_(iNumLinha)
  {
    var nomeCampo_cs_sustento_ = "cs_sustento_" + iNumLinha;
    var var_cs_sustento_ = scAjaxGetFieldText(nomeCampo_cs_sustento_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_sri_comprobante_sustento_validate_cs_sustento_(var_cs_sustento_, iNumLinha, var_script_case_init, do_ajax_form_sri_comprobante_sustento_validate_cs_sustento__cb);
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_sustento_

  function do_ajax_form_sri_comprobante_sustento_validate_cs_sustento__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "cs_sustento_" + iLineNumber;
    }
    else
    {
      sFieldValid = "cs_sustento_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "cs_sustento_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "cs_sustento_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_sustento__cb

  // ---------- Validate cs_comprobante_
  function do_ajax_form_sri_comprobante_sustento_validate_cs_comprobante_(iNumLinha)
  {
    var nomeCampo_cs_comprobante_ = "cs_comprobante_" + iNumLinha;
    var var_cs_comprobante_ = scAjaxGetFieldSelect(nomeCampo_cs_comprobante_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_sri_comprobante_sustento_validate_cs_comprobante_(var_cs_comprobante_, iNumLinha, var_script_case_init, do_ajax_form_sri_comprobante_sustento_validate_cs_comprobante__cb);
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_comprobante_

  function do_ajax_form_sri_comprobante_sustento_validate_cs_comprobante__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "cs_comprobante_" + iLineNumber;
    }
    else
    {
      sFieldValid = "cs_comprobante_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "cs_comprobante_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "cs_comprobante_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_comprobante__cb

  // ---------- Validate cs_estado_
  function do_ajax_form_sri_comprobante_sustento_validate_cs_estado_(iNumLinha)
  {
    var nomeCampo_cs_estado_ = "cs_estado_" + iNumLinha;
    var var_cs_estado_ = scAjaxGetFieldSelect(nomeCampo_cs_estado_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_sri_comprobante_sustento_validate_cs_estado_(var_cs_estado_, iNumLinha, var_script_case_init, do_ajax_form_sri_comprobante_sustento_validate_cs_estado__cb);
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_estado_

  function do_ajax_form_sri_comprobante_sustento_validate_cs_estado__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "cs_estado_" + iLineNumber;
    }
    else
    {
      sFieldValid = "cs_estado_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "cs_estado_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "cs_estado_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_sri_comprobante_sustento_validate_cs_estado__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_sri_comprobante_sustento_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_sri_comprobante_sustento_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_sri_comprobante_sustento_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_sri_comprobante_sustento_add_new_line_cb);
  } // do_ajax_form_sri_comprobante_sustento_add_new_line

  function do_ajax_form_sri_comprobante_sustento_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_sri_comprobante_sustento_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_sri_comprobante_sustento_backup_line(iNumLinha)
  {
    var var_cs_id_ = scAjaxGetFieldHidden("cs_id_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_sri_comprobante_sustento_backup_line(var_cs_id_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_sri_comprobante_sustento_backup_line_cb);
  } // do_ajax_form_sri_comprobante_sustento_backup_line

  function do_ajax_form_sri_comprobante_sustento_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_sri_comprobante_sustento_backup_line_cb

  function do_ajax_form_sri_comprobante_sustento_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_sri_comprobante_sustento_cancel_insert

  function do_ajax_form_sri_comprobante_sustento_cancel_update(iSeqVert)
  {
    do_ajax_form_sri_comprobante_sustento_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_sri_comprobante_sustento_cancel_update

  function do_ajax_form_sri_comprobante_sustento_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_sri_comprobante_sustento_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_sri_comprobante_sustento_table_refresh()
  {
    var var_cs_id_ = document.F2.cs_id_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_sri_comprobante_sustento_table_refresh(var_cs_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_sri_comprobante_sustento_table_refresh_cb);
  } //  do_ajax_form_sri_comprobante_sustento_table_refresh

  function do_ajax_form_sri_comprobante_sustento_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.cs_id_.value = scAjaxGetKeyValue("cs_id_");
    ...
    scAjaxAlert(do_ajax_form_sri_comprobante_sustento_table_refresh_cb_after_alert);
  } // do_ajax_form_sri_comprobante_sustento_table_refresh_cb
  function do_ajax_form_sri_comprobante_sustento_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_sri_comprobante_sustento_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_sri_comprobante_sustento_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_sri_comprobante_sustento_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    scAjaxHideMessage();
    var var_cs_id_ = scAjaxGetFieldHidden("cs_id_" + iNumLinha);
    var var_cs_sustento_ = scAjaxGetFieldText("cs_sustento_" + iNumLinha);
    var var_cs_comprobante_ = scAjaxGetFieldSelect("cs_comprobante_" + iNumLinha);
    var var_cs_estado_ = scAjaxGetFieldSelect("cs_estado_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_sri_comprobante_sustento_submit_form(var_cs_id_, var_cs_sustento_, var_cs_comprobante_, var_cs_estado_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_sri_comprobante_sustento_submit_form_cb);
  } // do_ajax_form_sri_comprobante_sustento_submit_form

  function do_ajax_form_sri_comprobante_sustento_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("cs_id_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("cs_sustento_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("cs_comprobante_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("cs_estado_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_sri_comprobante_sustento_submit_form_cb_after_alert);
  } // do_ajax_form_sri_comprobante_sustento_submit_form_cb
  function do_ajax_form_sri_comprobante_sustento_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_sri_comprobante_sustento_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_sri_comprobante_sustento_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_sri_comprobante_sustento_navigate_form, function() {});
    } else {
      perform_ajax_form_sri_comprobante_sustento_navigate_form();
    }
  }

  function perform_ajax_form_sri_comprobante_sustento_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_sri_comprobante_sustento_cancel_insert(sc_form_sri_comprobante_sustento_get_last_line_number());
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    {
      scAjaxHideErrorDisplay("cs_id_" + iNavForm);
      scAjaxHideErrorDisplay("cs_sustento_" + iNavForm);
      scAjaxHideErrorDisplay("cs_comprobante_" + iNavForm);
      scAjaxHideErrorDisplay("cs_estado_" + iNavForm);
    }
    var var_cs_id_ = document.F2.cs_id_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_sri_comprobante_sustento_navigate_form(var_cs_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_sri_comprobante_sustento_navigate_form_cb);
  } // do_ajax_form_sri_comprobante_sustento_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_sri_comprobante_sustento']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_sri_comprobante_sustento_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.cs_id_.value = scAjaxGetKeyValue("cs_id_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_sri_comprobante_sustento_navigate_form_cb_after_alert);
  } // do_ajax_form_sri_comprobante_sustento_navigate_form_cb
  function do_ajax_form_sri_comprobante_sustento_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_sri_comprobante_sustento_navigate_form_cb_after_alert

    function sc_form_sri_comprobante_sustento_get_last_line_number()
    {
    ...
    } //sc_form_sri_comprobante_sustento_get_last_line_number

  function sc_hide_form_sri_comprobante_sustento_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "cs_id_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "cs_sustento_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "cs_comprobante_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "cs_estado_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["cs_id_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_id'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["cs_sustento_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_sustento_tributario'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["cs_comprobante_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_tipo_comprobante'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["cs_estado_" + iNumLinha] = {"label": "<?php echo $this->Ini->Nm_lang['lang_estado'] ?><?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["cs_id_"][iNumLinha] = "cs_id_" + iNumLinha;
    ajax_field_mult["cs_sustento_"][iNumLinha] = "cs_sustento_" + iNumLinha;
    ajax_field_mult["cs_comprobante_"][iNumLinha] = "cs_comprobante_" + iNumLinha;
    ajax_field_mult["cs_estado_"][iNumLinha] = "cs_estado_" + iNumLinha;
    ajax_field_id["cs_comprobante_" + iNumLinha] = new Array("hidden_field_label_cs_comprobante_", "hidden_field_data_cs_comprobante_" + iNumLinha);
    ajax_field_id["cs_estado_" + iNumLinha] = new Array("hidden_field_label_cs_estado_", "hidden_field_data_cs_estado_" + iNumLinha);
    ajax_error_count["cs_id_" + iNumLinha] = "off";
    ajax_error_count["cs_sustento_" + iNumLinha] = "off";
    ajax_error_count["cs_comprobante_" + iNumLinha] = "off";
    ajax_error_count["cs_estado_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["cs_id_" + iNumLinha] = "on";
    ajax_read_only["cs_sustento_" + iNumLinha] = "off";
    ajax_read_only["cs_comprobante_" + iNumLinha] = "off";
    ajax_read_only["cs_estado_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["cs_id_" + iNumLinha] = "on";
    ajax_read_only["cs_sustento_" + iNumLinha] = "on";
    ajax_read_only["cs_comprobante_" + iNumLinha] = "on";
    ajax_read_only["cs_estado_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["cs_id_" + iNumLinha] = null;
    ajax_error_list["cs_sustento_" + iNumLinha] = null;
    ajax_error_list["cs_comprobante_" + iNumLinha] = null;
    ajax_error_list["cs_estado_" + iNumLinha] = null;
  }
  ...
  var ajax_field_mult = {
    "cs_id_": new Array(),
    "cs_sustento_": new Array(),
    "cs_comprobante_": new Array(),
    "cs_estado_": new Array()
  };
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["cs_id_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["cs_id_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["cs_id_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["cs_id_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["cs_id_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["cs_sustento_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["cs_sustento_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["cs_sustento_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["cs_sustento_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["cs_sustento_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["cs_comprobante_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["cs_comprobante_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["cs_comprobante_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["cs_comprobante_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["cs_comprobante_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["cs_estado_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["cs_estado_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["cs_estado_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["cs_estado_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["cs_estado_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['cs_id_'])) ? $this->nmgp_cmp_readonly['cs_id_'] : 'on';
?>
    scAjaxFieldRead("cs_id_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['cs_sustento_'])) ? $this->nmgp_cmp_readonly['cs_sustento_'] : 'off';
?>
    scAjaxFieldRead("cs_sustento_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['cs_comprobante_'])) ? $this->nmgp_cmp_readonly['cs_comprobante_'] : 'off';
?>
    scAjaxFieldRead("cs_comprobante_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['cs_estado_'])) ? $this->nmgp_cmp_readonly['cs_estado_'] : 'off';
?>
    scAjaxFieldRead("cs_estado_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("cs_id_" + iSeq, "on");
    scAjaxFieldRead("cs_sustento_" + iSeq, "on");
    scAjaxFieldRead("cs_comprobante_" + iSeq, "on");
    scAjaxFieldRead("cs_estado_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["cs_id_" + iRow] || "on" == ajax_error_count["cs_sustento_" + iRow] || "on" == ajax_error_count["cs_comprobante_" + iRow] || "on" == ajax_error_count["cs_estado_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_cs_id_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_cs_sustento_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_cs_comprobante_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_cs_estado_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["cs_id_" + iRow] && "off" == ajax_error_count["cs_sustento_" + iRow] && "off" == ajax_error_count["cs_comprobante_" + iRow] && "off" == ajax_error_count["cs_estado_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["cs_id_" + iRow] = "off";
        ajax_error_count["cs_sustento_" + iRow] = "off";
        ajax_error_count["cs_comprobante_" + iRow] = "off";
        ajax_error_count["cs_estado_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_cs_id_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_cs_sustento_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_cs_comprobante_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_cs_estado_" + iRow).removeClass("scFormErrorLine");
    }
  }
