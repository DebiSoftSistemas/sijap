<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="scd_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['scd_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_cambio_deudor']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_cambio_deudor']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_fac_solicitud_cambio_deudor"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_agregar_var_propietario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['scd_propietario_actual']); ?>";
var var_btn_agregar_var_solicitud = "<?php echo $this->form_encode_input($this->nmgp_dados_form['scd_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_seleccion_operacion_transferir_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_propietario*scin' + var_btn_agregar_var_propietario + '*scoutvar_solicitud*scin' + var_btn_agregar_var_solicitud + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinS*scout"; ?>', '', '_self', '0', '0', 'form_fac_seleccion_operacion_transferir');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_cambio_deudor']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_solicitud_cambio_deudor_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_solicitud_cambio_deudor_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function lookup_scd_estado(campo)
{
   dados  = document.F1.scd_estado.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_fac_solicitud_cambio_deudor')?>/form_fac_solicitud_cambio_deudor.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_scd_estado, "scd_estado", dados, false);
}
function recebe_scd_estado(string)
{
   document.getElementById('id_lookup_scd_estado').innerHTML = string;
}
<?php 
