<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="scp_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['scp_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_convenio_pago']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_convenio_pago']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_fac_solicitud_convenio_pago"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_agregar_var_solicitud = "<?php echo $this->form_encode_input($this->nmgp_dados_form['scp_id']); ?>";
var var_btn_agregar_var_propietario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['scp_propietario']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_seleccion_operacion_convenio_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_solicitud*scin' + var_btn_agregar_var_solicitud + '*scoutvar_propietario*scin' + var_btn_agregar_var_propietario + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinS*scout"; ?>', '', '_self', '0', '0', 'form_fac_seleccion_operacion_convenio');
}
...
{
    Crtl_btn_Imprimir = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Imprimir?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_convenio_pago']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_solicitud_convenio_pago_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_solicitud_convenio_pago_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function lookup_scp_estado(campo)
{
   dados  = document.F1.scp_estado.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_fac_solicitud_convenio_pago')?>/form_fac_solicitud_convenio_pago.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_scp_estado, "scp_estado", dados, false);
}
function recebe_scp_estado(string)
{
   document.getElementById('id_lookup_scp_estado').innerHTML = string;
}
<?php 
