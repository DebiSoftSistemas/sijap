...
$old_error_handler = set_error_handler("frm_CambioClave_scriptcase_error_handler");

function frm_CambioClave_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['frm_CambioClave']['contr_erro']) && $_SESSION['scriptcase']['frm_CambioClave']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_frm_CambioClave->contr_frm_CambioClave->Apl_com_erro = true;
            if ($inicial_frm_CambioClave->contr_frm_CambioClave->NM_ajax_flag)
            {
                $inicial_frm_CambioClave->contr_frm_CambioClave->NM_ajax_info['errList']['geral_frm_CambioClave'][] = $msg;
                return;
            }
            frm_CambioClave_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function frm_CambioClave_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['frm_CambioClave'])  && '' != $_SESSION['scriptcase']['error_icon']['frm_CambioClave'])  ? $_SESSION['scriptcase']['error_icon']['frm_CambioClave']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['frm_CambioClave']) && '' != $_SESSION['scriptcase']['error_close']['frm_CambioClave']) ? $_SESSION['scriptcase']['error_close']['frm_CambioClave'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['frm_CambioClave']) && '' != $_SESSION['scriptcase']['error_span_title']['frm_CambioClave']) ? $_SESSION['scriptcase']['error_span_title']['frm_CambioClave'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['frm_CambioClave']) && '' != $_SESSION['scriptcase']['error_icon_title']['frm_CambioClave']) ? $_SESSION['scriptcase']['error_icon_title']['frm_CambioClave'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['frm_CambioClave']['error_buffer'] .= ob_get_flush();
}
