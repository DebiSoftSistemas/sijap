<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="pem_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['pem_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_punto_emision']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_punto_emision']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_punto_emision"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_comprar_var_punto_emision = "<?php echo $this->form_encode_input($this->nmgp_dados_form['pem_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_v_del_promociones_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_punto_emision*scin' + var_btn_comprar_var_punto_emision + '*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scoutNMSC_cab*scinN*scout"; ?>', 'inicio', '_self', '0', '0', 'grid_v_del_promociones');
}
var var_btn_agregar_fisico_var_punto_emision = "<?php echo $this->form_encode_input($this->nmgp_dados_form['pem_id']); ?>";
function sc_btn_agregar_fisico()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_agregar_fisico(); }, 500);
      return;
    }
    sc_btn_agregar_fisico_ok();
}
function sc_btn_agregar_fisico_cancel()
{
}
function sc_btn_agregar_fisico_ok()
{
  tb_show('', '<?php echo $this->Ini->link_form_del_libretin_fisico_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "var_punto_emision*scin' + var_btn_agregar_fisico_var_punto_emision + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=440&width=630&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_punto_emision']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_del_punto_emision_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_del_punto_emision_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
