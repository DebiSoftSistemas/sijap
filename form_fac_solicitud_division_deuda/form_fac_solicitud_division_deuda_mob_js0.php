<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="sdd_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['sdd_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_division_deuda_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_division_deuda_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_fac_solicitud_division_deuda_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_simular_ok()
{
    ...
    document.F1.action = "form_fac_solicitud_division_deuda_mob.php";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_agregar_var_propietario = "<?php echo $this->form_encode_input($this->nmgp_dados_form['sdd_propietario']); ?>";
var var_btn_agregar_var_solicitud = "<?php echo $this->form_encode_input($this->nmgp_dados_form['sdd_id']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_form_fac_seleccion_operacion_dividir_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_propietario*scin' + var_btn_agregar_var_propietario + '*scoutvar_solicitud*scin' + var_btn_agregar_var_solicitud + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinS*scout"; ?>', '', '_self', '0', '0', 'form_fac_seleccion_operacion_dividir');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_fac_solicitud_division_deuda_mob']['where_detal']))
  {
?>  
...
function lookup_sdd_estado(campo)
{
   dados  = document.F1.sdd_estado.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_fac_solicitud_division_deuda_mob')?>/form_fac_solicitud_division_deuda_mob.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_sdd_estado, "sdd_estado", dados, false);
}
function recebe_sdd_estado(string)
{
   document.getElementById('id_lookup_sdd_estado').innerHTML = string;
}
<?php 
