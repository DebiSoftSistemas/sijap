...
class grid_resumen_compras_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_resumen_compras']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_resumen_compras']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tc_nombre' => "tc_nombre",'fc_fecha' => "fc_fecha",'fc_serie' => "fc_serie",'fc_usuario' => "fc_usuario",'pr_identificacion' => "pr_identificacion",'pr_nombre' => "pr_nombre");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('fc_id' => 'N','fc_proveedor' => 'N','fc_total' => 'N','fc_subtotal' => 'N','fc_subtotal_iva' => 'N','fc_subtotal_cero' => 'N','fc_subtotal_no_objeto' => 'N','fc_subtotal_excento' => 'N','fc_total_descuento' => 'N','fc_valor_ice' => 'N','fc_valor_iva' => 'N','fc_valor_irbpnr' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(fc_subtotal), sum(fc_total_descuento), sum(fc_valor_iva), sum(fc_total), sum(fc_subtotal_iva), sum(fc_subtotal_cero), sum(fc_subtotal_no_objeto), sum(fc_subtotal_excento), sum(fc_valor_ice), sum(fc_valor_irbpnr)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         ...
         $comando .= "select fc_subtotal as SC_metric1,fc_total_descuento as SC_metric2,fc_valor_iva as SC_metric3,fc_total as SC_metric4,fc_subtotal_iva as SC_metric5,fc_subtotal_cero as SC_metric6,fc_subtotal_no_objeto as SC_metric7,fc_subtotal_excento as SC_metric8,fc_valor_ice as SC_metric9,fc_valor_irbpnr as SC_metric10, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tc_nombre']['reg'] = "S";
      $format_dimensions['tc_nombre']['msk'] = "";
      $format_dimensions['fc_fecha']['reg'] = "S";
      $format_dimensions['fc_fecha']['msk'] = "";
      $format_dimensions['fc_serie']['reg'] = "S";
      $format_dimensions['fc_serie']['msk'] = "";
      $format_dimensions['fc_usuario']['reg'] = "S";
      $format_dimensions['fc_usuario']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "tc_nombre") {
              }
              if ($Cada_dim == "fc_serie") {
              }
              if ($Cada_dim == "fc_usuario") {
                  $this->Lookup->lookup_sc_free_group_by_fc_usuario($$Cada_dim,  $fc_usuario);
              }
   ...
   //----- 
   function Calc_resumo_fc_tipo_comprobante($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tc_nombre' => "tc_nombre");
      $cmps_quebra_atual = array("tc_nombre");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_fc_tipo_comprobante($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_fc_tipo_comprobante($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'fc_tipo_comprobante', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_fc_tipo_comprobante($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fc_subtotal), sum(fc_total_descuento), sum(fc_valor_iva), sum(fc_total), sum(fc_subtotal_iva), sum(fc_subtotal_cero), sum(fc_subtotal_no_objeto), sum(fc_subtotal_excento), sum(fc_valor_ice), sum(fc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(fc_subtotal), sum(fc_total_descuento), sum(fc_valor_iva), sum(fc_total), sum(fc_subtotal_iva), sum(fc_subtotal_cero), sum(fc_subtotal_no_objeto), sum(fc_subtotal_excento), sum(fc_valor_ice), sum(fc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][5] = $rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $rt->fields[5] = (string)$rt->fields[5]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][6] = $rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $rt->fields[6] = (string)$rt->fields[6]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][7] = $rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $rt->fields[7] = (string)$rt->fields[7]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][8] = $rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $rt->fields[8] = (string)$rt->fields[8]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][9] = $rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $rt->fields[9] = (string)$rt->fields[9]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][10] = $rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $rt->fields[10] = (string)$rt->fields[10]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['tot_geral'][11] = $rt->fields[10]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_fc_tipo_comprobante($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['contr_total_geral'] == "OK") 
      { 
   ...
   //-----  tc_nombre
   function quebra_tc_nombre_sc_free_group_by($tc_nombre, $Where_qb, $Cmp_Name) 
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_tc_nombre[0] = NM_encode_input(sc_strip_script($tc_nombre));
      }
      else {
          $tot_tc_nombre[0] = sc_strip_script($tc_nombre) ; 
      }
      $tot_tc_nombre[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tc_nombre[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_tc_nombre[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_tc_nombre[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_tc_nombre[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_tc_nombre[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_tc_nombre[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_tc_nombre[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_tc_nombre[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_tc_nombre[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_tc_nombre[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_tc_nombre;
   } 

   //-----  fc_fecha
   function quebra_fc_fecha_sc_free_group_by($fc_fecha, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_fc_fecha = array() ;  
      ...
      $tot_fc_fecha[0] = NM_encode_input(sc_strip_script($fc_fecha)) ; 
      $tot_fc_fecha[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fc_fecha[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fc_fecha[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_fc_fecha[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_fc_fecha[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_fc_fecha[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_fc_fecha[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_fc_fecha[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fc_fecha[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fc_fecha[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fc_fecha[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fc_fecha;
   } 

   //-----  fc_serie
   function quebra_fc_serie_sc_free_group_by($fc_serie, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_fc_serie = array() ;  
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_fc_serie[0] = NM_encode_input(sc_strip_script($fc_serie));
      }
      else {
          $tot_fc_serie[0] = sc_strip_script($fc_serie) ; 
      }
      $tot_fc_serie[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fc_serie[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fc_serie[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_fc_serie[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_fc_serie[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_fc_serie[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_fc_serie[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_fc_serie[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fc_serie[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fc_serie[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fc_serie[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fc_serie;
   } 

   //-----  fc_usuario
   function quebra_fc_usuario_sc_free_group_by($fc_usuario, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_fc_usuario = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(fc_subtotal), sum(fc_total_descuento), sum(fc_valor_iva), sum(fc_total), sum(fc_subtotal_iva), sum(fc_subtotal_cero), sum(fc_subtotal_no_objeto), sum(fc_subtotal_excento), sum(fc_valor_ice), sum(fc_valor_irbpnr) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['where_pesq']; 
      } 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_fc_usuario[0] = NM_encode_input(sc_strip_script($fc_usuario));
      }
      else {
          $tot_fc_usuario[0] = sc_strip_script($fc_usuario) ; 
      }
      $tot_fc_usuario[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fc_usuario[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fc_usuario[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_fc_usuario[4] = (string)$rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $tot_fc_usuario[5] = (string)$rt->fields[4]; 
      $rt->fields[5] = str_replace(",", ".", $rt->fields[5]);
      $tot_fc_usuario[6] = (string)$rt->fields[5]; 
      $rt->fields[6] = str_replace(",", ".", $rt->fields[6]);
      $tot_fc_usuario[7] = (string)$rt->fields[6]; 
      $rt->fields[7] = str_replace(",", ".", $rt->fields[7]);
      $tot_fc_usuario[8] = (string)$rt->fields[7]; 
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_fc_usuario[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_fc_usuario[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_fc_usuario[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fc_usuario;
   } 
   ...
   function quebra_pr_identificacion_sc_free_group_by($pr_identificacion, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_pr_identificacion[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_pr_identificacion[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_pr_identificacion[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
   ...
   function quebra_pr_nombre_sc_free_group_by($pr_nombre, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[8] = str_replace(",", ".", $rt->fields[8]);
      $tot_pr_nombre[9] = (string)$rt->fields[8]; 
      $rt->fields[9] = str_replace(",", ".", $rt->fields[9]);
      $tot_pr_nombre[10] = (string)$rt->fields[9]; 
      $rt->fields[10] = str_replace(",", ".", $rt->fields[10]);
      $tot_pr_nombre[11] = (string)$rt->fields[10]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_pr_nombre;
   } 

   //-----  tc_nombre
   function quebra_tc_nombre_fc_tipo_comprobante($tc_nombre, $arg_sum_tc_nombre) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_compras']['where_pesq'])) 
      { 
         $nm_comando .= " where tc_nombre" . $arg_sum_tc_nombre ; 
      } 
      else 
      { 
         $nm_comando .= " and tc_nombre" . $arg_sum_tc_nombre ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
