  ...
  if ($oField.length > 0) {
    switch ($oField[0].name) {
      ...
      case 'emp_tipo_suscripcion':
        sc_exib_ocult_pag('form_admin_empresas_mob_form0');
        break;
      case 'establecimiento':
        sc_exib_ocult_pag('form_admin_empresas_mob_form1');
        break;
      case 'punto_emision':
        sc_exib_ocult_pag('form_admin_empresas_mob_form2');
        break;
      case 'usuarios':
        sc_exib_ocult_pag('form_admin_empresas_mob_form3');
        break;
      case 'productos':
        sc_exib_ocult_pag('form_admin_empresas_mob_form4');
        break;
      case 'tienda_web':
        sc_exib_ocult_pag('form_admin_empresas_mob_form5');
        break;
    }
  }
...
function displayChange_field_tienda_web(row, status) {
    var fieldId;
	if ("on" == status && typeof $("#nmsc_iframe_liga_form_del_config_tienda_config_mob")[0].contentWindow.scRecreateSelect2 === "function") {
		$("#nmsc_iframe_liga_form_del_config_tienda_config_mob")[0].contentWindow.scRecreateSelect2();
	}
}
...
function scHidePage(pageNo) {
	$("#id_form_admin_empresas_mob_form" + pageNo).hide();
}
...
?>
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_admin_empresas_mob_validate_emp_fecha_crea(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
...
function scJQUploadAdd(iSeqRow) {
  $("#id_sc_field_emp_logo" + iSeqRow).fileupload({
    datatype: "json",
    url: "form_admin_empresas_mob_ul_save.php",
    dropZone: "",
    formData: function() {
      return [
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_admin_empresas_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
