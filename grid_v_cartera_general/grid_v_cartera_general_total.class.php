...
class grid_v_cartera_general_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_v_cartera_general']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_v_cartera_general']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['SC_Ind_Groupby'];
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   function Calc_resumo_Mensual($destino_resumo, $res_export=false)
   {
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('periodo_emision' => "periodo_emision",'mes_emision' => "mes_emision");
      $cmps_quebra_atual = array("periodo_emision","mes_emision");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
   ...
   function Res_Totaliza_Mensual($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      while (!$rt->EOF)
      {
          ...
          nmgp_Form_Num_Val($$Cada_dim, "", ",", "0", "S", "2", "", "N:", "-") ; 
              ...
              if ($Cada_dim == "mes_emision") {
                  $this->Lookup->lookup_Mensual_mes_emision($$Cada_dim,  $mes_emision);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang , $con_ruta, $con_id, $op_estado, $mes_emision, $mes_consumo;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_op_fecha_emision_sc_free_group_by($op_fecha_emision, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $con_ruta, $con_id, $op_estado, $mes_emision, $mes_consumo;
   ...
   //-----  periodo_emision
   function quebra_periodo_emision_Mensual($periodo_emision, $arg_sum_periodo_emision) 
   {
      global $tot_periodo_emision, $con_ruta, $con_id, $op_estado, $mes_emision, $mes_consumo;
      $tot_periodo_emision = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq'])) 
      { 
         $nm_comando .= " where periodo_emision" . $arg_sum_periodo_emision ; 
      } 
      else 
      { 
         $nm_comando .= " and periodo_emision" . $arg_sum_periodo_emision ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //-----  mes_emision
   function quebra_mes_emision_Mensual($periodo_emision, $mes_emision, $arg_sum_periodo_emision, $arg_sum_mes_emision) 
   {
      global $tot_mes_emision, $con_ruta, $con_id, $op_estado, $mes_emision, $mes_consumo;
      $tot_mes_emision = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(do_total), sum(do_subtotal), sum(do_valor_iva) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_v_cartera_general']['where_pesq'])) 
      { 
         $nm_comando .= " where periodo_emision" . $arg_sum_periodo_emision . " and mes_emision" . $arg_sum_mes_emision ; 
      } 
      else 
      { 
         $nm_comando .= " and periodo_emision" . $arg_sum_periodo_emision . " and mes_emision" . $arg_sum_mes_emision ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
