...
class grid_resumen_nd_compras_xls
{
   var $Db;
   var $Erro;
   var $Ini;
...
function actionBar_getStateHide($buttonName)
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_resumen_nd_compras/grid_resumen_nd_compras_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              ...
              $this->Res_xls = new grid_resumen_nd_compras_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_resumen_nd_compras.zip";
          $this->Arquivo   .= "_grid_resumen_nd_compras" . $this->Xls_tp;
          $this->Tit_doc    = "grid_resumen_nd_compras" . $this->Xls_tp;
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'] .= $this->Xls_tp;
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
...
$_SESSION['scriptcase']['grid_resumen_nd_compras']['contr_erro'] = 'off';
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_nd_serie()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->nd_serie = html_entity_decode($this->nd_serie, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->nd_serie = strip_tags($this->nd_serie);
         $this->nd_serie = NM_charset_to_utf8($this->nd_serie);
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->nd_serie, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->nd_serie, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- nd_secuencial
   function NM_export_nd_secuencial()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->nd_secuencial = html_entity_decode($this->nd_secuencial, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->nd_secuencial = strip_tags($this->nd_secuencial);
         $this->nd_secuencial = NM_charset_to_utf8($this->nd_secuencial);
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->nd_secuencial, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->nd_secuencial, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- pr_nombre
   ...
   function NM_export_nd_subtotal()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->nd_subtotal = NM_charset_to_utf8($this->nd_subtotal);
         if (is_numeric($this->nd_subtotal))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0.00';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->nd_subtotal);
         $this->Xls_col++;
   }
   //----- nd_valor_iva
   function NM_export_nd_valor_iva()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->nd_valor_iva = NM_charset_to_utf8($this->nd_valor_iva);
         if (is_numeric($this->nd_valor_iva))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0.00';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->nd_valor_iva);
         $this->Xls_col++;
   }
   //----- nd_total
   ...
   function NM_export_nd_fecha_emison_docmod()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->nd_fecha_emison_docmod = substr($this->nd_fecha_emison_docmod, 0, 10);
         if (empty($this->nd_fecha_emison_docmod) || $this->nd_fecha_emison_docmod == "0000-00-00")
         {
             if ($this->Use_phpspreadsheet) {
                 $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->nd_fecha_emison_docmod, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
             }
             else {
                 $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->nd_fecha_emison_docmod, PHPExcel_Cell_DataType::TYPE_STRING);
             }
         }
         else
         {
             $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->nd_fecha_emison_docmod);
             $this->NM_ctrl_style[$current_cell_ref]['format'] = $this->SC_date_conf_region;
         }
         $this->Xls_col++;
   }
   ...
   //----- nd_serie
   function NM_sub_cons_nd_serie()
   {
         $this->nd_serie = html_entity_decode($this->nd_serie, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->nd_serie = strip_tags($this->nd_serie);
         $this->nd_serie = NM_charset_to_utf8($this->nd_serie);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->nd_serie;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
   ...
   //----- nd_secuencial
   function NM_sub_cons_nd_secuencial()
   {
         $this->nd_secuencial = html_entity_decode($this->nd_secuencial, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->nd_secuencial = strip_tags($this->nd_secuencial);
         $this->nd_secuencial = NM_charset_to_utf8($this->nd_secuencial);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->nd_secuencial;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   //----- nd_subtotal
   function NM_sub_cons_nd_subtotal()
   {
         $this->nd_subtotal = NM_charset_to_utf8($this->nd_subtotal);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->nd_subtotal;
   ...
   //----- nd_valor_iva
   function NM_sub_cons_nd_valor_iva()
   {
         $this->nd_valor_iva = NM_charset_to_utf8($this->nd_valor_iva);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->nd_valor_iva;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "right";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0.00";
         $this->Xls_col++;
   ...
   //----- nd_fecha_emison_docmod
   function NM_sub_cons_nd_fecha_emison_docmod()
   {
         $this->nd_fecha_emison_docmod = substr($this->nd_fecha_emison_docmod, 0, 10);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->nd_fecha_emison_docmod;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "data";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = $this->SC_date_conf_region;
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['nolabel'])
       {
   ...
   function quebra_geral_sc_free_total_bot()
   {
       ...
       $mens_tot_base = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['tot_geral'][0] . "(" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['tot_geral'][1] . ")";
       $prim_lin_tot  = true;
     foreach ($NM_tipos_tot as $Tipo_Sum => $Tit_sum)
     {
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['field_order'] as $Cada_cmp)
       {
           if (isset($NM_cmp_tot['nd_subtotal'][$Tipo_Sum]) && $Cada_cmp == "nd_subtotal" && (!isset($this->NM_cmp_hidden['nd_subtotal']) || $this->NM_cmp_hidden['nd_subtotal'] != "off"))
           {
               ...
               $Vl_Tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['tot_geral'][$NM_cmp_tot['nd_subtotal'][$Tipo_Sum]];
               $prim_cmp = false;
           ...
           elseif (isset($NM_cmp_tot['nd_valor_iva'][$Tipo_Sum]) && $Cada_cmp == "nd_valor_iva" && (!isset($this->NM_cmp_hidden['nd_valor_iva']) || $this->NM_cmp_hidden['nd_valor_iva'] != "off"))
           {
               if ($Tipo_Sum == "C" || $Tipo_Sum == "D") {
                   $Format_Num = "#,##0";
               }
               ...
               $Vl_Tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['tot_geral'][$NM_cmp_tot['nd_valor_iva'][$Tipo_Sum]];
               $prim_cmp = false;
               $Vl_Tot = NM_charset_to_utf8($Vl_Tot);
               if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida']) {
                   $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $Vl_Tot;
           ...
           elseif (isset($NM_cmp_tot['nd_total'][$Tipo_Sum]) && $Cada_cmp == "nd_total" && (!isset($this->NM_cmp_hidden['nd_total']) || $this->NM_cmp_hidden['nd_total'] != "off"))
           {
               if ($Tipo_Sum == "C" || $Tipo_Sum == "D") {
                   $Format_Num = "#,##0";
               }
               ...
               $Vl_Tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['tot_geral'][$NM_cmp_tot['nd_total'][$Tipo_Sum]];
               $prim_cmp = false;
               $Vl_Tot = NM_charset_to_utf8($Vl_Tot);
               if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida']) {
                   $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $Vl_Tot;
           ...
           elseif (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['embutida']) {
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE>Reporte de Notas de Débito en Compras :: Excel</TITLE>
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resumen_nd_compras']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
