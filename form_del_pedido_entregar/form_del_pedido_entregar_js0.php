<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="prof_numero" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['prof_numero']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_pedido_entregar']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_pedido_entregar']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_del_pedido_entregar"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_entregar_pedido()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_entregar_pedido(); }, 500);
      return;
    }
    sc_btn_entregar_pedido_ok();
}
function sc_btn_entregar_pedido_cancel()
{
}
function sc_btn_entregar_pedido_ok()
{
    do_ajax_form_del_pedido_entregar_event_scajaxbutton_entregar_pedido_onclick();
}
var var_btn_baucher_factura_var_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['prof_factura']); ?>";
function sc_btn_baucher_factura()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_baucher_factura(); }, 500);
      return;
    }
    sc_btn_baucher_factura_ok();
}
function sc_btn_baucher_factura_cancel()
{
}
function sc_btn_baucher_factura_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_factura_baucher; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_documento*scin' + var_btn_baucher_factura_var_documento + '*scout"; ?>', '', '_blank', '0', '0', 'pdf_factura_baucher');
}
var var_btn_baucher_guia_var_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['prof_guia_remision']); ?>";
function sc_btn_baucher_guia()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_baucher_guia(); }, 500);
      return;
    }
    sc_btn_baucher_guia_ok();
}
function sc_btn_baucher_guia_cancel()
{
}
function sc_btn_baucher_guia_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_baucher_guia; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_documento*scin' + var_btn_baucher_guia_var_documento + '*scout"; ?>', '', '_blank', '0', '0', 'pdf_baucher_guia');
}
var var_btn_nota_venta_var_documento = "<?php echo $this->form_encode_input($this->nmgp_dados_form['prof_factura']); ?>";
function sc_btn_nota_venta()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_nota_venta(); }, 500);
      return;
    }
    sc_btn_nota_venta_ok();
}
function sc_btn_nota_venta_cancel()
{
}
function sc_btn_nota_venta_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdf_a5_factura; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_documento*scin' + var_btn_nota_venta_var_documento + '*scout"; ?>', '', '_blank', '0', '0', 'pdf_a5_factura');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_del_pedido_entregar']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("prof_fecha=disabled;prof_establecimiento=disabled;secuencial=disabled;prof_tipo_entrega=disabled;prof_ruc=disabled;prof_nombre=disabled;prof_email=disabled;prof_direccion=disabled;prof_subtotal=disabled;prof_subtotal_iva=disabled;prof_subtotal_cero=disabled;prof_subtotal_no_objeto=disabled;prof_subtotal_excento=disabled;prof_total_descuento=disabled;prof_valor_ice=disabled;prof_valor_iva=disabled;prof_valor_irbpnr=disabled;prof_propina=disabled;prof_total=disabled", "");
