function grid_jap__seleccionar_contrato_con_ruta_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_ruta" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_ruta_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_ruta" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_cedula_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_cedula" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_cedula_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_cedula" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_nombre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_nombre" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_nombre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_nombre" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_numero_contrato_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_numero_contrato" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_numero_contrato_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_numero_contrato" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_numero_medidor_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_numero_medidor" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_numero_medidor_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_numero_medidor" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_direccion_predio_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_direccion_predio" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_direccion_predio_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_direccion_predio" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_estado_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_estado" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_estado_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_estado" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_seleccionar_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_seleccionar" + seqRow).html();
}

function grid_jap__seleccionar_contrato_seleccionar_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_seleccionar" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_id_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_con_id" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_id_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_id" + seqRow).html(newValue);
}
function grid_jap__seleccionar_contrato_con_id_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_con_id" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_empresa_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_empresa" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_empresa_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_empresa" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_propietario_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_propietario" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_propietario_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_propietario" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_con_telefono_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_con_telefono" + seqRow).html();
}

function grid_jap__seleccionar_contrato_con_telefono_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_con_telefono" + seqRow).html(newValue);
}

function grid_jap__seleccionar_contrato_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("con_ruta" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_ruta_getValue(seqRow);
  }
  if ("con_cedula" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_cedula_getValue(seqRow);
  }
  if ("con_nombre" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_nombre_getValue(seqRow);
  }
  if ("con_numero_contrato" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_numero_contrato_getValue(seqRow);
  }
  if ("con_numero_medidor" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_numero_medidor_getValue(seqRow);
  }
  if ("con_direccion_predio" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_direccion_predio_getValue(seqRow);
  }
  if ("con_estado" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_estado_getValue(seqRow);
  }
  if ("seleccionar" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_seleccionar_getValue(seqRow);
  }
  if ("con_id" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_id_getValue(seqRow);
  }
  if ("con_empresa" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_empresa_getValue(seqRow);
  }
  if ("con_propietario" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_propietario_getValue(seqRow);
  }
  if ("con_telefono" == field) {
    SC_tmp = grid_jap__seleccionar_contrato_con_telefono_getValue(seqRow);
  }
...
function grid_jap__seleccionar_contrato_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("con_ruta" == field) {
    grid_jap__seleccionar_contrato_con_ruta_setValue(newValue, seqRow);
  }
  if ("con_cedula" == field) {
    grid_jap__seleccionar_contrato_con_cedula_setValue(newValue, seqRow);
  }
  if ("con_nombre" == field) {
    grid_jap__seleccionar_contrato_con_nombre_setValue(newValue, seqRow);
  }
  if ("con_numero_contrato" == field) {
    grid_jap__seleccionar_contrato_con_numero_contrato_setValue(newValue, seqRow);
  }
  if ("con_numero_medidor" == field) {
    grid_jap__seleccionar_contrato_con_numero_medidor_setValue(newValue, seqRow);
  }
  if ("con_direccion_predio" == field) {
    grid_jap__seleccionar_contrato_con_direccion_predio_setValue(newValue, seqRow);
  }
  if ("con_estado" == field) {
    grid_jap__seleccionar_contrato_con_estado_setValue(newValue, seqRow);
  }
  if ("seleccionar" == field) {
    grid_jap__seleccionar_contrato_seleccionar_setValue(newValue, seqRow);
  }
  if ("con_id" == field) {
    grid_jap__seleccionar_contrato_con_id_setValue(newValue, seqRow);
  }
  if ("con_empresa" == field) {
    grid_jap__seleccionar_contrato_con_empresa_setValue(newValue, seqRow);
  }
  if ("con_propietario" == field) {
    grid_jap__seleccionar_contrato_con_propietario_setValue(newValue, seqRow);
  }
  if ("con_telefono" == field) {
    grid_jap__seleccionar_contrato_con_telefono_setValue(newValue, seqRow);
  }
  if ("con_id_Hidden" == field) {
    grid_jap__seleccionar_contrato_con_id_Hidden_setValue(newValue, seqRow);
  }
}

function scJQAddEvents(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_seleccionar" + seqRow).click(function () {
    let actionBarButtonRow = "", actionBarButtonState = "", actionBarButtonConfirm = "";
    if ("" != actionBarButtonConfirm) {
      scJs_confirm(actionBarButtonConfirm, function() { scAjaxCall_seleccionar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm); }, function() {});
      return;
    }
    scAjaxCall_seleccionar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm);
  }).mouseover(function() { $(this).css("cursor", "pointer"); })
    .mouseout(function() { $(this).css("cursor", "pointer"); })
    .addClass(($("#id_sc_field_seleccionar" + seqRow).parent().hasClass("scGridFieldOddFont") ? "scGridFieldOddLink" : "scGridFieldEvenLink"));
}
function scAjaxCall_seleccionar_onClick(seqRow, actionBarButtonRow, actionBarButtonState, actionBarButtonConfirm) {
  nmAjaxProcOn();
  $.ajax({
    type: "POST",
    url: "index.php",
    data: "nmgp_opcao=ajax_event&nmgp_event=seleccionar_onClick&script_case_init=" + document.F3.script_case_init.value + "&seleccionar=" + grid_jap__seleccionar_contrato_getValue("seleccionar", seqRow) + "&con_id=" + grid_jap__seleccionar_contrato_getValue("con_id", seqRow) + ""
   })
   .done(function(jsonReturn) {
      ...
      if (oResp["setValue"]) {
        for (i = 0; i < oResp["setValue"].length; i++) {
          grid_jap__seleccionar_contrato_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
        }
      }
