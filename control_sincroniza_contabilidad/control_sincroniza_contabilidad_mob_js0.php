...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_sincroniza_contabilidad_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_sincroniza_contabilidad_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_sincroniza_contabilidad_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_AsientosEmision_ok()
{
    ...
    document.F1.action = "control_sincroniza_contabilidad_mob.php";
...
?>
    ...
    {
        do_ajax_control_sincroniza_contabilidad_mob_navigate_form();
    }
} 
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_control_sincroniza_contabilidad_mob_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "control_sincroniza_contabilidad_mob_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "control_sincroniza_contabilidad_mob_fim.php";
    document.F4.submit();
  }
}
